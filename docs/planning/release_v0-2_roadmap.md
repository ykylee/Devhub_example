...
| **§2 FR 기능 요구사항 (SDLC 요구사항 단계, 2026-06-19 신규 — `docs/requirements/v0.2.0-functional-requirements.md`)** | ✅ | 4가지 기능 (Ingest / Curate / Query / Graph) × 32 cell 매핑 + 20 endpoint request/response schema (Pydantic v2) + Path Y endpoint 별 필수 12 / 권장 8 / 없음 0 + 5 governance field 매핑 + Query scope priority 4-tier (org > personal > project > public) + §3.1 API 매트릭스 1:1 정합 + §3.6 Path Y 정합 + ADR-0035 §3.3 (Python 3.13+ / FastAPI / Pydantic v2) 정합 + §1 도메인 분류 32 cell (REQ-D-001 ~ REQ-D-032) 의 primary/secondary 기능 1:1 매핑 + 28 PoC cell (decision 4 cell 제외, M-v0.2.1+ 검토) |
