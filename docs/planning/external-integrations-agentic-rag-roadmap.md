...
| 2026-06-23 | **`backend-knowledge/` → 외부 repo [`yklee/ai_library`](https://homelab.ddn777.synology.me/gitea/yklee/ai_library) extraction (2026-06-23 결정, vendor import pattern 역방향) + cross-section 정합** — 본 child doc 의 2026-06-17 row 의 "[`backend-knowledge/sources/`](../../backend-knowledge/sources/)" link 가 outdated. 2026-06-23 결정으로 `backend-knowledge/` 디렉터리 본 저장소에서 git rm → 외부 repo [`yklee/ai_library`](https://homelab.ddn777.synology.me/gitea/yklee/ai_library) 로 extraction. 본 child doc 의 source plugin reference 는 umbrella doc 의 §1.2 G3 redirect (2026-06-23 row) 와 정합 — 외부 repo 의 `ai_library/sources/` 가 source plugin 의 SSOT. 본 child doc 의 §3 (Phase 1 adapter pattern) 의 backend-core integration 흡수는 *별도 추적* (vendor import 패턴의 cross-section 정공법 — 본 child doc 의 외부 연동 결정 = umbrella 가 single source-of-truth, 본 child doc 의 §3 의 backend-core reference 는 historical 보존 + umbrella doc 의 §3.1 API 매트릭스 의 backend-core 영향 ❌ 정합). d-1 정공법 = frontend shell + 영향 doc 4종 통째 이식. cross-section 정합 fix 5 위치: 본 §8 row + [umbrella §1.2 G1 redirect](./release_v0-2_roadmap.md) + [umbrella §9 2026-06-23 row](./release_v0-2_roadmap.md) + [ADR-0038 §6 Supersession 신규 row](../adr/0038-backend-knowledge-creation.md) + [AGENTS.md line 29 redirect](../AGENTS.md). | `docs/work_260623-ai-library-extraction` sprint (umbrella 4-way 동기 갱신) |
