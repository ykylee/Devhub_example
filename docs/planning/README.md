...
| [`external-integrations-agentic-rag-roadmap.md`](./external-integrations-agentic-rag-roadmap.md) | **active (2026-06-18, §17.3 P1 row 3 처리 — Q7 결정 기반 umbrella publish signal trigger, PR #645 MERGED 2026-06-18 + PR #647 머지)** | 외부 시스템 연동 (`infrastructure/` + `integrations/adapters/`) 의 **`agentic-integrations/` 통합** + **adapter pattern** + **agentic RAG + agentic plan + tool invocation** long-term 진화. Phase 1 (v1.1) = 디렉터리 통합 + port interface, Phase 2 (v1.2) = RAG retrieve + agentic planner + tool invoker. 사용자 2026-06-10 결정. PR #531 §6.7 명명 재검토 결정의 구체화. **2026-06-18 status active 전환 (Q7 결정 trigger 발동)**. |
| [`release_v0-2_roadmap.md`](./release_v0-2_roadmap.md) | **accepted (2026-06-17 publish, v0.2.0 umbrella doc — 18 main section + 80+ subsection, PR #645 MERGED 2026-06-18)** | **v0.2.0 umbrella doc = 외부 시스템 연동 (`infrastructure/` + `integrations/adapters/`) 분리 + OKF 형 knowledge bundle 생성/관리 + AI agent + 사용자 query 응답. §0~§17 18 main section (§1 컨셉 / §2 신규 백엔드 / §3 3가지 기본 기능 API / §4 1차 raw API / §5 마일스톤 / §6 1차 독립→연동 / §7 Q&A 18/18 / §8 timeline / §9 변경 이력 / §10 DB-based raw / §11 runbook / §12 frontend / §13 cross-cutting / §14 release notes / §15 ADR supersession / §16 API versioning / §17 종합 review). 18/18 Q&A 결정 완료 + 28 metrics M-v0.2.3+ production + 4 cross-cutting 정공법 (§13/§14/§15/§16). Tier: 사외 (vendor-neutral). 5종 PoC source plugin: gitea_repo_pull / gitea_issue / gitea_wiki / gitea_action / homelab_mock. M-v0.2.0 sprint 진입 (PR #645 MERGED + PR #646 state.json + PR #647 child doc active + PR #648 wiki mirror).** |
| [`../adr/0037-okf-adoption.md`](../adr/0037-okf-adoption.md) | **accepted (2026-06-17 publish, ADR 신규)** | **Google OKF v0.1 채택 (1차 출처: Google SPEC.md / README.md, Apache 2.0, 1 concept = 1 .md, frontmatter `type` 1개 필수, 8종 type enum). 1 concept = 1 .md 원칙 + frontmatter 표준화 + `x_devhub_*` prefix 의 DevHub 확장. 본 저장소 의 OKF 형식 영향 (umbrella doc §3.5~§3.9 + §6.5~§6.7 + §10 + §11 + §12 + §13 + §14 + §15 + §16) + ADR-0038 §3 정합. §6 Supersession section 신규 (M-v0.2.3+ 부터 supersession 가능, docs/governance/worker_division.md §4.2 1:1 정합).** |
| [`../adr/0038-backend-knowledge-creation.md`](../adr/0038-backend-knowledge-creation.md) | **accepted (2026-06-17 publish, ADR 신규)** | **신규 백엔드 `backend-knowledge` 신설 (Python 3.13+ / FastAPI / OKF / Pi (pi.dev) v0.79.6 LLM enrich, 완전 standalone — 다른 backend 연결 ❌, OIDC ❌, 외부 시스템 7종 source 만 단방향). Gitea 통합 4 sub-plugin (gitea_repo_pull / gitea_issue / gitea_wiki / gitea_action) + homelab_mock = 5종 PoC. 5 카테고리 결정 (이슈 트래커 / 위키 / SCM / CI-CD / 코드 품질). Path Y caller-provided user context (X-DevHub-User-Context header). §6 Supersession row 추가 (M-v0.2.3+ 부터 supersession 가능).** |
...
| 2026-06-18 | **§5.2 v0.2.0 신규 문서 4 종 인덱스 추가 (umbrella doc + ADR-0037 + ADR-0038 + child doc status 갱신, §17.3 P2 row 6 처리)** — (1) `external-integrations-agentic-rag-roadmap.md` status: `planned (draft, 2026-06-10)` → **`active (2026-06-18, §17.3 P1 row 3 처리 — Q7 결정 기반 umbrella publish signal trigger, PR #645 MERGED 2026-06-18 + PR #647 머지)`** (2) `release_v0-2_roadmap.md` entry 추가 (accepted 2026-06-17 publish, v0.2.0 umbrella doc — 18 main section + 80+ subsection, PR #645 MERGED 2026-06-18) (3) `../adr/0037-okf-adoption.md` entry 추가 (accepted 2026-06-17 publish, Google OKF v0.1 채택, §6 Supersession section 신규) (4) `../adr/0038-backend-knowledge-creation.md` entry 추가 (accepted 2026-06-17 publish, 신규 백엔드 `backend-knowledge` 신설, §6 Supersession row 추가) (5) cross-section 정합 fix: `docs/DOCUMENT_INDEX.md` (deprecated) §5 v0.2.0 신규 문서 4 종 entry 추가 (참조용) + `docs/llm-wiki/operation-sop.md` 상태 + 최종 수정일 갱신 (2026-06-18 v0.2.0 mirror 갱신 trigger) (6) `docs/DOCUMENT_INDEX.md` 가 deprecated 이지만, v0.2.0 신규 문서 진입점 역할로 §5 추가 (거버넌스 README.md 가 진짜 SoT 임 명시) |
