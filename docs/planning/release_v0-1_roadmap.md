...
| **N-9** | **Repository build-runs (P1-7) 구현 + 잔여 4건 sub-issue 모두 완료** — 2026-06-01 통합 테스트 ISSUE-04. **2026-06-11 정합** (sprint `chore/work_260611-d-n9-status-align`): 본 carve 의 endpoint `GET /api/v0-1/repositories/:repository_id/build-runs` + `platformStoreOrUnavailable` 가드 + `ListRepositoryBuildRuns` (postgres.go `repository_ops.go`, `started_at DESC, id DESC` 정렬) + UT 3건 (`TestRepositoryBuildRuns_ReturnsCIRunsData` / `_NoCIRuns` / `_StatusFilter`) + IT 1건 (`TestIntegration_ListRepositoryBuildRuns_StatusFilter`) + openapi.yaml §repositories/build-runs 정의 + frontend `repositoryService.getRepositoryBuildRuns` + `DeveloperView` 위젯 (`buildRuns.length === 0` empty state + "View Logs" modal) + e2e `repository-dashboard.spec.ts` 의 inline… **잔여 4건 sub-issue 분리** (PR #555 본문). **(1) RBAC 404 가드** + **(2) Histogram metric** — 2026-06-11 PR #555 후속 main 반영 완료 (`httpapi/repository_ops.go:144-152` `GetRepositoryByID` 404 가드 + `errRepositoryNotFound` + `repository_ops_test.go:90` 검증 / `httpapi/repository_ops.go:177` `observeBuildRunsQueryDuration` + `metrics.go:29` `BuildRunsQueryDurationSeconds` HistogramVec). **(3) custom hook** + **(4) dashboard widget** — 2026-06-17 sprint `chore/260617-n9-residual-build-runs-polish` 완료. frontend `useRepositoryBuildRuns` (TanStack Query 도입 안함, useState + useEffect, architectural sprint 분리 결정) + `RepositoryBuildRunsSection` (단일 repository 상세 view 의 KPI/Tests sub-section sibling) + `tests/e2e/repository-build-runs-section.spec.ts` (TC-E2E-BUILD-RUNS-SECTION-01). sprint plan [`docs/planning/2026-06-17-n9-residual-build-runs-polish-sprint-plan.md`](./2026-06-17-n9-residual-build-runs-polish-sprint-plan.md). **status**: ✅ resolved (전체 4건 sub-issue). |
</input>
