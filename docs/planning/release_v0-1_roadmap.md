...
| **N-9** | **N-9 잔여 backend sub-issue #556 (RBAC 403/404 가드) + #557 (Histogram metric) 정공법 검증 test 추가** (sprint `chore/260617-n9-sub1-2-backend-validation`, test-only, docs only). 본 P2 mention row 의 본문에서 sub-3 + sub-4 (frontend 2건) + codex P2 review fix 정공법 다룸. **backend 2건 (sub-1 RBAC + sub-2 Histogram) 은 PR #555 (`feat: N-2 draft-publish UT + P1-6 Keycloak revocation + N-3 E2E SCM flow`, 2026-05-30) 의 main 정합 시점에 이미 구현 완료** — handler 의 `GetRepositoryByID` 404 not_found 가드 (line 141-148) + `routePermissionTable` 의 `ResourcePlatformRepositories, ActionView` 매핑 (line 241) + `metrics.go` 의 `devhub_repository_build_runs_query_duration_seconds{status_filter}` 정의 + `repository_ops.go:177` 의 observe 호출. 단 **assertion test 0건** + **routePermissionTable 회귀 test 부재** 의 검증 사각지대를 본 follow-up 으로 정공법 검증. Tier: **공용** (backend test only + docs, 사내 한정 정보 0). (1) `backend-core/internal/httpapi/repository_ops_test.go` (MODIFY, +91 line) — `TestRepositoryBuildRuns_HistogramObserved_WithStatusFilter` + `TestRepositoryBuildRuns_HistogramObserved_NoStatusFilter` 신규 2 case. `dto.Metric` 직접 read 로 histogram SampleCount + SampleSum 정공법 검증. (2) `backend-core/internal/httpapi/permissions_test.go` (MODIFY, +6 line) — `TestEnforceRoutePermission_RoleAllowedAndDenied` 의 build-runs 회귀 row 2개 추가 (developer pass + guest deny-by-default). 검증: build-runs 6/6 PASS + permissions_test 4/4 PASS + `go build ./...` PASS + httpapi+rbac-permissions 3 packages PASS. issue #556 + #557 close 정공법. |
