...
| **X-8** | Keycloak SPI realm events push 전환 (P2-6/P3-5) | BE/사내 | 🟡 **in_progress (compose + docs only, 2026-06-17)** — [`infra/idp/keycloak-event-listener-spi/`](../../infra/idp/keycloak-event-listener-spi/) 의 Java SPI (Keycloak 26.0 + Java 21, [`DevHubEventListenerProvider.java`](../../infra/idp/keycloak-event-listener-spi/src/main/java/com/devhub/keycloak/spi/DevHubEventListenerProvider.java) async sendAsync POST + [`DevHubEventListenerProviderFactory.java`](../../infra/idp/keycloak-event-listener-spi/src/main/java/com/devhub/keycloak/spi/DevHubEventListenerProviderFactory.java) env-var wire) main 정합. `docker-compose.colima.yml` + `docker-compose.deploy.yml` keycloak service 에 SPI JAR volume mount + DEVHUB_BACKEND_SPI_WEBHOOK_URL + DEVHUB_KEYCLOAK_SPI_WEBHOOK_SECRET env 추가 (P2-6 deploy 정공법). `keycloak-realm.{dev,prod}.json` eventsListeners `["jboss-logging", "devhub-event-listener"]` 정합 (CI 의 .ci.json 의도적 미적용). [`docs/setup/keycloak_event_listener_spi.md`](../setup/keycloak_event_listener_spi.md) 정공법 SOP (build → mount → realm config → verify). [`scripts/verify-keycloak-spi.sh`](../../scripts/verify-keycloak-spi.sh) 자동 검증 4 항목 (SPI JAR classpath / Realm eventsListeners / Env var / Webhook push smoke latency < 1s) 작성. backend 변경 0 (webhook handler + polling cron + dedup 모두 main 정합). residual polling cron = at-least-once deliver 보장 (push fail fallback). |
