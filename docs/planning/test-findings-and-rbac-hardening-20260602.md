...
- 범위: 인증/권한(RBAC), 세션 관리, SCM/CI 연동 중 v1.0 릴리즈 영향 항목.
- 상태: active
- 작성일: 2026-06-02
- 입력 문서:
...
- 조치: `POST /api/v1/ci-runs` 구현 + status enum validation + traceability ID 발급.
...
- 조치: `POST /api/v1/auth/logout` + 프론트 signout route 연계 + refresh token revoke 정책 명시.
...
2. BUG-02/BUG-03/ISSUE-04/ISSUE-05를 v1.0 sprint backlog에 P0/P1로 재배치.
