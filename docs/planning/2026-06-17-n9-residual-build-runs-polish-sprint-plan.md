...
| 2026-06-17 | **N-9 잔여 backend sub-issue (#556 RBAC 403/404 가드 + #557 Histogram metric) 정공법 검증 test 추가** (별도 PR `chore/260617-n9-sub1-2-backend-validation`). 본 sprint 의 frontend 2건 (sub-3 + sub-4) 과 별도 backend 검증 scope. (a) #556 sub-1 RBAC 가드: handler 의 `GetRepositoryByID` 404 not_found 가드 (line 141-148) + `routePermissionTable` 의 `ResourcePlatformRepositories, ActionView` 매핑 (line 241) 으로 403 unauthorized middleware 자동 가드. 정공법 검증 회귀 test 2 row 추가 (`TestEnforceRoutePermission_RoleAllowedAndDenied` 의 build-runs developer pass + guest deny-by-default). (b) #557 sub-2 Histogram metric: `metrics.go` 의 `devhub_repository_build_runs_query_duration_seconds{status_filter}` 정의 + `repository_ops.go:177` 의 observe 호출 main 반영. **assertion test 0건** 이 본 follow-up 의 핵심 — `dto.Metric` 직접 read 로 sample_count ≥ 1 + sample_sum ≥ 0 정공법 검증 test 2건 (`TestRepositoryBuildRuns_HistogramObserved_WithStatusFilter` + `TestRepositoryBuildRuns_HistogramObserved_NoStatusFilter`). 검증: build-runs 6/6 PASS (기존 4 + 신규 2) + permissions_test 4/4 PASS (기존 2 sub-test + 신규 build-runs 2 row) + backend `go build ./...` PASS + httpapi+rbac-permissions 3 packages PASS. 신규 ID 발급 0 (test-only). Tier: 공용. | (본 sprint) |
