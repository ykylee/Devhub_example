# Sprint Plan: Test-Driven v0.1.0 Gap Closure

- **문서 목적**: 2026-06-01 통합 테스트 결과 기반 v0.1.0 GAP 해소를 위한 sprint 계획 수립
- **범위**: P0~P1 신규 carve + v0.1.0 sprint 순서 재조정
- **대상 독자**: Claude, Codex, Gemini, Sisyphus
- **상태**: active
- **최종 수정일**: 2026-06-01
- **관련 문서**: `release_v0-1_roadmap.md` (§3.1~3.5 P0/P1 carve), `integrated_test_report_20260601.md` (§8~10 평가)
...
본 sprint plan은 이 GAP을 해소하고 v0.1.0 로드맵(release_v0-1_roadmap.md)에 신규 carve를 통합한다.
...
| 작업 | ID | 영역 | 워커 | 예상 기간 |
|------|----|------|------|----------|
| `POST /api/v0-1/ci-runs` endpoint 구현 | P0-4 (ISSUE-05) | Backend (Go) | Claude | 2일 |
...
| 작업 | ID | 영역 | 워커 | 예상 기간 |
|------|----|------|------|----------|
| `POST /api/v0-1/auth/logout` endpoint (token 폐기 + session 종료) | P1-6 (BUG-03) | Backend | Claude | 1일 |
| `GET /api/v0-1/repos/{id}/build-runs` (ci_runs 기반) | P1-7 (ISSUE-04) | Backend | Claude | 1일 |
...
| v0.1.0 종합 E2E 회귀 테스트 | - | 모두 | 전 워커 | 2일 |
...
| 항목 | 기준 |
|------|------|
| P0-4 CI Run API | `POST /api/v0-1/ci-runs` → 201 + DB 저장 + `GET /api/v0-1/ci-runs` 조회 |
| P1-1 Role sync | Keycloak `devhub_role` 변경 → DevHub `users.role` 자동 반영 |
| P1-6 Sign-out | `POST /api/v0-1/auth/logout` → token expired + 200 |
| P1-7 Build-runs | `GET /api/v0-1/repos/{id}/build-runs` → `ci_runs` 기반 결과 반환 |
| N-10 Manager RBAC | mgr-user-b로 `/api/v0-1/platforms` 조회 → developer와 다른 scope 확인 |
...
| 일자 | 변경 |
|------|------|
| 2026-06-01 | 최초 작성 — 통합 테스트 결과 기반 v0.1.0 GAP sprint plan |
