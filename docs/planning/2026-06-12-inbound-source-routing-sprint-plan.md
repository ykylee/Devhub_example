...
- 문서 목적: ADR-0028 §6 의 (a) carve out — `project.inbound_source` 자동 routing 의 구현 sprint 진입 전 **정공법 + 스코프 + ID slot + 의존** 정합. v1.0 출시 후 v1.1 milestone 진입 후보.
...
- 상태: **draft** (v1.0 출시 직전 검토, 구현은 v1.1 milestone 진입 시점)
- 최종 수정일: 2026-06-12
...
- 관련 문서:
  ...
  - [release_v1_roadmap.md §3.5 N-12](../planning/release_v1_roadmap.md) (voc + notification 정합)
  - [release_v1_roadmap.md §3.5 N-11 (CI e2e 복원)](../planning/release_v1_roadmap.md) (CI 정합)
...
**현재 voc 등록 흐름**:
1. 외부 시스템 → `POST /api/v1/dev-requests/:external_ref` (intake token 인증) → `dev_request_vocs` INSERT (status=received) → `assignee` 에 in-app notification.
2. 담당자/관리자 dashboard → `POST /api/v1/dev-requests/:external_ref/route` (system_admin) → `dev_request_vocs` UPDATE (status=routed) + `dev_requests` INSERT (단일 트랜잭션) → `assignee` 에 in-app notification (의뢰 라우팅 완료).
...
**API**:
- `PATCH /api/v1/platforms/:platform_id` body 에 `inbound_source_type` + `inbound_source_config` 추가 (system_admin).
- `POST /api/v1/dev-requests/:dev_request_id` (voc 등록) 의 응답에 `auto_routed: true|false` + `dev_request_id: <uuid>` 추가 (자동 routing 적용 시).
...
| API | `API-103` | PATCH /api/v1/platforms/:platform_id inbound_source + POST /api/v1/dev-requests/:external_ref auto_routed 응답 |
...
- **본 sprint plan 의 역할**: v1.0 출시 후 **v1.1 milestone 진입 시점** 의 sprint 진입 후보 정합.
- `release_v1_roadmap.md` §4.2 v1.1 milestone 에 본 plan 의 옵션 A 의 ID slot 추가.
- `release_v1_roadmap.md` §3.5 N-13 row 신규 (post-MVP carve a 의 정식 ID).
...
| 9 | **traceability §2.1~§2.6 8 row** + release_v1_roadmap §3.5 N-13 + §4.2 | (없음) | 30분 |
| 10 | **ADR-0028 §6 amendment** (본 plan 결정 사항) | 1-9 | 15분 |
| **합계** | | | **~9시간** (1 sprint) |

## 5. 결정 보류 사유 (현 sprint = post-MVP 검토)

본 plan 은 **option D 검토 = 정공법 + ID slot + 의존 정합** 만 수행. **구현 = v1.1 milestone 진입 시점** 의 별도 sprint 에서 결정.
