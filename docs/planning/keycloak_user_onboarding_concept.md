...
- **2026-05-21** (`main` HEAD `4d882d5` + PR #267 head `9f3cec1` 위에 stack, sprint `claude/onboarding-adr-2026-05-21`): **ADR 발행 done**. `docs/adr/0021-onboarding-self-service-unit-selection.md` 신규 — 컨셉 + 요구사항 + 설계 누적 결정 명문화. ADR-0020 partial supersession (§3.2 신규 user unit 초기 배치 row + §4.1 sub-carve B + §4.2 + §6.2 의 lazy auto-create 결정). ADR-0020 메타 헤더 + §3.2/§4.1/§4.2/§6.1/§6.2 4 위치에 inline supersession banner 추가. ADR-0020 의 핵심 결정 (옵션 A 책임 경계 / `rbac_subject_roles` 제거 / service account 권한 축소) 은 변경 없이 유지. `docs/traceability/report.md` §3 + §4 + §6 동기화. 본 문서 §9 next-phase #3 done. 다음 단계 = #4 IMPL carve 분할.
