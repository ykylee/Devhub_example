...
- 상태: draft
- 최종 수정일: 2026-06-12 (**N-13 housekeeping 정공법 follow-up** — §3.5 N-13 row 의 status `⏳ planned` 유지 + PR #548 close 결과 정공법 (Test 1 e2e seed 중복 + Test 2 Sign-out timeout) + follow-up 결정 (3 branch) + ADR-0028 §6 (a) + sprint plan §3.3/§5 + traceability/report.md §6 + §9 본 row 추가. 본 sprint = docs only housekeeping follow-up, 구현은 v1.1 milestone 진입 시점 별도 sprint.)
...
| **N-13** | **project.inbound_source 자동 routing (ADR-0028 §6 carve a, post-MVP)** — v1.0 출시 후 v1.1 milestone 진입 시점 sprint. ADR-0028 §3 의 voc 흐름 (외부 → voc 등록 → system_admin routing → dev-request 등록) 의 post-MVP 자동화. `applications.inbound_source_type` (gitea/jira/other) + `inbound_source_config` (JSONB) 컬럼 + sync 자동 routing 로직. **scope**: 1 migration (000007) + 1 schema 확장 + 1 API (PATCH /platforms inbound_source) + 1 worker (없음, synchronous routing) + 1 E2E + ADR-0028 §6 amendment. **2026-06-11 정공법** (sprint `feat/work_260611-a-n13-inbound-source-housekeeping`, docs only): ID slot 9 row 발급 + `conventions.md §1` RM 표기 정책 확장 (도메인 prefix 관행 `RM-{domain}-{nn}` 명문화) + `docs/traceability/report.md` §2.1/§2.1.5/§2.2/§2.3/§2.4/§2.5/§2.6 + §3 dev-request row + §4 ADR 인덱스 ADR-0028 row + §6 변경 이력 + 헤더 메타 정합. **status `✅ resolved (implemented, 2026-06-12, sprint `feat/work_260612-7-v1-1-inbound-source-impl`)`** — N-13 follow-up 3 branch 종합 (A: e2e seed fix PR #574, B: signout timeout fix PR #575, C: 구현 follow-up = 본 sprint). routing/auto_route.go + voc_handler 통합 + IT 3 case + openapi.yaml 정합 + e2e TC-INBOUND-SRC-01 + ADR-0028 §6 (a) amendment + 9 ID row 정합 (REQ-FR-113 / UC-DEV-REQ-15 / ARCH-23 / API-103 / RM-DEV-REQ-15 / IMPL-inbound-source-01 / IMPL-platform-patch-02 / UT-inbound-source-01 / TC-INBOUND-SRC-01). 정공법 + ID slot + 의존: [`docs/planning/2026-06-12-inbound-source-routing-sprint-plan.md`](2026-06-12-inbound-source-routing-sprint-plan.md) — 옵션 A 권장 (applications.inbound_source 컬럼 + sync 자동 routing). 검증 보고서 link = N/A (N-10 / N-13 은 code-level 정합 검증 = 구현 sprint 에서 별도).<br>**2026-06-11 PR #548 close 정공법** (sprint `feat/work_260611-a-n13-inbound-source-impl`): CI 11/12 PASS, E2E Internal 1 fail (Test 1: e2e seed 중복 strict mode violation `getByText('e2e-repo-a')` 2 elements + Test 2: Sign-out timeout N-8 race 유사), codex review = COMMENTED (blocker 아님), 자동 재실행 안 됨 (main 의 PR #550 spec timing fix 미적용, run 시각 < PR #550 머지). **follow-up 결정** (2026-06-12, sprint `fix/work_260612-1-n13-housekeeping-followup`): (1) **Test 1 e2e seed 중복** → spec/e2e seed 정합 fix 별도 sprint (mock data 유일성 보장); (2) **Test 2 Sign-out timeout** → main rebase + 자동 재실행 검증 별도 (PR #550 spec timing fix 가 해결 가능성 ↑); (3) **구현 follow-up = v1.1 milestone 진입 시점 별도 sprint** (rebase main + PR #550 fix 적용 + e2e seed 중복 spec fix + 자동 재실행). branch 이름 `feat/work_260611-a-n13-inbound-source-impl` 는 close (PR #548), v1.1 진입 시점의 신규 구현 sprint 는 새 branch 이름 (예: `feat/work_YYMMDD-v1-1-inbound-source-impl`) 별도 결정.
...
| **N-13** (project.inbound_source 자동 routing) | **✅ resolved (implemented, 2026-06-12, sprint `feat/work_260612-7-v1-1-inbound-source-impl`)** | Claude |
