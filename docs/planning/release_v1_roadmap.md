...
| **N-10** | **Manager role RBAC 검증** — E2E seed `bob` (team_manager) 의 권한 scope 확인 + ListProjects/ListPlatforms row filter + org unit subtree scope 검증. 검증 보고서 [docs/validation/N-10-manager-rbac.md](../validation/N-10-manager-rbac.md) (2026-06-04) — V-01..V-10 결과 + P1 follow-up 1건 (E2E spec-vs-구현 갭 6 TC: `TC-RBAC-ROW-READ-01/02` + `TC-RBAC-LOGOUT-01/02` + `TC-RBAC-ROLE-DRIFT-01` + `TC-RBAC-CODE-01` + `TC-RBAC-TRACE-01`). **2026-06-10 follow-up resolved** (sprint `maintenance/work_260610-c-N10-rbac-e2e-tcs`): E2E 4 TC active — `TC-RBAC-LOGOUT-02` + `TC-RBAC-ROW-READ-01` + `TC-RBAC-ROW-READ-02` + `TC-RBAC-CODE-01` (frontend e2e spec `frontend/tests/e2e/rbac-data-scope.spec.ts`); IT/UT 3 TC (`TC-RBAC-LOGOUT-01` / `TC-RBAC-ROLE-DRIFT-01` / `TC-RBAC-LEGACY-01`) + Process 1 TC (`TC-RBAC-TRACE-01` = PR review 단계 cross-ref 검증) 은 spec 문서와 1:1 매핑 + spec file header 주석에 scope-out 명시. **status**: ✅ verified (P1 follow-up resolved, 2026-06-10). | 테스트 | Sisyphus |
...
| 2026-06-10 | **N-10 P1 follow-up resolved — RBAC E2E 4 TC active + spec 문서 1:1 매핑** (sprint `maintenance/work_260610-c-N10-rbac-e2e-tcs`). §3.5 **N-10 row** 의 follow-up 6 TC 중 4 TC 를 `frontend/tests/e2e/rbac-data-scope.spec.ts` 에 active (TC-RBAC-LOGOUT-02 + TC-RBAC-ROW-READ-01/02 + TC-RBAC-CODE-01). 기존 작성 spec 의 header 주석에 본 sprint housekeeping 1 line 추가 + scope-out 명시 (IT/UT 3 TC + Process 1 TC). §3.5 N-10 status `✅ verified (P1 follow-up resolved, 2026-06-10)` 마킹. §9 본 row 추가. 신규 ID 발급 없음 (validation 보고서 §3.1 follow-up 6 TC 의 spec vs 구현 갭 해소). main HEAD `33715d8` (PR #508). | `maintenance/work_260610-c-N10-rbac-e2e-tcs` |
