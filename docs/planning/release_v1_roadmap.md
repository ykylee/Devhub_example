...
| **N-10** | **Manager role RBAC 검증** — E2E seed `bob` (team_manager) 의 권한 scope 확인 + ListProjects/ListPlatforms row filter + org unit subtree scope 검증. 검증 보고서 [docs/validation/N-10-manager-rbac.md](../validation/N-10-manager-rbac.md) (2026-06-04) — V-01..V-10 결과 + P1 follow-up 1건 (E2E spec-vs-구현 갭 6 TC: `TC-RBAC-ROW-READ-01/02` + `TC-RBAC-LOGOUT-01/02` + `TC-RBAC-ROLE-DRIFT-01` + `TC-RBAC-CODE-01` + `TC-RBAC-TRACE-01`). **2026-06-10 follow-up partial** (sprint `maintenance/work_260610-c-N10-rbac-e2e-tcs`): **E2E 4 TC active** — `TC-RBAC-LOGOUT-02` + `TC-RBAC-ROW-READ-01` + `TC-RBAC-ROW-READ-02` + `TC-RBAC-CODE-01` (frontend e2e spec `frontend/tests/e2e/rbac-data-scope.spec.ts`); **IT/UT 3 TC pending** (`TC-RBAC-LOGOUT-01` / `TC-RBAC-ROLE-DRIFT-01` / `TC-RBAC-LEGACY-01` — backend 영역 별도 sprint, Keycloak drift 환경 의존); **Process 1 TC pending** (`TC-RBAC-TRACE-01` = PR review 단계 cross-ref 검증). spec 문서와 1:1 매핑 + spec file header 주석에 scope-out 명시. **status**: ⏳ verified (partial — E2E 4 TC done, IT/UT 3 TC + Process 1 TC scoped to follow-up sprints, 2026-06-10). | 테스트 | Sisyphus |
...
| **N-13** | **project.inbound_source 자동 routing (ADR-0028 §6 carve a, post-MVP)** — v1.0 출시 후 v1.1 milestone 진입 시점 sprint. ADR-0028 §3 의 voc 흐름 (외부 → voc 등록 → system_admin routing → dev-request 등록) 의 post-MVP 자동화. `applications.inbound_source_type` (gitea/jira/other) + `inbound_source_config` (JSONB) 컬럼 + sync 자동 routing 로직. **scope**: 1 migration (000007) + 1 schema 확장 + 1 API (PATCH /platforms inbound_source) + 1 worker (없음, sync 결정). **현 sprint = 정공법 + ID slot + 의존 정합만** (구현은 v1.1 진입 시점 별도 sprint). 정공법: [`docs/planning/2026-06-12-inbound-source-routing-sprint-plan.md`](2026-06-12-inbound-source-routing-sprint-plan.md) §2 (옵션 A 권장) + §3 (ID slot 8건) + §4 (정공법 10 step). **status**: ⏳ planned (v1.1 milestone 진입 시점 구현). | Backend | v1.1 milestone sprint (post-MVP) |
...
| 2026-06-10 | **N-10 P1 follow-up partial — RBAC E2E 4 TC active + 잔여 3 TC + 1 Process scoped-out** (sprint `maintenance/work_260610-c-N10-rbac-e2e-tcs` + codex P2 follow-up `fix/work_260611-c-n10-status-partial-apply`). §3.5 **N-10 row** 의 follow-up 6 TC 중 4 TC 를 `frontend/tests/e2e/rbac-data-scope.spec.ts` 에 active (TC-RBAC-LOGOUT-02 + TC-RBAC-ROW-READ-01/02 + TC-RBAC-CODE-01). 기존 작성 spec 의 header 주석에 본 sprint housekeeping 1 line 추가 + scope-out 명시 (IT/UT 3 TC + Process 1 TC). **codex P2 follow-up 정정** (PR #509 review): 본 PR 의 N-10 status `✅ verified` 가 너무 강함 — IT/UT 3 TC + Process 1 TC 는 별도 sprint / PR review 단계 에서 정합. status `⏳ verified (partial — E2E 4 TC done, IT/UT 3 TC + Process 1 TC scoped to follow-up sprints)` 로 정정. §9 본 row 추가. 신규 ID 발급 없음. main HEAD `cb59b39` (PR #509 + PR #510). | `maintenance/work_260610-c-N10-rbac-e2e-tcs` |
