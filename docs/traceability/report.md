...
| External Integration (provider/binding/homelab) | **부분 활성화** — provider lifecycle (`admin-integrations.spec.ts`, sprint `claude/work_260518-h..j`) + bindings lifecycle (`admin-integration-bindings.spec.ts`, sprint `claude/work_260518-m`) 모두 active. 잔여: ingest (webhook 외부 영향 외) / topology (TC-INT-HOMELAB-03) / degraded provider 시각화. | `docs/tests/test_cases_m4_integration.md` §3 의 TC-INT-FRONTEND-LIST/CREATE/EDIT/SYNC/DELETE/RBAC + TC-INT-FRONTEND-BIND-LIST/CREATE/RBAC active. 잔여: topology v2 (API-76, 78) React Flow spec ts, degraded provider 카드 시각화. | P1 (M4 진입 게이트, frontend 진입점 + bindings 까지 active) |
