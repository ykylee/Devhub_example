...
| 2026-05-18 | sprint `claude/work_260518-c` (본 PR) — **3 신규 ADR 발급**: [ADR-0015](../adr/0015-homelab-adapter-pull-strategy.md) HomeLab adapter pull source 전략 (PR #139 1차 결정 사후 명문화 — file+HTTP mutually exclusive, lightweight goroutine + feature flag, exponential backoff, 메트릭 5종) + [ADR-0016](../adr/0016-prometheus-alerts-policy.md) Prometheus 알림 규칙 정책 (3 alert × stage/prod 임계 분리, Grafana panel 5종, multi-instance `max by(provider)` aggregation) + [ADR-0017](../adr/0017-dreq-intake-token-operational-hardening.md) DREQ intake token 운영 hardening (PR #137 의 `expires_at` + middleware 만료 체크 + PATCH allowed_ips 활성화 사후 명문화). ADR-0014 §6 carve out 갱신 — 2 항목 모두 ADR-0017 로 분기. §4 ADR 인덱스에 3 row 추가. current session |
