...
| **Application/Project 도메인** | REQ-FR-APP-001..012; REQ-FR-PROJ-000..010; REQ-NFR-PROJ-001..006 | UC-APP-01..10, UC-PROJ-01..10 | API-41..50 (activated — §2.2 Application/Repository 서브 표), API-51..58 (planned). ADR-0011 (RBAC row-scoping, accepted). | M3 backlog (design entry) — RM 발급은 후속 sprint | IMPL-application-store-XX (16 메서드 본체) + IMPL-application-handler-XX (10 endpoint, 가드 + audit) + IMPL-application-router-01 + IMPL-application-rbac-01 + 마이그레이션 000012..000018 | UT-application-handler-XX (19 test, sprint claude/work_260514-b) | (carve out — e2e 다음 sprint) |
