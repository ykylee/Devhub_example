...
| 2026-05-18 | sprint `claude/work_260518-d` (본 PR) — **DREQ-E2E TC 정식 발급 + spec ts 정합화**: `docs/tests/test_cases_m5_dreq.md` 신규 카탈로그 (TC-DREQ-* 13건 발급, E2E + UT 영역 매핑 + 카버리지 매핑 표). `frontend/tests/e2e/dev-requests.spec.ts` 정합화 — 기존 mega test 를 `test.step()` 6 단계로 분리 + 각 step 명에 TC ID 명시. 신규 2 test 추가: (1) TC-DREQ-INTAKE-AUTH-NEG-01 invalid bearer → 401, (2) TC-DREQ-ADMIN-TOKEN-PATCH-01 PATCH allowed_ips happy path (API-79). §3 매트릭스 DREQ row TC 컬럼 갱신 (부분 활성화 → 정식 발급 TC-DREQ-* 카탈로그 + 13 TC). | current session |
