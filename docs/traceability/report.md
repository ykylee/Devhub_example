...
| **Dev Request (DREQ) 도메인** | REQ-FR-DREQ-001..011; REQ-NFR-DREQ-001..006 (§5.5) | UC-DREQ-01..10 | ARCH-DREQ-01..06 (`docs/architecture.md §7`); API-59..65 (§14, spec staged, sprint `claude/work_260515-f`) | M5 (Concept staged, sprint `claude/work_260515-f`) — RM 발급은 DREQ-Backend sprint | (미진입 — DREQ-AuthADR 머지 후 IMPL-dreq-{store,handler,router,rbac} + 마이그레이션 000022 진입 예정) | (미진입 — UT-dreq-* 후속 sprint) | (미진입 — TC-DREQ-* 후속 sprint) |
