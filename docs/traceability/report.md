...
| **Dev Request (DREQ) 도메인** | REQ-FR-DREQ-001..011; REQ-NFR-DREQ-001..006 (§5.5) | UC-DREQ-01..10 | ARCH-DREQ-01..06 (`docs/architecture.md §7`); API-59..65 (§14, **activated** sprint `claude/work_260515-i`, API-62 promote 1차 → **promote-tx 단일 트랜잭션 활성화** sprint `claude/work_260515-m`). ADR-0012 (외부 수신 인증, accepted, sprint `claude/work_260515-g`) + ADR-0013 (RBAC row-scoping, accepted, sprint `claude/work_260515-m`). | M5 — Concept staged (sprint `f`) + AuthADR (sprint `g`) + Backend 1차 (sprint `i`) + RBAC-ADR / Promote-Tx (sprint `m`). RM-DREQ-XX 본격 발급은 Frontend/E2E sprint 와 함께. | IMPL-dreq-{domain,store,handler,router,rbac,intake_auth,promote_tx}-01 + 마이그레이션 000022 dev_requests + 000023 dev_request_intake_tokens + 000024 RBAC seed (sprint `i`) | UT-dreq-handler-XX (15 + 5 promote tests, sprint `i` + `m`) + UT-dreq-intake_auth-XX (4) + UT-clientIP-01 | (미진입 — TC-DREQ-* 후속 DREQ-E2E sprint) |
