...
| 2026-05-18 | sprint `claude/work_260518-p` (본 PR) — **ADR-0015 §6 carve outs 부분 처리** (다음 세션 directive 4순위): (1) **size limit + streaming decode** — `HomeLabFilePuller.MaxBytes` + `HomeLabHTTPPuller.MaxBytes` 필드 도입. file mode 는 `os.Stat` 사전 검사 + `os.Open` + `json.NewDecoder` streaming (in-memory full ReadFile 회피), HTTP mode 는 Content-Length 사전 검사 + `io.LimitReader(body, limit+1)` + streaming decode. `Config.HomeLabPullMaxBytes` (env `DEVHUB_HOMELAB_PULL_MAX_BYTES`, default 0 unlimited / 운영 권장 5 MB) + `main.go` puller wire. 회귀 unit test 5건 (file oversized + unlimited + HTTP Content-Length 사전 reject + body over limit + unlimited). (2) **agent token rotation SOP** — `docs/setup/homelab_agent_token_rotation.md` 신규 (발급/주기 rotation/zero-downtime/비상 revoke/자동화 carve out). (3) dedicated worker binary 와 (4) push/pull dedup 정책은 carve out 유지 (M4 진입 시 재평가). ADR-0015 §4.3 env 표에 MAX_BYTES row 추가 + §6 carve out (1)+(2) → resolved + §7 변경 이력 row 추가. backend 단독 변경 — frontend 영향 없음. | current session |
