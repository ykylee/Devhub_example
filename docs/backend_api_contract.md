...
`types`가 비어 있으면 legacy 개발 모드에서는 전체 event를 받을 수 있지만, RBAC policy store가 설정된 환경에서는 `400 Bad Request`로 거절한다. 운영 환경에서는 `actor`/`role` query를 신뢰하지 않고 Bearer token 또는 session 기반 actor context로 전환해야 한다.
...
executor 후보 query는 `command_type=service_action`, `status=pending`, `dry_run=false`, `requires_approval=false`만 반환한다. 2026-05-07 기준 Go Core에는 이 query와 `ServiceActionExecutor` adapter interface가 준비되어 있다.
