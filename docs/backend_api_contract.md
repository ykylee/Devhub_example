...
| API ID | 본문 위치 | 항목 | 상태 |
| --- | --- | --- | --- |
| `API-32` | §16.2 | `GET /api/v1/me` (응답 shape 확장 — `onboarding_required` flag) | **기존 endpoint 응답 확장** (sprint `claude/onboarding-arch-2026-05-21`) |
| `API-83` | §16.3 | `POST /api/v1/me/onboarding` (제출) | spec staged |
| `API-84` | §16.4 | `GET /api/v1/organizations/search` (typeahead) | spec staged |
| `API-85` | §16.5 | `PATCH /api/v1/me` (self-service primary_unit_id 변경) | spec staged |
| `API-33` | §16.6 | `POST /api/v1/users` (admin 사전 등록 — 동작 명시) | **기존 endpoint 동작 명시** (sprint `claude/onboarding-arch-2026-05-21`) |
| `API-86` | §16.7 | `POST /api/v1/admin/users/:user_id/review` (review_status transition) | spec staged |

### 16.2 GET `/api/v1/me` — 응답 확장 (API-32)

기존 `GET /api/v1/me` (§10.1 인덱스 기준 API-32) 의 응답 shape 에 다음 필드를 신규 추가한다:
...
### 16.3 Onboarding 제출 — `POST /api/v1/me/onboarding` (API-83)
...
### 16.4 조직 검색 — `GET /api/v1/organizations/search` (API-84)
...
### 16.5 Self-service 프로필 변경 — `PATCH /api/v1/me` (API-85)
...
### 16.6 관리자 사전 등록 — `POST /api/v1/users` (API-33 확장)

기존 `POST /api/v1/users` (§10.3, API-33) 의 의미를 명시. ADR-0020 sub-carve E (PR #244) 머지 후 password 분기가 제거된 상태이므로 본 endpoint 는 **admin 사전 등록** 의미로 정착.
...
### 16.7 관리자 검토 confirm — `POST /api/v1/admin/users/:user_id/review` (API-86)
