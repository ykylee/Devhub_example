...
## 10. Application 개발 대시보드 (APPDASH) 도메인

Application 상세 대시보드에서 제공해야 할 다차원 롤업 정보, 실시간 헬스 메트릭 및 요구사항-개발-배포 연계 추적을 처리하는 도메인. 컨셉 문서: [`docs/planning/application_dashboard_concept.md`](./planning/application_dashboard_concept.md). 요구사항: [`docs/requirements.md §5.8`](./requirements.md). Usecase: [`UC-APPDASH-01..07`](./planning/system_usecases.md).

### 10.1 아키텍처 구조도 (ARCH-APPDASH-01)
...
### 10.2 헬스 및 품질 스코어 정규화 모형 (ARCH-APPDASH-02)
...
### 10.3 프로젝트 진척 및 리스크 분석 알고리즘 (ARCH-APPDASH-03)
...
### 10.4 DREQ 프로젝트 승격(Promote) 트랜잭션 보장 (ARCH-APPDASH-04)

대기 중인 개발 의뢰(DREQ)를 프로젝트(Project)로 격상시키는 흐름은 분산 데이터 환경에서 일관성을 위해 **단일 데이터베이스 트랜잭션** 내에서 처리됩니다.
...
sequenceDiagram
    ...
    API->>SCM: 3. (Gitea) 기본 브랜치 보호 및 멤버 자동 초대 위임
    alt Gitea Sync Failed
        API->>DB: Rollback Transaction
        API-->>User: 502 Bad Gateway (provider_unreachable)
    else Gitea Sync Success
        API->>DB: 4. DREQ.status = 'registered' & target_id 업데이트 (Update)
        API->>DB: 5. account.onboarding_completed 또는 dreq.promoted Audit 생성 (Insert)
        Note over API: Commit Transaction
        API-->>User: 201 Created (Success)
    end
```

### 10.5 장애 격리 및 우아한 성능 저하 (ARCH-APPDASH-05)
...
### 10.6 Audit action 카탈로그 (ARCH-APPDASH-06)

| action | target_type | payload | 트리거 |
| --- | --- | --- | --- |
| `dev_request.promoted` | `dev_request` | `{ dreq_id, created_project_id, promoted_by }` | DREQ의 프로젝트 승격 완료 시 |
