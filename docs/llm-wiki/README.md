...
| **LLM Wiki** (D-72 신규) | `ai-workflow/wiki/raw/projects/devhub/` (out-of-repo) | LLM agent — RAG source, second brain 의 DevHub 부분 | DevHub repo 의 의미 있는 file (mirror) | in-repo (v0.7.17+, GitHub push, 사외 적격) |
...
- LLM Wiki = `ai-workflow/wiki/raw/projects/devhub/docs/adr/0030-sso-integrations-and-auth-session-port.md` (mirror) — DevHub 의 **SSOT 직접 mirror**, 인간 큐레이션 X.

**두 wiki 의 cross-link 정책**: Public Wiki 는 `docs/` 의 SSOT 정합만 신경. LLM Wiki 는 `ai-workflow/wiki/` 의 SSOT 정합만 신경. **cross-link 없음** (의도적 분리, yklee 결정 2026-06-10).
...
├── mirror-list.md                        ← Phase 1 mirror list (어떤 file 을 ai-workflow/wiki/raw/projects/devhub/ 로 보낼지)
...
### 2.2 `ai-workflow/wiki/` out-of-repo 변경 (Phase 1 mirror 실행 후, 본 PR scope 외)
...
- **lint 자동화 정합**: `docs/llm-wiki/lint-config.toml` 의 L07 ADR 면제 config 가 `ai-workflow/wiki/` 의 wiki-lint 가 자동으로 DevHub 의 의도적 supersede (예: ADR-0030 → ADR-0031) 의 false positive 회피.
...
- **`ai-workflow/wiki/` out-of-repo 변경 미실시**: mirror 실행은 본 PR scope 외. `bash scripts/wiki-sync-devhub.sh` 실행은 사용자 (yklee) 가 직접.
...
| T-d-72-3 | P3 | `ai-workflow/wiki/` 의 `wiki-lint` skill 에 `--project` + `--project-config` 옵션 추가 (my_harness 측 D-73) | T-d-72-2 |
...
**DevHub 의 3-tier 정책 (= GitHub vs 사내 SCM push 분리) 은 DevHub repo 자체에만 적용**. `ai-workflow/wiki/` 는 별도 in-repo (v0.7.17+) repo (my_harness 의 D-72 응답 §3 결정) — 다른 정책.
...
- `scripts/wiki-sync-devhub.sh` + `scripts/wiki-ingest-from-raw.sh` + `scripts/wiki-query.sh` + `scripts/wiki-pr-update.sh` — 4 script 모두 sync/ingest/query/PR-update 의 thin wrapper, GitHub 공개 적격 (실제 vault 갱신 은 in-repo (v0.7.17+) 만).
- 단 `mirror-list.md` 의 mirror source list 에는 DevHub 의 사내 한정 정보 (`DEVHUB_KEYCLOAK_*`, `internal-registry.*`, `kc.internal.example.com`, `devhub.example.com`, `172.16.0.0/12`, `infra/idp/_archive_*/`) 가 포함되지 않음 — D-72 응답 §3 + yklee 결정 (wiki 는 in-repo (v0.7.17+) 만) 으로 `sa-internal/` 격리 불요, 사외/공용 정보만 mirror.
