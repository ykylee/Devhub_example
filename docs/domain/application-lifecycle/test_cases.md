# Application/Project Role-Based View Scope TC

- 문서 목적: 새 역할 모델 (two-dimensional RBAC) 기준 Application/Project view scope 의 test case 를 정의한다.
...
- traceability: 본 문서 TC-088 ~ TC-097 은 `docs/traceability/report.md` §3.6 Application/Project 행의 TC 컬럼에서 참조.
...
| `user_app_leader` | developer | app-alpha LeaderUserID ← application leader |
...
### TC-090: Developer — member 인 project 의 Application 은 LIST 에 포함

- **Given**: `user_dev1` (developer, proj-front member)
- **When**: `ListApplications` 호출
...
## 4. Application Leader: Application 관리 정보 접근

### TC-093: Application Leader — leader 로 지정된 application 의 관리 정보 접근
...
### TC-094: Application Leader — leader 가 아닌 application 의 관리 정보 403
...
| TC-093 | ROLE-006 | Application leader dashboard | P1 | Phase 2 |
| TC-094 | ROLE-007 | Application leader non-lead 403 | P1 | Phase 2 |
