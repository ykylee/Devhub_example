...
1. **Application + Project 운영 모델 + Repository 계층**(§2 / §3, 기존 master `docs/requirements.md` §5.4) — DevHub 의 최상위 계층(Application > Repository > Project > GitHub 실행 단위) + 상태 머신 + 롤업.
2. **Application 개발 대시보드(APPDASH)**(§4 / §5, 기존 master `docs/requirements.md` §5.9) — DevHub 핵심 단위인 Application 상세 대시보드의 기능 요구사항.

## 2. Project 운영 모델 + Application/Repository 계층 (REQ-FR-PROJ / REQ-FR-APP)
...
## 3. Application 개발 대시보드 (REQ-FR-APPDASH / REQ-NFR-APPDASH)

본 절은 컨셉 문서([`./dashboard_concept.md`](./dashboard_concept.md))에 정의된 DevHub 핵심 단위인 Application 상세 대시보드의 기능 요구사항을 정의한다.
