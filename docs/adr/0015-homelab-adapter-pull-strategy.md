...
| 2026-05-18 | codex hotfix #8 P1 #1 + P2 #3 — (P1) `homelab_http_puller.go`: `io.ErrUnexpectedEOF` 가 transient transport 실패 (upstream mid-response close 등) 도 포함하므로 무조건 invalid 분류 회피. `*io.LimitedReader` 보존 + `lr.N == 0` 일 때만 oversized 명시 감지 + 그 외 (`ErrUnexpectedEOF` 포함) retryable=true 유지 → MAX_BYTES enabled 운영의 retry 신뢰성 복원. 회귀 unit test `TestHomeLabHTTPPullerTreatsTransientEOFAsRetryable` (hijack + partial JSON 1차 + 정상 2차) 추가. (P2) `homelab_file_puller.go`: `Decode(&raw)` 후 `dec.Token()` 호출해 `io.EOF` 검증 — 후행 JSON object 가 silent 무시되는 회귀 가드. 회귀 unit test `TestHomeLabFilePullerRejectsTrailingJSON` 추가. | sprint `claude/work_260518-w` (본 PR) |
