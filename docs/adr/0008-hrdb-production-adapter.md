...
| 실시간 sync 요구 (퇴사자 즉시 차단) 시 별도 ADR | **부분 결정 (2026-05-19, sprint `claude/work_260519-g`)**: [docs/infrastructure/keycloak-idp/offboarding_immediacy.md](../infrastructure/keycloak-idp/offboarding_immediacy.md) 신규 design 으로 1차 명문화. Phase 1 = 옵션 C HR ETL → Keycloak Admin API push (hourly ETL + access_token TTL 5분, worst case ≤ 1h). Phase 2 = 옵션 E LDAP/AD federation (≤ 15분). **별도 ADR 발행은 Phase 2 진입 시 재평가** (ADR-0021 은 Onboarding 으로 2026-05-21 발급됨 — Phase 2 진입 시점에 다음 번호 사용) — 1차는 ADR-0019 §5.3 carve resolved 만으로 한정. ADR-0019 와 통합 reference. |
...
| 2026-05-19 | §6 "실시간 sync 요구 (퇴사자 즉시 차단) 시 별도 ADR" 항목 부분 결정 — [docs/infrastructure/keycloak-idp/offboarding_immediacy.md](../infrastructure/keycloak-idp/offboarding_immediacy.md) design 으로 명문화 (Phase 1 HR ETL push + Phase 2 LDAP federation). ADR-0019 §5.3 carve (7) off-boarding 즉시성 design resolved 와 통합. (sprint `claude/work_260519-g`) |
