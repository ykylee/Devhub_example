...
| **M-v0.2.2** | 외부 시스템 6종 source wire + backend-ai 폐기 | 외부 시스템 **6종** source plugin wire (**Gitea 4 sub-plugin** + homelab + metrics) + `backend-ai/` 디렉터리 제거 | M-v0.2.1 | ✅ **done (2026-06-22, PR #663 + #664)** |
...
| 2026-06-23 | **`backend-knowledge` → 외부 repo [`yklee/ai_library`](https://homelab.ddn777.synology.me/gitea/yklee/ai_library) extraction (2026-06-23 결정, vendor import pattern 역방향) — 본 ADR-0038 의 `backend-knowledge/` 신설 결정 자체는 변경 ❌ (흡수 source = `ai_library/adr/0001-ai-library-extraction`, 외부 repo 신설)** | 사용자 결정 ("1. d가 맞아. 2. ~/repos/ai_library. 3. 완전 독립. 4. cross-ref 정합. 5. 1 참고 정리", 2026-06-23). §15 ADR supersession 정공법 정합 (M-v0.2.3+ 부터 supersession 가능). 본 ADR-0038 의 결정 (신규 `backend-knowledge/` 디렉터리 신설 + standalone 정책 + OIDC 제외 + Pi 채택 + §1.2 G7 완전 standalone + §3.5 운영 환경 standalone 정합 + source plugin 7종 + 6 마일스톤) 흡수 + `ai_library` 외부 repo 의 ADR-0001 (ai_library extraction) 로 supersede. **boundary = d-1 정공법**: frontend web/ (SvelteKit 2 + Svelte 5 + vitest 21 src file, 25 file + node_modules 제외) + spec (umbrella §1~§17 + ADR-0037/0038 + child doc + 영향 doc 4종) 통째 이식. cross-section 정합 fix 5 위치: 본 §6 row + umbrella doc §1.2 G1 redirect + §9 2026-06-23 row + ADR-0037 §6 row + child doc §8 row + AGENTS.md line 29 redirect. **본 결정은 본 ADR 의 supersede 가 아닌 *외부 repo 흡수 결정*** — §15 의 5 step 정공법 (New ADR 작성 → 본 ADR frontmatter `superseded-by` 추가 → 본 ADR §6 row 추가 → cross-reference 4~5 file 갱신 → state.json `adrs` field 갱신) 중 1+3+4 적용 (New ADR = `ai_library/adr/0001`, §6 row = 본 row, cross-reference = umbrella + ADR-0037 + child + AGENTS.md 4 file 갱신). deprecation policy: 12개월 semver 정합. **DevHub 측 본 ADR 는 historical 보존** (`docs/adr/_archived/` 이동 ❌, 본 위치 유지 — DevHub 의 ADR index 가 umbrella doc 본 ADR reference 유지하므로), cross-reference 만 `ai_library/adr/0001` 로 redirect. |
