...
- **상태**: Accepted
- **작성일**: 2026-06-17
- **수정일**: 2026-06-18 (umbrella doc §3.5 concept organization + §3.6 Path Y caller-provided user context + §3.7 data normalization pipeline + §3.8 source plugin 작성 정공법 + §3.9 OKF concept 운영 lifecycle 신규 + path pattern 정합 fix 에 따른 §4.3 영향 section 갱신 — §3.2.1 5 카테고리 결정 + §3.3 `x_devhub_category` + §3.5 concept organization + §3.6 data governance & query scoping + §3.7 data normalization pipeline + §3.8 source plugin 작성 정공법 + §3.9 OKF concept 운영 lifecycle 7 row 추가)
