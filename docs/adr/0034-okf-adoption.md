...
- **상태**: Accepted
- **작성일**: 2026-06-17
- **수정일**: 2026-06-18 (umbrella doc §3.5~§3.9 + §10 신규 + path pattern 정합 fix 에 따른 §4.3 영향 section 갱신 — §3.2.1 5 카테고리 결정 + §3.3 `x_devhub_category` + §3.5 concept organization + §3.6 data governance & query scoping + §3.7 data normalization pipeline + §3.8 source plugin 작성 정공법 + §3.9 OKF concept 운영 lifecycle + **§10 DB-based raw + Pi periodic ingest pipeline (dual storage mode: file|db, normalize_mode: rule-based|pi-sdk|pi-rpc)** 8 row 추가)
