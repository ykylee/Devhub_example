...
- **문서 목적**: DevHub v1.0 출시 직전 swagger UI 1차 bootstrap 의 OpenAPI spec 운영 방식 (hand-maintained yaml + 정적 HTML + CDN) 결정을 명문화한다.
...
- **상태**: accepted
- **최종 수정일**: 2026-06-10
- **결정 근거 sprint**: v1.0 출시 직전 swagger UI 1차 bootstrap (코드/문서 정합 본 sprint, 별도 task 완료).
...
v1.0 출시 직전 시점에 DevHub 백엔드 API 가 100+ endpoint 에 도달했으나, 외부/내부 consumer 가 endpoint shape 를 확인할 수 있는 swagger UI 가 부재한 상태였다. 운영자/QA/외부 consumer 가 실시간으로 API contract 를 조회 가능하도록 swagger UI 1차 bootstrap 이 필요했다.

검토 요구:

- (a) swagger UI 즉시 노출 가능 (envelope + health + me + logout + metrics 1차 4 path + 7 schema, 도메인 endpoint 100+ 은 별도 sprint)
- (b) Go 의존성 추가 최소화 (v1.0 출시 직전 회귀 위험 차단)
- (c) Gin v1.12.0 (현재 main 의 Go web framework) 과의 호환 검증 부담 회피
- (d) 향후 v1.1 에서 도메인 endpoint 100+ 를 spec 으로 흡수할 확장성 보장
...
- **옵션 4 (Redocly)**: Redoc 정적 HTML 생성을 Node CLI 로 빌드. 빌드 step 추가 + 사내 Node toolchain 일관성 부담. 장점 = redoc 의 3-pane UI 가 깔끔하나 v1.0 bootstrap 의 4 path spec 에는 과한 투자.
...
3. **의존성 0**: 옵션 3 은 Go module 에 신규 의존성 0개 추가. v1.0 출시 직전 회귀 위험 차단.
4. **v1.0 출시 직전 위험 최소**: 정적 HTML 한 파일 + CDN pin + embed.FS + disk serve = 단일 PR 검토 부담. 회귀 시 옵션 1/2 의 vendor 해제 + 코드 회수 부담 없음.
...
- 인증 미요구 → §5.3 (c) v1.1 system_admin 가드 carve
...
**완화**: §6 carve (b). v1.1 에서 self-host 로 전환 (정적 파일 vendor). 단기 (v1.0) 에서는 CDN 가용성 99%+ 가정 + fallback UI (static "spec 파일 직접 조회 안내" HTML) 는 후속.

### 5.3 인증 미요구

**문제**: swagger UI 페이지 자체는 인증 미요구 (내부 spec 조회).

**완화**: §6 carve (c). v1.1 에서 `system_admin` 가드 미들웨어 추가 (Cookie session 검증). 단기 (v1.0) 에서는 §5.1 의 opt-in + reverse proxy IP allowlist 로 defense in depth.
...
| # | 항목 | 우선순위 | 비고 |
| --- | --- | --- | --- |
| (a) | prod 노출 위험 → reverse proxy IP allowlist 가이드 | P1 | v1.0 운영 SOP 에서 처리 |
| (b) | CDN 의존성 → v1.1 self-host (정적 파일 vendor) | P2 | v1.1 milestone |
| (c) | 인증 미요구 → v1.1 system_admin 가드 | P2 | v1.1 milestone |
| (d) | 도메인 endpoint 100+ spec 확장 | P1 | 별도 sprint (`feat/openapi-domain-extend` 후보) |
| (e) | CI lint gate (openapi.yaml schema validity check) | P3 | v1.1+ |

## 7. 변경 이력

| 일자 | 변경 | sprint |
| --- | --- | --- |
| 2026-06-10 | 1차 발행. v1.0 출시 직전 swagger UI 1차 bootstrap 결정 명문화. 옵션 3 (정적 HTML + CDN) 채택. 결정 근거 4 항목 (Gin v1.12.0 미검증 / Go 1.25 미검증 / 의존성 0 / 위험 최소). carve out 5 항목 (prod 노출 / CDN 의존 / 인증 미요구 / 도메인 endpoint 100+ / CI lint). 신규 ID: `IMPL-swagger-01`. | 본 sprint (v1.0 출시 직전 swagger UI 1차 bootstrap) |
