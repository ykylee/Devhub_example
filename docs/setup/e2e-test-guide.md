...
| **Docker deploy 환경 E2E**에서 `/api/v1/*` 호출이 nginx 404 | frontend bundle 의 `NEXT_PUBLIC_BASE_PATH` 누락 — client 가 root relative `/api/v1/...` 호출 → nginx 의 `/devhub/api/` location 에 도달 못함 | docker build ARG 로 `NEXT_PUBLIC_BASE_PATH=devhub` 전달 (`docs/setup/docker-packaging-deployment-guide.md` §11.2). nginx 에 root `/api/` proxy 추가는 ADR-0018 위반 — 절대 금지 (PR #398 close 사례) |
