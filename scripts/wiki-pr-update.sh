...
#   3. vault side effects (my_harness 의 run_wiki_pr_update.py) — prs/<num>.md 신규 + log.md 1 line + index.md 갱신
#
...
#   1 — gh pr view 실패, 또는 my_harness skill 부재, 또는 vault side effect 실패
...
if [[ "$PROJECT" != "devhub" && "$PROJECT" != "my-harness" ]]; then
  echo "[wiki-pr-update] error: invalid --project: $PROJECT (must be devhub|my-harness)" >&2
  exit 2
fi

# ----- my_harness skill 부재 확인 -----
if [[ ! -f "$WIKI_PR_UPDATE_SCRIPT" ]]; then
  echo "[wiki-pr-update] error: wiki-pr-update skill 미설치: $WIKI_PR_UPDATE_SCRIPT" >&2
  echo "[wiki-pr-update]   my_harness 측 SSOT: ~/repos/my_harness/ai-workflow/skills/wiki-pr-update/" >&2
  echo "[wiki-pr-update]   또는 MYHARNESS_ROOT 환경 변수로 경로 명시" >&2
  exit 1
fi
...
if [[ $REINGEST -eq 1 && -n "$TOUCHED_FILES" ]]; then
  ...
  while IFS= read -r f; do
    [[ -z "$f" ]] && continue
    # mirror-list.md 의 source 와 매칭되는 것만 (ADR / governance / planning / setup / requirements / openapi / ai-workflow-memory)
    case "$f" in
      docs/adr/*|docs/governance/*|docs/planning/*|docs/setup/*|docs/requirements.md|docs/openapi.yaml|ai-workflow/memory/state.json|ai-workflow/memory/session_handoff.md|ai-workflow/memory/work_backlog.md)
        ...
        if [[ $DRY_RUN -eq 0 ]]; then
          if [[ $QUIET -eq 1 ]]; then
            bash "$WIKI_INGEST_WRAPPER" --project "$PROJECT" --source "$f" --apply --skip-lint --quiet
          else
            bash "$WIKI_INGEST_WRAPPER" --project "$PROJECT" --source "$f" --apply --skip-lint
          fi
        else
          if [[ $QUIET -eq 1 ]]; then
            bash "$WIKI_INGEST_WRAPPER" --project "$PROJECT" --source "$f" --skip-lint --quiet
          else
...
# ----- step 3: vault side effects (my_harness skill 호출) -----
log "[wiki-pr-update] step 3/3: vault side effects (prs/<num>.md + log.md + index.md)"

# PR JSON 을 임시 파일로 전달 (긴 body 포함)
...
if [[ -n "$TOUCHED_FILES" ]]; then
  ...
  PR_UPDATE_ARGS+=(--touched-files "$TOUCHED_FILES_TMP")
fi
if [[ $DRY_RUN -eq 0 ]]; then
  PR_UPDATE_ARGS+=(--apply)
fi
[[ $QUIET -eq 1 ]] && PR_UPDATE_ARGS+=(--quiet)

log "[wiki-pr-update]   command: python3 $WIKI_PR_UPDATE_SCRIPT ${PR_UPDATE_ARGS[*]}"
python3 "$WIKI_PR_UPDATE_SCRIPT" "${PR_UPDATE_ARGS[@]}"
PR_UPDATE_EXIT=$?

if [[ $PR_UPDATE_EXIT -ne 0 ]]; then
  echo "[wiki-pr-update] error: vault PR update failed (exit $PR_UPDATE_EXIT)" >&2
  exit 1
fi

log "[wiki-pr-update] DONE"
log "[wiki-pr-update]   prs/<num>.md: $VAULT_ROOT/wiki/projects/$PROJECT/prs/$PR_NUM.md"
log "[wiki-pr-update]   log.md: $VAULT_ROOT/log.md (1 line appended)"
