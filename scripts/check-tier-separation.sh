...
for changed in $diff_output; do
  ...
  # Lint self-detection: 본 lint script 자체는 검사 대상에서 제외 (사내 한정
  # 패턴의 정규식 정의 자체가 매칭되어 false positive 발생). lint 가 본 lint
  # 의 own definition 을 violation 으로 false-alarm 하지 않도록 skip.
  if [[ "$changed" == "scripts/check-tier-separation.sh" ]]; then
    continue
  fi
  # diff 로 + 라인 만 추출
  ...
  # Pattern 매칭 시 comment 라인 (prefix '#') 과 string literal 라인 ('...' 형태) skip.
  # 사내 한정 패턴이 lint script 본인이 아닌 application code 에 매칭되도록.
  added_lines_filtered=$(echo "$added_lines" | grep -vE "^\+\s*#|^\+.*'[a-zA-Z0-9 .,/_()-]+\$'|^\+\s*$" || true)
  for pattern in "${internal_patterns[@]}"; do
    if echo "$added_lines_filtered" | grep -qE "$pattern"; then
      echo "  ❌ $changed: 사내 한정 패턴 매칭 ($pattern)"
      echo "$added_lines_filtered" | grep -E "$pattern" | head -3 | sed 's/^/      | /'
      violations=$((violations + 1))
    fi
  done
done
