...
class ProjectService {
  ...
  async getRepositoryProjects(repositoryId: number, params?: { status?: string; include_archived?: boolean }): Promise<Project[]> {
    const resp = await apiClient.get<{ data: Project[] }>(`/api/v1/repositories/${repositoryId}/projects`, { params });
    return resp.data.data;
  }
