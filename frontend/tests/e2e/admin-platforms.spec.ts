...
test.describe("/admin/settings/platforms — CRUD UI smoke", () => {
  test.beforeEach(async ({ page }) => {
    await loginAs(page, SEEDED.systemAdmin);
    await page.goto(appPath("/admin/settings/platforms"));
    await expect(page).toHaveURL(/\/admin\/settings\/platforms(\/|$)/, { timeout: 20_000 });
    const createBtn = page.getByRole("button", { name: /new platform/i });
    const visible = await createBtn.isVisible().catch(() => false);
    if (!visible && !STRICT_ADMIN_UI) {
      test.skip(true, "platforms create UI unavailable in current build/profile");
    }
    await expect(createBtn).toBeVisible({ timeout: 20_000 });
  });

  test("TC-APP-UI-01 — Platforms 탭 진입 + New Platform 버튼 노출", async ({ page }) => {
    await expect(page.getByRole("button", { name: /new platform/i })).toBeVisible();
  });

  test("TC-APP-UI-02 — New Platform 모달 open/close", async ({ page }) => {
  ...
  test("TC-APP-UI-03 — 필수값 없이 submit 시 브라우저 검증으로 제출 차단", async ({ page }) => {
    ...
    await dialog.getByRole("button", { name: /create platform/i }).click();
  ...
  test("TC-APP-SEARCH-01 — leader/dev unit 키워드로 검색 가능", async ({ page }) => {
    ...
    // Platform Leader ComboBox
    const leaderField = dialog.locator("label", { hasText: "Platform Leader" }).locator("..");
