...
test.describe("application/project detail negative states", () => {
  test("TC-APP-DETAIL-ERR-01 — dashboard 조회 실패 시 에러와 retry 노출", async ({ page }) => {
    // PR #389 가 application detail page 의 데이터 소스를 rollup → dashboard
    ...
    await page.route(/\/api\/v1\/applications\/1\/dashboard$/, async (route) => {
      dashboardRequestCount += 1;
      await route.fulfill({
        status: 500,
        contentType: "application/json",
        body: JSON.stringify({ error: "forced application dashboard failure" }),
      });
    });
