            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar relative z-10">
              {currentMembers.map(member => (
                <div 
                  key={member.id}
                  onClick={() => toggleMember(member.id)}
                  className="flex items-center justify-between p-3 rounded-xl hover:bg-accent/10 cursor-pointer group transition-colors border border-transparent hover:border-accent/20"
                >
                  <button className="p-1.5 rounded-lg bg-red-500/10 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
