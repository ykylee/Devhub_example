...
export function Sidebar({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  ...
  return (
    <aside className={cn("glass border-r border-border w-64 h-screen sticky top-0 flex flex-col z-40", className)} {...props}>
      <div className="p-6 flex flex-col h-full">
        ...
        <div className="mt-auto pt-6 border-t border-border/60">
          {showSystem && (
            <div className="mb-3">
              {renderMenuItem(systemBottomMenu, pathname)}
            </div>
          )}
          <Link href="/account">
  ...
  return (
    <Link key={item.href} href={item.href}>
      ...
      >
        ...
        <item.icon className={cn("h-5 w-5 transition-colors z-10", isActive ? item.color : "group-hover:text-foreground dark:group-hover:text-primary-foreground")} />
