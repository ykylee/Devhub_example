...
export default function PlatformsStatusPage() {
  const [apps, setApps] = useState<PlatformWithRollup[]>([]);
  ...
  const loadData = async () => {
    try {
      setError(null);
      setLoading(true);
      const fetchedApps = await platformService.listPlatforms();
  ...
  return (
    <div className="space-y-10 pb-20">
      <DashboardHeader 
        ...
        subtitle="Real-time monitoring of all production and staging platform services."
      />
      ...
      <FilterBar 
        ...
        placeholder="Search platforms by name or key..."
      />
      ...
      <div className="grid gap-6">
        {filteredApps.map((app, i) => (
          ...
          >
            <div className="flex items-center gap-6 flex-1">
              ...
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <Link href={`/platforms/${app.id}`} className="hover:underline decoration-primary underline-offset-4 decoration-2">
            ...
            <div className="flex items-center gap-12 text-right">
              ...
              <Link href={`/platforms/${app.id}`} className="p-3 rounded-xl hover:bg-muted/30 transition-all text-muted-foreground hover:text-primary">
        ...
        {filteredApps.length === 0 && !loading && (
          <PageEmpty message="No platforms matching your filters" />
        )}
      </div>
    </div>
  );
