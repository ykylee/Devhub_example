...
import { platformService, PlatformDashboard, Platform } from "@/domain/platform-lifecycle/service/platform.service";
...
export default function ApplicationDetailPage() {
  ...
  const [repositories, setRepositories] = useState<PlatformRepository[]>([]);
  ...
  const handlePromoteSubmit = async (e: React.FormEvent) => {
    ...
    // 검증 1: 본 platform 에 연결된 repository 가 있어야 promote 가능.
    const primaryRepo = repositories.find((r) => r.role === "primary") ?? repositories[0];
    if (!primaryRepo) {
      alert("Cannot promote: this platform has no linked repository. Please link a repository first.");
      return;
    }
  ...
  if (loading) {
    return <PageLoading label="Loading platform dashboard..." />;
  }

  if (error || !dashboard) {
    return (
      <div className="space-y-6">
        <PageError message={error || "The requested platform dashboard could not be loaded."} onRetry={() => void loadData()} />
        <div>
          <button
            onClick={() => router.back()}
  ...
  return (
    <div className="space-y-8 pb-20 px-4 md:px-8">
      ...
      <AnimatePresence>
        ...
        {isEditModalOpen && platform && (
          <PlatformCreationModal
            initialData={{
              ...platform,
              status: platform.status as PlatformStatus,
              visibility: platform.visibility as PlatformVisibility,
              start_date: platform.start_date ?? undefined,
              due_date: platform.due_date ?? undefined,
              archived_at: platform.archived_at ?? undefined,
            }}
            onClose={() => setIsEditModalOpen(false)}
            onCreated={(newApp) => {
              toast(`Platform ${newApp.name} updated successfully`, "success");
