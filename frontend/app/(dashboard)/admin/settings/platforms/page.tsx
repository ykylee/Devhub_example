...
import { Platform, PlatformRepository, Project } from "@/domain/platform-lifecycle/schema/project.types";
import { PlatformTable } from "@/domain/platform-lifecycle/view/PlatformTable";
...
import { ProjectTable } from "@/domain/platform-lifecycle/view/ProjectTable";
...
export default function AdminSettingsPlatformsPage() {
  ...
  const [editingApp, setEditingApp] = useState<Platform | null>(null);
  const [selectedApp, setSelectedApp] = useState<Platform | null>(null);
  const [appRepos, setAppRepos] = useState<PlatformRepository[]>([]);
  ...
  }, [platforms, query, activeStatus]);

  const refresh = useCallback(async (showLoading = true) => {
    if (showLoading) setIsLoading(true);
    try {
      const data = await projectService.getPlatforms();
      setPlatforms(data);
    } catch (error) {
      console.error("[admin/settings/platforms] load failed:", error);
  ...
  const handleEdit = (app: Platform) => {
    setEditingApp(app);
    setShowCreateModal(true);
  };

  // archived 상태 → hard-delete (permanent), 그 외 → archive (soft-delete).
  const handleArchive = async (app: Platform) => {
    ...
    try {
      ...
      toast(isHard ? "Failed to delete platform" : "Failed to archive platform", "error");
    }
  };

  const loadAppChildren = useCallback(async (app: Platform) => {
    setSelectedApp(app);
    ...
    const reposPromise = projectService.getPlatformRepositories(app.id).catch((err) => {
      console.error("[admin/settings/platforms] repositories load failed:", err);
      toast("Failed to load repositories for selected platform", "error");
      return [] as PlatformRepository[];
    });
    const projectsPromise = projectService.getPlatformProjectsV2(app.id).catch((err) => {
      // DEVHUB_PROJECT_MODEL=legacy 시 v2 route 410 gone. 빈 array fallback.
      console.warn("[admin/settings/platforms] v2 projects load skipped (likely legacy mode):", err);
      return [] as Project[];
    });
  ...
  const handleDisconnectRepo = useCallback(async (repo: PlatformRepository) => {
    ...
    <div className="space-y-8">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-32 gap-4">
          ...
          <p className="text-muted-foreground font-bold animate-pulse uppercase tracking-[0.3em] text-[10px]">Loading Platforms...</p>
        </div>
      ) : (
        <PlatformTable
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black uppercase tracking-widest text-muted-foreground">
              Platform Scope: {selectedApp.key}
            </h3>
      ...
      <AnimatePresence>
        {showCreateModal && (
          <PlatformCreationModal
            ...
            onCreated={(newApp) => {
              toast(`Platform ${newApp.name} ${editingApp ? 'updated' : 'created'}`, "success");
              refresh();
            }}
          />
        )}
