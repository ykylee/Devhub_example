      ...
      <section className="flex-1 min-h-[600px] glass rounded-3xl border border-border overflow-hidden relative shadow-2xl">
        <div className="absolute top-6 left-6 z-20 flex items-center gap-4">
          ...
          <div className="flex gap-2">
            {["Node View", "Edge View", "Log View"].map((tab) => (
              <button key={tab} className="glass border-border/60 px-3 py-1.5 rounded-xl text-[10px] font-bold text-muted-foreground hover:text-foreground dark:hover:text-primary-foreground transition-all">
                {tab}
              </button>
            ))}
          </div>
        ...
        {isLoading && (
          <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-background/50 backdrop-blur-sm">
            ...
            <p className="text-foreground/65 dark:text-primary-foreground/50 text-xs font-bold uppercase tracking-[0.2em] animate-pulse">Mapping Topology...</p>
          </div>
        )}
