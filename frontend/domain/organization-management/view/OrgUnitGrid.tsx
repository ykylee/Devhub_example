...
import { OrgNode } from "@/domain/organization-management/service/identity.service";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-xl font-black text-foreground dark:text-primary-foreground uppercase tracking-tight">Organization <span className="text-accent">Units</span></h3>
      </div>
          ...
          return (
            <motion.div
              ...
              className={cn(
                ...
                type === 'team' ? "border-accent/20" : "border-border/60"
              )}
            >
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                ...
                <div className="text-right">
                  ...
                  <p className={cn(
                    ...
                    type === 'team' ? "text-accent" : "text-foreground/80"
                  )}>{type}</p>
                </div>
              </div>

              <div className="relative z-10">
                <h4 className="text-lg font-black text-foreground mb-2 truncate" title={node.data.label}>
                  {node.data.label}
                </h4>
                {node.data.leader_id && (
              ...
              <div className="flex items-center justify-between pt-4 border-t border-border/60 relative z-10">
                <div className="flex items-center gap-2">
                  <div className="flex -space-x-2">
                    {Array.from({ length: Math.min(currentMemberCount, 3) }).map((_, i) => (
                      <div key={i} className="w-6 h-6 rounded-full border-2 border-background bg-muted/50" />
                    ))}
                ...
                <button 
                  ...
                  className="text-[10px] font-black uppercase tracking-widest text-accent hover:text-primary-foreground transition-colors flex items-center gap-1 group/btn px-3 py-1.5 rounded-lg bg-accent/10 hover:bg-accent/20"
                >
