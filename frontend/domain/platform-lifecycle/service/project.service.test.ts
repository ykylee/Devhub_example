...
describe("projectService", () => {
  ...
  describe("withQuery / getPlatforms", () => {
    it("serializes query params for getPlatforms", async () => {
      apiClientMock.mockResolvedValue({ data: [] });
      const { projectService } = await import("./project.service");

      await projectService.getPlatforms({ status: "active", include_archived: true, q: "devhub" });

      expect(apiClientMock).toHaveBeenCalledWith(
        "GET",
        "/api/v1/platforms?status=active&include_archived=true&q=devhub",
      );
    });
    ...
    it("returns plain path when no params (undefined)", async () => {
      ...
      await projectService.getPlatforms();

      expect(apiClientMock).toHaveBeenCalledWith("GET", "/api/v1/platforms");
    });
    ...
    it("skips undefined fields in query but keeps defined ones", async () => {
      ...
      await projectService.getPlatforms({ status: "active", include_archived: undefined });

      expect(apiClientMock).toHaveBeenCalledWith("GET", "/api/v1/platforms?status=active");
    });
    ...
    it("returns path without ?qs when all params are undefined (raw empty)", async () => {
      ...
      await projectService.getPlatforms({ status: undefined, include_archived: undefined });
  ...
  describe("Platform CRUD", () => {
    it("createPlatform POSTs body and returns data", async () => {
      apiClientMock.mockResolvedValue({ data: { id: "a1", name: "App" } });
      const { projectService } = await import("./project.service");

      const out = await projectService.createPlatform({ name: "App" });

      expect(apiClientMock).toHaveBeenCalledWith("POST", "/api/v1/platforms", { name: "App" });
      expect(out).toEqual({ id: "a1", name: "App" });
    });

    it("getPlatform GETs /api/v1/platforms/:id", async () => {
      apiClientMock.mockResolvedValue({ data: { id: "a1" } });
      const { projectService } = await import("./project.service");

      const out = await projectService.getPlatform("a1");

      expect(apiClientMock).toHaveBeenCalledWith("GET", "/api/v1/platforms/a1");
      expect(out).toEqual({ id: "a1" });
    });

    it("updatePlatform PATCHes /api/v1/platforms/:id with body", async () => {
      apiClientMock.mockResolvedValue({ data: { id: "a1", status: "on_hold" } });
      const { projectService } = await import("./project.service");

      const out = await projectService.updatePlatform("a1", { status: "on_hold", hold_reason: "freeze" });
    ...
    it("archivePlatform DELETEs plain path when hard is falsy", async () => {
      apiClientMock.mockResolvedValue(undefined);
      const { projectService } = await import("./project.service");

      await projectService.archivePlatform("a1");
      await projectService.archivePlatform("a2", false);

      expect(apiClientMock).toHaveBeenNthCalledWith(1, "DELETE", "/api/v1/platforms/a1");
      expect(apiClientMock).toHaveBeenNthCalledWith(2, "DELETE", "/api/v1/platforms/a2");
    });

    it("archivePlatform DELETEs with ?hard=true when hard is true", async () => {
      apiClientMock.mockResolvedValue(undefined);
      const { projectService } = await import("./project.service");

      await projectService.archivePlatform("a1", true);

      expect(apiClientMock).toHaveBeenCalledWith("DELETE", "/api/v1/platforms/a1?hard=true");
    });
  });

  describe("Repository linking", () => {
    it("getPlatformRepositories GETs /api/v1/platforms/:id/repositories", async () => {
      apiClientMock.mockResolvedValue({ data: [{ platform_id: "a1" }] });
      const { projectService } = await import("./project.service");

      const out = await projectService.getPlatformRepositories("a1");

      expect(apiClientMock).toHaveBeenCalledWith("GET", "/api/v1/platforms/a1/repositories");
    ...
    it("connectRepository POSTs role+repo data", async () => {
      ...
      expect(apiClientMock).toHaveBeenCalledWith(
        "POST",
        "/api/v1/platforms/a1/repositories",
    ...
    it("disconnectRepository URL-encodes the provider/repo composite key", async () => {
      ...
      expect(apiClientMock).toHaveBeenCalledWith(
        "DELETE",
        "/api/v1/platforms/a1/repositories/gitea%2Fteam%2Frepo",
      );
    });
  });

  describe("getRepositoryProjects / getPlatformProjects", () => {
    ...
    it("delegates getPlatformProjects to getPlatformProjectsV2", async () => {
      const { projectService } = await import("./project.service");
      apiClientMock.mockResolvedValueOnce({ data: [{ id: "p1", name: "A" }] });

      const projects = await projectService.getPlatformProjects("app-1");

      expect(projects).toEqual([{ id: "p1", name: "A" }]);
      expect(apiClientMock).toHaveBeenCalledWith("GET", "/api/v1/platforms/app-1/projects");
    });

    it("passes query params through to getPlatformProjectsV2", async () => {
      const { projectService } = await import("./project.service");
      apiClientMock.mockResolvedValueOnce({ data: [] });

      await projectService.getPlatformProjects("app-1", { status: "active" });

      expect(apiClientMock).toHaveBeenCalledWith(
        "GET",
        "/api/v1/platforms/app-1/projects?status=active",
      );
    });
    ...
    it("returns empty array when V2 endpoint returns no data", async () => {
      ...
      const projects = await projectService.getPlatformProjects("app-empty");
  ...
  describe("Project create flow", () => {
    ...
    it("getPlatformProjectsV2 with params", async () => {
      apiClientMock.mockResolvedValue({ data: [] });
      const { projectService } = await import("./project.service");

      await projectService.getPlatformProjectsV2("a1", { status: "active" });

      expect(apiClientMock).toHaveBeenCalledWith(
        "GET",
        "/api/v1/platforms/a1/projects?status=active",
      );
    });

    it("getPlatformProjectsV2 without params", async () => {
      apiClientMock.mockResolvedValue({ data: [] });
      const { projectService } = await import("./project.service");

      await projectService.getPlatformProjectsV2("a1");

      expect(apiClientMock).toHaveBeenCalledWith("GET", "/api/v1/platforms/a1/projects");
    });
  ...
  describe("createPlatformProject — ApiError fallback (404/405)", () => {
    it("happy path POSTs to /api/v1/platforms/:id/projects", async () => {
      apiClientMock.mockResolvedValue({ data: { id: "p1" } });
      const { projectService } = await import("./project.service");

      const out = await projectService.createPlatformProject("a1", { name: "P" });

      expect(apiClientMock).toHaveBeenCalledWith(
        "POST",
        "/api/v1/platforms/a1/projects",
        { name: "P" },
      );
    ...
    it("falls back to createProject when ApiError 404 + repository_ids present", async () => {
      ...
      const out = await projectService.createPlatformProject("a1", {
        name: "P",
        repository_ids: [11],
      });
      ...
      expect(apiClientMock).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({ name: "P", platform_id: "a1", repository_ids: [11] }),
      );
    ...
    it("falls back to createProject when ApiError 405 + repository_id (singular)", async () => {
      ...
      expect(apiClientMock).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({ platform_id: "a1" }),
      );
    ...
    it("re-throws ApiError 404 when no repository hint available", async () => {
      ...
      await expect(
        projectService.createPlatformProject("a1", { name: "P" }),
      ).rejects.toBe(err);
    });
    ...
    it("re-throws ApiError with other status (e.g., 500) — does not fallback", async () => {
      ...
      await expect(
        projectService.createPlatformProject("a1", { name: "P", repository_ids: [1] }),
      ).rejects.toBe(err);
  ...
  describe("listAllProjects — aggregates standalone + per-platform", () => {
    it("returns standalone + per-platform projects, deduped by id", async () => {
      const { projectService } = await import("./project.service");
      apiClientMock
    ...
    it("swallows standalone fetch failure and continues with per-platform", async () => {
    ...
    it("swallows per-platform fetch failure and continues with other apps", async () => {
      const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => undefined);
      const { projectService } = await import("./project.service");
      apiClientMock
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining("per-platform fetch failed"),
        expect.any(Error),
      );
    });

    it("forwards params (status, include_archived) to standalone + per-platform", async () => {
      const { projectService } = await import("./project.service");
      apiClientMock
        .mockResolvedValueOnce({ data: [] })
        .mockResolvedValueOnce({ data: [] });
      ...
      expect(apiClientMock).toHaveBeenCalledWith(
        "GET",
        "/api/v1/platforms?include_archived=true",
      );
    });

    it("returns [] when both standalone and platforms list return empty", async () => {
      const { projectService } = await import("./project.service");
      apiClientMock
        .mockResolvedValueOnce({ data: [] })
        .mockResolvedValueOnce({ data: [] });
