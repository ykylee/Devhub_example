...
const createPlatformProject = vi.fn();
...
vi.mock("@/domain/platform-lifecycle/service/project.service", () => ({
  projectService: {
    ...
    createPlatformProject: (...a: unknown[]) => createPlatformProject(...a),
...
beforeEach(() => {
  ...
  createPlatformProject.mockReset();
...
describe("ProjectCreationModal", () => {
  ...
  it("creates a standalone project when no platform is selected", async () => {
  ...
  it("creates an platform-bound project when platformId is provided", async () => {
    const created: Project = {
      id: "p-new",
      key: "P2",
      name: "Bound",
    ...
    createPlatformProject.mockResolvedValueOnce(created);
    ...
    await waitFor(() => {
      expect(createPlatformProject).toHaveBeenCalledWith(
        "app-1",
        expect.objectContaining({ platform_id: "app-1" }),
      );
    });
  });
  ...
  it("falls back to empty platforms/scm/users arrays when service calls reject", async () => {
    getPlatforms.mockReset();
    listProviders.mockReset();
    getUsers.mockReset();
    getPlatforms.mockRejectedValueOnce(new Error("a"));
  ...
  it("promotes another member to leader and demotes existing leader (role-change branch)", async () => {
    ...
    // querySelector("select") would also match platform/status/visibility/repo
  ...
  it("renders existing platforms in the platform select and lets user choose one", async () => {
    getPlatforms.mockReset();
    getPlatforms.mockResolvedValueOnce([
    ...
    const appSelect = screen.getByDisplayValue("No platform (independent)") as HTMLSelectElement;
    ...
    await waitFor(() => {
      expect(createPlatformProject).toHaveBeenCalledWith(
        "app-a",
        expect.objectContaining({ platform_id: "app-a" }),
      );
    });
  });
