...
- 상태: M1·M2·M3 done (1차). Application Domain backend 1차 (2026-05-14). DREQ 도메인 1차 + carve out 1/4 + 2/4 (2026-05-15). **2026-05-18 메모리 갭 동기화 세션** — 직전 claude EOD (PR #132) 이후 외부 에이전트 8 PR (#133~#140) 미반영분 흡수. **DREQ P2 carve out 6/6 모두 해소 확인** + **DREQ E2E carve out 3/4 부분 흡수** (PR #136 / #138 / #140) + **External Integration 도메인 신규 1차 완성** (concept staged PR #135 → backend 1차 PR #139, API-69..78 + HomeLab pull adapter + Prometheus + migration 000028/000029). main HEAD `caa80c7`. **다음 세션 directive**: (1) traceability §3 외부 PR ID 매핑 검수, (2) DREQ-E2E intake auth Playwright spec 잔여, (3) External Integration design/impl carve out (frontend 진입점 + ADR 발급) — session_handoff.md "다음 세션 directive" 절 참조.
- 최종 수정일: 2026-05-18
...
| **DREQ Domain (full + Admin-UI + token hardening)** | ✅ done (1차) | 2026-05-17 | API-59..68 activated + ADR-0012/0013/0014 + intake token admin (plain 1회 노출, idempotent revoke) + token expires_at + PATCH allowed_ips (PR #137). P2 carve out 6/6 해소 (PR #137/#140 + DestructiveConfirmModal + Show/Hide toggle 등). 잔여: intake auth Playwright spec, TC-DREQ-* 정식 발급. |
| **External Integration Domain (backend 1차)** | 🟢 1차 완료 | 2026-05-17 | concept staged (PR #135) → backend 1차 (PR #139). API-73..78 활성화, HomeLab pull adapter (file/HTTP puller + retry/backoff + pull loop scheduler), Prometheus `/metrics`, migration 000028 integration_registry + 000029 infra_service_snapshots. 잔여: frontend 진입점, API-69..72 wire 검증, ADR 발급. |
...
| 2026-05-18 | **메모리 갭 동기화 세션** — 새 코드 작업 없이 flat memory (`state.json` / `session_handoff.md` / `work_backlog.md`) 갱신. 직전 sprint q (PR #132, `253063e`) 이후 외부 에이전트 8 PR (#133~#140) 흡수. **P2 carve out 6/6 모두 해소 확인** (코드 grep + 파일 존재 검증): (1) promote-tx race `WHERE status IN ('pending', 'in_review')` `dev_requests.go:266` + `dev_requests_promote.go:29`, (2) `failPromote` dead field 제거, (3) `DestructiveConfirmModal` 도입 + wire, (4) plain_token `showToken` toggle + Eye/EyeOff, (5) expires_at migration 000027, (6) PATCH allowed_ips. **DREQ carve out 3/4 (E2E) 부분 흡수** (PR #136 + #138 + #140). **External Integration 도메인 신규 1차 완성** (concept staged PR #135 → backend 1차 PR #139). main HEAD `caa80c7`. 다음 directive: traceability §3 외부 PR ID 매핑 검수 → intake auth Playwright spec → External Integration design/impl carve out. |
