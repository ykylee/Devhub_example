...
- 상태: **2026-06-11 update — main HEAD `3e2701b3` (v0.1.1-alpha 격하 정공법, 2026-06-11 06:30 UTC) + PR #554 (N-6 skip + v1.0 release blocker 해제, mergeCommit `d860b7c9`, 2026-06-11 06:27 UTC) + PR #555 (v0.1.0-alpha release 발표 자료 — CHANGELOG + md + HTML 슬라이드, commit `356d08b7`) + PR #556 (v0.1.1-alpha 격하 정공법 — 잔여 5 의 v0.1.x patch release, commit `3e2701b3`). 06-09~06-11 사이 PR #514~#554 (40 PR) 정합. **N-9 (P1-7 Repository build-runs) 기본 구현 완료 정합** (sprint `chore/work_260611-d-n9-status-align`, PR 머지 후): release_v1_roadmap.md §3.5 N-9 row status `✅ resolved (기본 구현, 2026-06-11)` + §3.2 P1-7 row 비고 갱신 + §9 변경 이력 1 row + state.json M-v1.0 notes 보강. **잔여 4건 sub-issue 분리** (v1.1 milestone 진입 시점): (a) RBAC 403/404 가드 (backend) / (b) Histogram metric `devhub_repository_build_runs_query_duration_seconds{status_filter}` (backend) / (c) TanStack Query `useRepositoryBuildRuns` hook + status filter dropdown + skeleton + 무한 스크롤 (frontend) / (d) dashboard widget "Recent repository activity" 통합 + 독립 e2e spec `tests/e2e/repository-build-runs.spec.ts` (TC-E2E-BUILD-RUNS-01). issue [#487](https://github.com/ykylee/Devhub_example/issues/487) (정식 ID = N-9) — PR 머지 후 close. 신규 ID 발급 0건 (docs only 정합).
