...
| ADR-0017 | (b) PATCH expires_at | ✅ resolved | adr0017-token-mutation |
...
3. **ADR-0018 승격 + Phase 2 staging** (단일 포트 reverse proxy) — staging 환경 nginx 적용 + 1주 검증
4. **ADR-0019 승격 + Phase 2 staging** (Keycloak SSO federation) — 사내 IT 부서 + 보안팀 검토 후 진입
5. **External Integration 후속 강화** — React Flow group sub-node + WebSocket 실시간 + v2 node click action
6. **M4 RM-M4-XX 본격 진입** — WebSocket 확장 / AI Gardener gRPC / System Admin 대시보드 / Gitea Hourly Pull
7. **Bindings UI 강화** — scope_id lookup combobox / Edit/Delete binding / pagination
