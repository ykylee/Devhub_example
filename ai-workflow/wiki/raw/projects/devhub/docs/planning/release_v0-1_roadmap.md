# DevHub v0.1.0 릴리즈 로드맵 — 1차 릴리즈 scope + 우선순위 + 워커 분업

- 문서 목적: DevHub 의 1차 릴리즈 (v0.1.0) 를 위한 단일 source-of-truth 로드맵. 잔여 carve 통합 인벤토리 + 우선순위 P0~P3 + 마일스톤 재정의 + 워커(Claude/Codex/Gemini/Reasonix/OpenCode) 분업.
- 범위: 1차 릴리즈에 포함될 기능 scope, 제외 기능, 잔여 carve 우선순위, 신규 마일스톤(M-v0.1.0, M-v0.1.1, M-v2), GitHub project + milestone 등록 plan, UI 검증 방식.
- 대상 독자: 프로젝트 리드, 모든 워커 (Claude, Codex, Gemini, OpenCode), 후속 작업자.
- 상태: draft
- 최종 수정일: 2026-06-12 (**N-13 housekeeping 정공법 follow-up** — §3.5 N-13 row 의 status `⏳ planned` 유지 + PR #548 close 결과 정공법 (Test 1 e2e seed 중복 + Test 2 Sign-out timeout) + follow-up 결정 (3 branch) + ADR-0028 §6 (a) + sprint plan §3.3/§5 + traceability/report.md §6 + §9 본 row 추가. 본 sprint = docs only housekeeping follow-up, 구현은 v0.1.1 milestone 진입 시점 별도 sprint.)
- 직전 결정 근거: 2026-06-11 (N-13 P1 follow-up 정공법 — ID slot 9 row 발급 + conventions.md §1 RM 표기 확장 + ADR-0028 §6 (a) 정공법, sprint `feat/work_260611-a-n13-inbound-source-housekeeping`, PR #547 머지, main `f37305d7`)
- 결정 근거 sprint: `fix/work_260612-1-n13-housekeeping-followup` (본 문서, N-13 housekeeping follow-up), `claude/work_260520-f-roadmap` (1차), `feat/work_260611-a-n13-inbound-source-housekeeping` (직전, ID slot 발급)
...
본 문서는 **2026-05-20 이후 모든 sprint 의 진입점**. 기존 [`docs/development_roadmap.md`](../development_roadmap.md) (M0~M6 historical) 는 done milestone 의 사후 명문화 자산으로 보존 — 본 문서가 v0.1.0 / v0.1.1 / v2 의 새 source-of-truth.
...
3. **마일스톤 진행** §4 의 M-v0.1.0/M-v0.1.1/M-v2 표에서 본 sprint 의 마일스톤 + 동반 issue 확인
...
| --- | --- | --- |
| **인증/조직/대시보드** | Keycloak OIDC 로그인 + `/admin/settings/*` (users/organization/permissions + audit + dev-requests + dev-request-tokens + integrations + integration-bindings) + dashboard (developer/manager/admin) + 역할 routing | Backend done (M1/M2/M3). Frontend done. **계정/사용자 리팩토링 Phase 3 sub-carve A done** (ADR-0020, sprint -d). sub-carve B (`/api/v0-1/accounts/*` 폐기 + lazy auto-create) 가 v0.1.0 의 마지막 큰 backend 변경. |
...
| **WebSocket replay + 리소스 필터링** | v0.1.1 carve | RM-M4-02 — backend WebSocket 기반 인프라 + last event replay. v0.1.0 은 polling/refresh 로 충분 |
| **System Admin 대시보드 (Runner 상태)** | v2 P3 | RM-M4-07 — Gitea Runner 운영 view. v0.1.0 scope 외 |
| **Gitea Hourly Pull worker** | v2 P3 | RM-M4-06 — 외부 SCM 비동기 동기화. v0.1.0 은 HomeLab pull 만 |
| **외부 SSO (Gitea 연동 등)** | v0.1.1 carve | RM-M4-09 — Keycloak identity broker. v0.1.0 은 단일 Keycloak realm 전제 |

### 1.3 v0.1.0 완료 정의 (DoD)
...
7. 사내 staging 환경 1주 운영 + 외부 사용자 ≥ 5명 로그인 동작 ([Onboarding 운영 SOP](../setup/onboarding_operations.md) §7 DoD 8 항목 통과) — **(N-6 skipped, 사용자 결정, 2026-06-11, v0.1.0 release blocker 해제)**
...
| 모듈 | 위치 (backend) | 위치 (frontend) | v0.1.0 상태 |
| --- | --- | --- | --- |
...
| 조직 단위 (units + hierarchy) | `internal/httpapi/organization.go` (`/api/v0-1/organization/*`) | `app/(dashboard)/admin/settings/organization/page.tsx` | ✅ done |
...
| Provider webhook ingest (push) | `POST /api/v0-1/integration/providers/:provider_id/webhook` | — | ✅ done (Bypass) |

## 3. 잔여 carve 통합 인벤토리 + 우선순위 매트릭스

### 3.1 P0 — v0.1.0 릴리즈 차단 (sprint -f 즉시 진입)

| ID | Carve | 출처 | 워커 | 비고 |
| --- | --- | --- | --- | --- |
| **P0-1** | ADR-0020 sub-carve B — `/api/v0-1/accounts/*` 4 endpoint 제거 + lazy auto-create + frontend `account.service.ts` 폐기 + admin/settings/users 페이지 정리 | sprint -d ADR-0020 §4.1 B | **Claude (backend)** + **Gemini (frontend done ✅)** | v0.1.0 Keycloak 단일 IdP 정합의 마지막 큰 변경. e2e TC-ACC-* 갱신 동반 |
...
| **P1-6** | **Sign-out endpoint 구현** (`POST /api/v0-1/auth/logout`) — access token 폐기 + session 종료 | 2026-06-01 통합 테스트 BUG-03 | **Claude (backend)** | **신규 P1** — 세션 관리 기본. refresh token rotate 포함 여부 결정. 정식 GitHub issue = §3.5 N-8 ([#488](https://github.com/ykylee/Devhub_example/issues/488)). |
| **P1-7** | **Repository build-runs endpoint 구현** (`GET /api/v0-1/repos/{id}/build-runs`) — `ci_runs` 테이블 기반 repo-scoped 조회 | 2026-06-01 통합 테스트 ISSUE-04 | **Claude (backend) + Gemini (frontend)** | **기본 구현 완료** (2026-06-11, N-9 정합). 정식 GitHub issue = §3.5 N-9 ([#487](https://github.com/ykylee/Devhub_example/issues/487)). 본 carve 의 endpoint `GET /api/v0-1/repositories/:repository_id/build-runs` (main 컨벤션, `/repos` 약칭이 아닌 repositories 풀패스) + `platformStoreOrUnavailable` 가드 + UT 3건 + IT 1건 + frontend `repositoryService.getRepositoryBuildRuns` + `DeveloperView` 위젯 모두 main 반영. **잔여 4건 sub-issue 분리** (v0.1.1 milestone 진입 시점): RBAC 403/404 가드 / Histogram metric `devhub_repository_build_runs_query_duration_seconds` / TanStack Query `useRepositoryBuildRuns` hook + status filter dropdown + skeleton / dashboard widget "Recent repository activity" 통합 + 독립 e2e spec. |

### 3.3 P2 — v0.1.0 운영 안정성 + v0.1.1 carve
...
| **P2-4** | Bindings UI 강화 — scope_id lookup combobox + Edit/Delete + pagination | development_roadmap §6 잔여 | **Gemini (frontend+UX done ✅)** | v0.1.0 UI polish 동반. backend CRUD 지원 추가 완료. |
...
| **P3-6** | RM-M4-01..03 WebSocket 확장 (replay + 리소스 필터링 + command status UI) | development_roadmap M4 | **Claude (backend) + Gemini (frontend)** | v0.1.1 또는 v2 |
...
> v0.1.1 영역 작업(Gitea SCM sync·SCM↔시스템 양방향·auth_mode full·repository draft/publish·admin catalog)이 **v0.1.0 전에 backend 주도로 선행**됐다. 그 결과 코드는 앞서고 테스트·운영 가시성·문서가 뒤처진 상태 → 다음 사이클은 "굳히기(harden)" 우선. 상세 분석은 [코드베이스 스냅샷 §06 향후 방향](../analysis/2026-05-27-codebase-snapshot/06_future_direction.md).

#### NOW — v0.1.0 마감 + 품질 굳히기
...
| **N-6** | v0.1.0 staging 1주 운영 검증 (외부 사용자 ≥5 로그인 + Onboarding SOP DoD 8) | 사내 | 사용자 | **status**: ✅ skipped (사용자 결정, 2026-06-11, v0.1.0 release blocker 해제).
...
| **N-9** | **Repository build-runs (P1-7) 구현** — 2026-06-01 통합 테스트 ISSUE-04. **2026-06-11 정합** (sprint `chore/work_260611-d-n9-status-align`): 본 carve 의 endpoint `GET /api/v0-1/repositories/:repository_id/build-runs` + `platformStoreOrUnavailable` 가드 + `ListRepositoryBuildRuns` (postgres.go `repository_ops.go`, `started_at DESC, id DESC` 정렬) + UT 3건 (`TestRepositoryBuildRuns_ReturnsCIRunsData` / `_NoCIRuns` / `_StatusFilter`) + IT 1건 (`TestIntegration_ListRepositoryBuildRuns_StatusFilter`) + openapi.yaml §repositories/build-runs 정의 + frontend `repositoryService.getRepositoryBuildRuns` + `DeveloperView` 위젯 (`buildRuns.length === 0` empty state + "View Logs" modal) + e2e `repository-dashboard.spec.ts` 의 inline…
| **N-9** | **Repository build-runs (P1-7) 구현 + 잔여 4건 sub-issue 모두 완료** — 2026-06-01 통합 테스트 ISSUE-04. **2026-06-11 정합** (sprint `chore/work_260611-d-n9-status-align`): 본 carve 의 endpoint `GET /api/v0-1/repositories/:repository_id/build-runs` + `platformStoreOrUnavailable` 가드 + `ListRepositoryBuildRuns` (postgres.go `repository_ops.go`, `started_at DESC, id DESC` 정렬) + UT 3건 (`TestRepositoryBuildRuns_ReturnsCIRunsData` / `_NoCIRuns` / `_StatusFilter`) + IT 1건 (`TestIntegration_ListRepositoryBuildRuns_StatusFilter`) + openapi.yaml §repositories/build-runs 정의 + frontend `repositoryService.getRepositoryBuildRuns` + `DeveloperView` 위젯 (`buildRuns.length === 0` empty state + "View Logs" modal) + e2e `repository-dashboard.spec.ts` 의 inline… **잔여 4건 sub-issue 분리** (… **2026-06-17 codex P2 review 정공법 fix** (동일 branch follow-up commit, sprint `chore/260617-n9-residual-build-runs-polish`): (1) backend `store.ListRepositoryBuildRuns` 의 `(runs, total, err)` 3-tuple total 을 frontend 가 합성하지 않고 `service.getRepositoryBuildRunsWithMeta` (legacy `getRepositoryBuildRuns` caller 무손상 보존) 로 expose, hook 이 `meta.total` 사용 (fallback = `items.length >= pageSize`) — `hasMore` 정확. (2) hook 의 cleanup boolean + ref-reset race → `AbortController` per-request token + `isCurrent` 가드 (ref 일치 + `signal.aborted`) + service 옵션 `signal` → `apiClient` plumb. `apiClient` 4번째 인자 `options.signal?: AbortSignal` 추가 (backward-compatible) + refresh 후 2nd fetch 동일 signal 유지. 검증: hook 9/9 + section 7/7 + service 13/13 + apiClient 17/17 + RepositoryDashboardView 8/8 PASS, `tsc --noEmit` PR 영향 7 file 모두 0. Tier: 사외. `service.getRepositoryBuildRuns` 자체는 정공법 caller 무손상 보존 — `app/(dashboard)/repositories/page.tsx` + `view/DeveloperView.tsx` + `view/RepositoryDashboardView.test.tsx` 3 caller 변경 0.) |
...
| **N-12** | **dev-requests voc 도메인 + 9 field + in-app notification (ADR-0028)** — v0.1.0 출시 직전 sprint `feat/work_260612-a-dev-requests-voc-domain` (PR #514, ADR-0028 §3 결정). 외부 시스템 의 의뢰가 project 결정 전 도착하는 경우의 staging 공간 + 9 field (title/details/requester/req_department/assignee/dev_department/request_date/dev_schedule/external_ref) 매칭 + in-app notification (`/api/v0-1/me/notifications`). **scope**: 2 신규 table (dev_request_vocs + user_notifications) + dev_requests 4 column 확장 + 5 신규 API (voc 등록 / routing / 조회 + notification list / mark read). `(source_system, external_ref)` UNIQUE for idempotency. **status**: ✅ resolved (backend code + ADR 정합, 2026-06-12). | Backend | OpenCode (v0.1.0 출시 직전 sprint) |
| **N-13** | **project.inbound_source 자동 routing (ADR-0028 §6 carve a, post-MVP)** — v0.1.0 출시 후 v0.1.1 milestone 진입 시점 sprint. ADR-0028 §3 의 voc 흐름 (외부 → voc 등록 → system_admin routing → dev-request 등록) 의 post-MVP 자동화. `applications.inbound_source_type` (gitea/jira/other) + `inbound_source_config` (JSONB) 컬럼 + sync 자동 routing 로직. **scope**: 1 migration (000007) + 1 schema 확장 + 1 API (PATCH /platforms inbound_source) + 1 worker (없음, synchronous routing) + 1 E2E + ADR-0028 §6 amendment. **2026-06-11 정공법** (sprint `feat/work_260611-a-n13-inbound-source-housekeeping`, docs only): ID slot 9 row 발급 + `conventions.md §1` RM 표기 정책 확장 (도메인 prefix 관행 `RM-{domain}-{nn}` 명문화) + `docs/traceability/report.md` §2.1/§2.1.5/§2.2/§2.3/§2.4/§2.5/§2.6 + §3 dev-request row + §4 ADR 인덱스 ADR-0028 row + §6 변경 이력 + 헤더 메타 정합. **status `✅ resolved (implemented, 2026-06-12, sprint `feat/work_260612-7-v0-1-1-inbound-source-impl`)`** — N-13 follow-up 3 branch 종합 (A: e2e seed fix PR #574, B: signout timeout fix PR #575, C: 구현 follow-up = 본 sprint). routing/auto_route.go + voc_handler 통합 + IT 3 case + openapi.yaml 정합 + e2e TC-INBOUND-SRC-01 + ADR-0028 §6 (a) amendment + 9 ID row 정합 (REQ-FR-113 / UC-DEV-REQ-15 / ARCH-23 / API-103 / RM-DEV-REQ-15 / IMPL-inbound-source-01 / IMPL-platform-patch-02 / UT-inbound-source-01 / TC-INBOUND-SRC-01). 정공법 + ID slot + 의존: [`docs/planning/2026-06-12-inbound-source-routing-sprint-plan.md`](2026-06-12-inbound-source-routing-sprint-plan.md) — 옵션 A 권장 (applications.inbound_source 컬럼 + sync 자동 routing). 검증 보고서 link = N/A (N-10 / N-13 은 code-level 정합 검증 = 구현 sprint 에서 별도).<br>**2026-06-11 PR #548 close 정공법** (sprint `feat/work_260611-a-n13-inbound-source-impl`): CI 11/12 PASS, E2E Internal 1 fail (Test 1: e2e seed 중복 strict mode violation `getByText('e2e-repo-a')` 2 elements + Test 2: Sign-out timeout N-8 race 유사), codex review = COMMENTED (blocker 아님), 자동 재실행 안 됨 (main 의 PR #550 spec timing fix 미적용, run 시각 < PR #550 머지). **follow-up 결정** (2026-06-12, sprint `fix/work_260612-1-n13-housekeeping-followup`): (1) **Test 1 e2e seed 중복** → spec/e2e seed 정합 fix 별도 sprint (mock data 유일성 보장); (2) **Test 2 Sign-out timeout** → main rebase + 자동 재실행 검증 별도 (PR #550 spec timing fix 가 해결 가능성 ↑); (3) **구현 follow-up = v0.1.1 milestone 진입 시점 별도 sprint** (rebase main + PR #550 fix 적용 + e2e seed 중복 spec fix + 자동 재실행). branch 이름 `feat/work_260611-a-n13-inbound-source-impl` 는 close (PR #548), v0.1.1 진입 시점의 신규 구현 sprint 는 새 branch 이름 (예: `feat/work_YYMMDD-v0-1-1-inbound-source-impl`) 별도 결정.
...
| **X-6** | Keycloak group staging-prod 적용 (P1-3, issue #214) | 사내 | 🟡 **in_progress (docs only, 2026-06-17)** — [`docs/setup/keycloak_operations.md` §4.3 + §4.4](../setup/keycloak_operations.md) 의 group 4종 + composite role 1:1 매핑 SOP + `scripts/verify-keycloak-groups.sh` 자동 검증 (4 항목 + exit code + FAIL 케이스 + §6.2/§6.3 stage/prod 적용 시점) main 정합. issue #214 close 정공법 (본 turn docs only). 잔여 = 사내 Keycloak admin console 1회 작업 (group 4 + composite role assign, 사용자 결정). |
| **X-7** | ADR-0016 §6 alert 임계 확정 (P2-2) | Codex |
| **X-8** | Keycloak SPI realm events push 전환 (P2-6/P3-5) | BE/사내 | 🟡 **in_progress (staging hand-off + CI e2e smoke 정공법, 2026-06-17)** — [`infra/idp/keycloak-event-listener-spi/`](../../infra/idp/keycloak-event-listener-spi/) 의 Java SPI (Keycloak 26.0 + Java 21) main 정합. `docker-compose.colima.yml` + `docker-compose.deploy.yml` + `docker-compose.test.yml` keycloak service 에 SPI build (Dockerfile.keycloak multi-stage) + SPI JAR volume mount + DEVHUB_BACKEND_SPI_WEBHOOK_URL + DEVHUB_KEYCLOAK_SPI_WEBHOOK_SECRET env 추가 (PR #641 + PR #642). `keycloak-realm.{dev,prod,ci}.json` eventsListeners `["jboss-logging", "devhub-event-listener"]` 정합. [`docs/setup/keycloak_event_listener_spi.md`](../setup/keycloak_event_listener_spi.md) + [`docs/setup/keycloak_event_listener_spi_staging.md`](../setup/keycloak_event_listener_spi_staging.md) 정공법 SOP. [`scripts/verify-keycloak-spi.sh`](../../scripts/verify-keycloak-spi.sh) + [`scripts/build-keycloak-spi.sh`](../../scripts/build-keycloak-spi.sh) 자동화. [`frontend/tests/e2e/keycloak-event-listener-spi.spec.ts`](../../frontend/tests/e2e/keycloak-event-listener-spi.spec.ts) **NEW** CI e2e smoke (TC-KEYCLOAK-SPI-01 push smoke + TC-KEYCLOAK-SPI-02 env 정합). backend 변경 0 (webhook handler + polling cron + dedup 모두 main 정합). residual polling cron = at-least-once deliver 보장 (push fail fallback). 잔여 = 사내 빌드 + staging/prod 적용 (사용자 결정) + 사내 staging/prod 적용 후 verify-keycloak-spi.sh 1회 실행. |
...
### 4.1 M-v0.1.0 — 1차 릴리즈 (target: 2026-06-15)

**완료 정의**: §1.3 의 8개 DoD 항목 모두 PASS.

**현재 status (2026-06-12)**: v0.1.0-alpha (2026-06-11) released — 8 DoD 모두 close (7 ✅ + 1 ✅ skipped=N-6). v0.1.1-alpha 개발 중 — 잔여 5 (T-d-72-5/6 + D-73/74 + X-1~8) 의 v0.1.1-alpha 격하 정공법 (commit `3e2701b3`). **M-v0.1.0 GA target = 2026-06-15**, 단 v0.1.1-alpha release 후 N-6 staging 1주 운영 사용자 결정 시점. lint phase 2 chunk 2 0/0/0 달성 (PR #569 merge 완료, 2026-06-12).

**구성 sprint** (예상):
- sprint -f: P0-1 sub-carve B (`/api/v0-1/accounts/*` 폐기 + lazy auto-create + frontend cleanup) — Claude+Gemini 분담
...
- sprint -k: v0.1.0 e2e 종합 검증 + 운영 환경 1주 monitoring — 전 워커. **N-11 잔여 DoD 흡수 완료 (2026-06-09, N-8 hotfix 4차 머지 + PR #505 e2e shard 1..3 PASS 재확인)**, N-6 staging 1주 운영 검증만 잔여.

### 4.2 M-v0.1.1 — 안정성 + 운영 강화 (target: 2026-07-31)
...
| **N-13** (project.inbound_source 자동 routing) | **✅ resolved (implemented, 2026-06-12, sprint `feat/work_260612-7-v0-1-1-inbound-source-impl`)** | Claude |
...
> **2026-06-09 결정 — 본 §5 의 워커 분담 표는 전면 취소되었다.** 사용자 (Owner) 의 Claude/Codex 자유 이용 불가 결정으로 §5.1 영역별 분담 + §5.2 v0.1.0 sprint 별 분담 + §5.3 인계 SOP 모두 무효. 모든 신규 작업은 **어느 에이전트로든 자유롭게** 진행 가능. 자세한 사항 + 유지되는 정책은 [worker_division.md §0](../governance/worker_division.md) 참조.
...
### 5.2 v0.1.0 sprint 별 워커 분담 권장 — **취소 (2026-06-09)**
...
`gh project create --title "DevHub v0.1.0 릴리즈" --owner ykylee` — 단일 project. 다음 column:
- **Backlog** — 미진입 issue
...
| --- | --- | --- |
| `v0.1.0 Release` | 2026-06-15 | P0-1, P0-2, P0-3, P1-1, P1-2, P2-1, + v0.1.0 종합 검증 |
| `v0.1.1 Stability` | 2026-07-31 | P1-3, P1-5, P2-2, P2-4..P2-6, P2-8..P2-12, P3-1 (P1-4 off-boarding cron + P2-7 HRDB ETL + P3-12 Sign Up 은 cancelled — 외부 Keycloak 시나리오 + ADR-0021 self-service 로 무효화. **P2-3 ✅ resolved** PR #137 + adr0017-token-mutation. **P2-8 ✅ resolved** PR #278 Carve A backend.) |
...
2. **sprint -h** (or 동시): P0-1 sub-carve B (`/api/v0-1/accounts/*` 폐기) + **P0-4 CI Run 생성 API** — Claude backend 우선 처리. P0-4는 CI 기능의 v0.1.0 출시 차단 요건
...
5. **sprint -k**: v0.1.0 종합 검증 + staging 1주 monitoring — 전 워커
...
| 일자 | 변경 | sprint |
| --- | --- | --- |
| 2026-05-27 | **코드베이스 스냅샷 정합** — Onboarding IMPL carve A/B/C/D 전부 ✅ resolved (§2.3 + §3.3 P2-8..12 + §4.2, PR #278/#288/#289/#290/#291, lazy_auto_create 폐기 issue #284 closed) + v0.1.1 영역 작업 v0.1.0 전 선행 명시(Gitea SCM sync·SCM↔시스템 양방향·auth_mode full·repository draft/publish·admin catalog) + §3.5 신규 도출 백로그(N-1..6 / X-1..8 / E-1..8, 코드베이스 스냅샷 §06 연계) 추가. 헤더 날짜 2026-05-21→2026-05-27. | `claude/work_260527-codebase-review-roadmap-refresh` |
| 2026-05-20 | 1차 작성 — v0.1.0 scope 정의 (3 domain) + 잔여 carve 통합 인벤토리 (P0/P1/P2/P3, 30+ item) + 마일스톤 재정의 (M-v0.1.0 / M-v0.1.1 / M-v2) + 워커 분업 매트릭스 (Claude=backend / Codex=infra+CI / Gemini=frontend+UX) + GitHub project + milestone plan + UI Playwright screenshot mode | `claude/work_260520-f-roadmap` |
| 2026-05-20 | **P3-12 Sign Up 영구 취소** — 사용자 결정. DevHub 가 Keycloak admin 권한이 없는 외부 IdP 운영 시나리오 (ADR-0020 결정 A 정합). user 생성은 IdP 팀 admin console 또는 HRDB ETL push 책임. §1.2 제외 기능 표 분류 'v0.1.1 carve' → 'permanently cancelled' + §3.4 P3-12 strikethrough + §5.2 워커 분담 표 strikethrough + §4.2 v0.1.1 milestone 본문 정정 + issue #235 closed | `claude/work_260520-i-cancel-signup` |
| 2026-06-01 | **통합 테스트 결과 반영** — §3.1 P0-4 (CI Run API) 신규 P0 carve 추가 + §3.2 P1-6 (Sign-out) P1-7 (Build-runs) 신규 P1 carve 추가 + §3.5 N-7~N-10 신규 도출 백로그 반영 + §4.1 sprint 구성 P0-4 포함 재조정 + §4.2 v0.1.1 milestone 신규 P1 항목 추가 + §7 sprint 진입 순서 갱신. 출처: `deepseek/test-scenarios-20260601` 브랜치 통합 테스트 | `deepseek/test-scenarios-20260601` |
...
| 2026-06-10 | **v0.1.0 출시 직전 housekeeping — §3.5 N-8/N-11 row close + §4.1 sprint -k 마킹 + §3.5 N-10 follow-up 6 TC 명시.** (1) §3.5 **N-8 race row status** `✅ resolved (issue #501 closed)` 명시. (2) §3.5 **N-11 row** "잔여 DoD" → "✅ 잔여 DoD 해소" + **status `✅ resolved`** (PR #502 + #503 + PR #505 e2e shard 1..3 PASS 재확인 정합). (3) §4.1 sprint -k 의 N-11 잔여 DoD 완료 마킹 (N-6 staging 1주 운영만 잔여). (4) §3.5 N-10 row 의 follow-up 6 TC 명시 (`TC-RBAC-ROW-READ-01/02` + `TC-RBAC-LOGOUT-01/02` + `TC-RBAC-ROLE-DRIFT-01` + `TC-RBAC-CODE-01` + `TC-RBAC-TRACE-01`) + follow-up sprint `maintenance/work_260610-c-N10-rbac-e2e-tcs` 진입 예정. (5) [docs/traceability/report.md §4](../traceability/report.md) ADR-0025 (envelope encryption, 2026-05-27 PR #350) + ADR-0026 (Keycloak role excluded decision, 2026-06-02 `deepseek/work_260602`) row 보강 — swagger UI 1차 bootstrap sprint 가 note 만 남겼던 ID slot 누락 해소. (6) §9 본 row 추가. 신규 ID 발급 없음 (governance housekeeping). main HEAD `ad8d481` (PR #505). | `maintenance/work_260610-b-v0-1-pre-release-housekeeping` |
| 2026-06-12 | **v0.1.0 출시 직전 finalizing — N-12 (voc + notification) + housekeeping + N-10 잔여 backend IT** (sprint `maintenance/work_260612-b-v0-1-finalizing`). (1) §3.5 N-12 row 신규 + status `✅ resolved` 마킹 — dev-requests voc 도메인 + 9 field + in-app notification (ADR-0028) 정합. (2) traceability/report.md §2.4 에 IMPL-voc-01 + IMPL-notification-01 cross-cutting row 추가 + §6 변경 이력 본 sprint row 1건. (3) [voc list API (ADR-0028 §6 carve (d))] — `GET /api/v0-1/vocs?status=received&assignee=&limit=&offset=` 신규 (system_admin 도구, N-6 staging 운영 SOP 정합). (4) [follow-up N-10 backend IT 3 TC] — `TC-RBAC-LOGOUT-01` + `TC-RBAC-ROLE-DRIFT-01` + `TC-RBAC-LEGACY-01` backend integration test. §9 본 row 추가. 신규 ID 발급 없음 (voc + notification 정합 + housekeeping). main HEAD `ba7823f` (PR #514). | `maintenance/work_260612-b-v0-1-finalizing` |
| 2026-06-12 | **v0.1.0 출시 직전 option D 검토 — project.inbound_source 자동 routing (ADR-0028 §6 carve a) sprint plan** (sprint `maintenance/work_260612-c-inbound-source-plan`). (1) §3.5 N-13 row 신규 + status `⏳ planned` (v0.1.1 milestone 진입 시점 구현). (2) [`docs/planning/2026-06-12-inbound-source-routing-sprint-plan.md`](2026-06-12-inbound-source-routing-sprint-plan.md) 신규 — 옵션 A/B/C 비교 + 옵션 A 권장 결정 (applications.inbound_source 컬럼 + sync 자동 routing) + ID slot 8건 (REQ-FR-113 + UC-DEV-REQ-15 + ARCH-23 + API-103 + RM-DEV-REQ-15 + IMPL-inbound-source-01 + IMPL-platform-patch-02 + UT-inbound-source-01 + TC-INBOUND-SRC-01) + 정공법 10 step (~9시간, 1 sprint). (3) §9 본 row 추가. 신규 ID 미발급 (v0.1.1 milestone 진입 시점 발급, 본 sprint = post-MVP 검토). main HEAD `f7d2705` (PR #515). | `maintenance/work_260612-c-inbound-source-plan` |
| 2026-06-11 | **N-13 P1 follow-up 정공법 — ID slot 9 row 발급 + conventions.md §1 RM 표기 확장 (docs only, post-MVP)** (sprint `feat/work_260611-a-n13-inbound-source-housekeeping`). (1) `conventions.md §1` 의 RM 표기 정책 확장 — `RM-M{n}-{nn}` legacy milestone + `RM-{domain}-{nn}` 도메인 prefix (예: `RM-ONBOARD-01..04` / `RM-APPDASH-01` / `RM-DEV-REQ-15`) 두 관행 혼용 명문화. (2) `docs/traceability/report.md` 에 N-13 ID slot 9 row 발급 — `REQ-FR-113` / `UC-DEV-REQ-15` / `ARCH-23` / `API-103` / `RM-DEV-REQ-15` / `IMPL-inbound-source-01` / `IMPL-platform-patch-02` / `UT-inbound-source-01` / `TC-INBOUND-SRC-01` (sprint `feat/work_260611-a-n13-inbound-source-impl` v0.1.1 진입 시점 코드 변경). (3) `docs/adr/0028-dev-requests-voc-external-ref.md` §6 (a) 의 ID slot 발급 사실 + 도메인 prefix 표기 관행 정합 + §7 변경 이력 1 row. (4) 본 §3.5 N-13 row 의 status `⏳ planned` 보강 + housekeeping 정공법 완료 마킹. (5) §9 본 row 추가. **신규 ID 발급 9 row, 코드 변경 0줄**. main HEAD `f37305d7` (PR #545 머지 후). | `feat/work_260611-a-n13-inbound-source-housekeeping` |
| 2026-06-11 | **N-9 (P1-7 Repository build-runs) 기본 구현 완료 정합 + 잔여 4건 sub-issue 분리 (docs only)** (sprint `chore/work_260611-d-n9-status-align`). (1) §3.5 **N-9 row status `✅ resolved (기본 구현, 2026-06-11)`** — 본 carve 의 endpoint `GET /api/v0-1/repositories/:repository_id/build-runs` (router.go:509) + `platformStoreOrUnavailable` 가드 + `ListRepositoryBuildRuns` (postgres.go `repository_ops.go`) + UT 3건 + IT 1건 + openapi.yaml §repositories/build-runs 정의 + frontend `repositoryService.getRepositoryBuildRuns` + `DeveloperView` 위젯 + e2e `repository-dashboard.spec.ts` 의 inline build-runs mock 검증 모두 main 반영. (2) §3.2 **P1-7 row 비고** 갱신 — `신규 P1` → `기본 구현 완료` + endpoint path 차이 (`/repos/{id}` → `/repositories/:repository_id`, main 컨벤션) 명시 + 잔여 4건 sub-issue 분리. (3) **잔여 4건 sub-issue 분리** (v0.1.1 milestone 진입 시점, 별도 carve): **(a)** RBAC 403/404 가드 (backend); **(b)** Histogram metric `devhub_repository_build_runs_query_duration_seconds{status_filter}` (backend); **(c)** TanStack Query `useRepositoryBuildRuns` hook + status filter dropdown (queued/running/success/failed/cancelled/skipped/unknown) + skeleton + 무한 스크롤 cursor pagination (frontend); **(d)** dashboard widget "Recent repository activity" 통합 (manager/admin dashboard, frontend) + 독립 e2e spec `tests/e2e/repository-build-runs.spec.ts` (TC-E2E-BUILD-RUNS-01). (4) issue [#487](https://github.com/ykylee/Devhub_example/issues/487) (정식 ID = N-9) — close + 잔여 4건 sub-issue 별도 발급 (PR 머지 후). 본 sprint = docs only 정합 — 신규 ID 발급 없음. | `chore/work_260611-d-n9-status-align` |
...
| 2026-06-12 | **N-13 PR #548 close follow-up 결정 (docs only)** (sprint `fix/work_260612-1-n13-housekeeping-followup`). PR #548 (`feat/work_260611-a-n13-inbound-source-impl`, 2026-06-11 05:40 UTC CLOSED) 의 E2E Internal 1 fail 2건 (Test 1: e2e seed 중복 strict mode violation `getByText('e2e-repo-a')` 2 elements + Test 2: Sign-out timeout N-8 race 유사) + 자동 재실행 미적용 (PR #550 spec timing fix 미반영, run 시각 < PR #550 머지) 정공법. §3.5 **N-13 row** status `⏳ planned` 유지 + 본문 보강 (PR #548 close 결과 + follow-up 3 branch 결정). follow-up 결정: (1) **Test 1 e2e seed 중복** → spec/e2e seed 정합 fix 별도 sprint (mock data 유일성 보장); (2) **Test 2 Sign-out timeout** → main rebase + 자동 재실행 검증 별도 (PR #550 spec timing fix 가 해결 가능성 ↑); (3) **구현 follow-up = v0.1.1 milestone 진입 시점 별도 sprint** (rebase main + PR #550 fix + e2e seed 정합 fix + 자동 재실행 종합). branch `feat/work_260611-a-n13-inbound-source-impl` close (PR #548) — v0.1.1 진입 시점의 신규 구현 sprint 는 새 branch 이름 (예: `feat/work_YYMMDD-v0-1-1-inbound-source-impl`) 별도 결정. [ADR-0028 §6 (a)](../adr/0028-dev-requests-voc-external-ref.md) + [sprint plan §3.3/§5/§6](../planning/2026-06-12-inbound-source-routing-sprint-plan.md) + [traceability/report.md §6](../traceability/report.md) + 메모리 4 file 동기화. 헤더 메타의 최종 수정일/직전 결정 근거/결정 근거 sprint 갱신. 신규 ID 발급 0건 (housekeeping follow-up). | `fix/work_260612-1-n13-housekeeping-followup` |
| 2026-06-12 | **N-13 follow-up 3 branch 종합 v0.1.1 sprint -a 구현 정합** (sprint `feat/work_260612-7-v0-1-1-inbound-source-impl`). §3.5 **N-13 row** status `⏳ planned` → `✅ resolved (implemented, 2026-06-12)` + 본문 보강 (구현 정공법 + 9 ID row 정합). §4.2 M-v0.1.1 milestone 표에 N-13 row 신규 + `✅ resolved` 마킹. (1) routing/auto_route.go (NEW, 3 case pattern matcher + AutoRouter interface) + voc_handler.go 통합 (AutoRouter.Route() + RouteVoc() + auto_routed envelope 응답) + IT 3 case (GiteaOK / NoMatch / RouteErrorDegradation). (2) openapi.yaml 정합 (PATCH /platforms inbound_source_type + inbound_source_config + POST /dev-requests/{id} auto_routed 응답 + DevRequestVoc schema). (3) e2e TC-INBOUND-SRC-01 (NEW, seed platform 사용, POST platform 단계 제거 정공법 — PR #576 의 commit `90256ec` 정공법 정합). (4) ADR-0028 §6 (a) amendment + traceability report.md 9 ID row status `planned` → `implemented` 정합. **N-13 follow-up 결정 (PR #573) 의 3 branch 종합**: A (PR #574 e2e seed fix) + B (PR #575 signout timeout fix) + C (본 sprint 구현 follow-up). backend foundation (PR A-1: migration 000007 + domain.Platform 2 field + UpdatePlatformInboundSource + view.UpdatePlatform + 4 UT) 은 main 에 byte-identical 포함 (PR #549 T-d-72-2 wiki mirror). 본 sprint = PR A-2 만. **신규 ID 발급 0건** (PR #547 의 ID slot 정합의 구현 정합). Tier: **공용** (코드 + ADR + openapi + e2e 모두 사내 한정 정보 미포함). CI 11/12 PASS (e2e shard 1/2/3 + backend + frontend + lint 모두 PASS; E2E Internal = OBSOLETE, PR #578 e2e-internal 폐기 결정 정합). 헤더 메타의 최종 수정일/직전 결정 근거/결정 근거 sprint 갱신. | `feat/work_260612-7-v0-1-1-inbound-source-impl` |

| 2026-06-17 | **N-9 잔여 build-runs polish frontend 2건 + PR #633 codex P2 review 정공법 fix** (sprint `chore/260617-n9-residual-build-runs-polish`, 동일 branch 2 commit). (1) §3.5 N-9 row 의 "잔여 4건 sub-issue" 3, 4번 (custom hook + dashboard widget) 정공법 — `useRepositoryBuildRuns` (TanStack Query 도입 안함, useState+useEffect) + `RepositoryBuildRunsSection` widget + ManagerView sibling 통합. (2) PR #633 의 codex P2 review 코멘트 2건 정공법 fix: (P2-1) `service.getRepositoryBuildRunsWithMeta` 신규 (legacy `getRepositoryBuildRuns` caller 무손상 보존) + hook 이 `meta.total` 사용 (fallback = `items.length >= pageSize`); (P2-2) `AbortController` per-request token + `isCurrent` 가드 + service 옵션 `signal` → `apiClient` plumb (4번째 인자 `options.signal?: AbortSignal` 추가, backward-compatible) + refresh 후 2nd fetch 동일 signal 유지. 검증: hook 9/9 + section 7/7 + service 13/13 + apiClient 17/17 + RepositoryDashboardView 8/8 PASS, `tsc --noEmit` PR 영향 7 file 모두 0. §3.5 N-9 row 본문 보강 (P2 fix mention append). §9 본 row + §0 본 row 추가. 신규 ID 발급 0 (frontend hook/service/shared contract 변경, REQ/ARCH/API/IMPL/UT/TC ID slot 무관). Tier: 사외. main HEAD `c2c14ddeb` (PR #633) → follow-up 동일 branch. | `chore/260617-n9-residual-build-runs-polish` |

| 2026-06-17 | **X-6 (Keycloak group staging-prod 적용) 정공법 = docs only** (sprint `feat/260617-x6-keycloak-groups-docs`, docs only, 사외 한정 정보 0). X-6 (issue #214) 의 backend/script 변경 0 — 이미 main 정합. **fix 정공법**: (1) `docs/setup/keycloak_operations.md` §4.3 (group 4종 + composite realm role 1:1 매핑 SOP, [keycloak_groups_mapping.md](../domain/rbac-permissions/keycloak_groups_mapping.md) 옵션 B 채택) + §4.4 ([`scripts/verify-keycloak-groups.sh`](../../scripts/verify-keycloak-groups.sh) 자동 검증 4 항목 + exit code + FAIL 케이스) main 정합. (2) 본 §3.5 NEXT block 의 X-6 row status `⏳ planned` → `🟡 in_progress (docs only, 2026-06-17)` + Keycloak admin SOP + verify-keycloak-groups.sh mention + 잔여 = 사내 Keycloak admin console 1회 작업 (사용자 결정). (3) issue #214 close 정공법 (SOP + 자동 검증 main 정합 + 사용자 admin 작업 잔여). 검증: `tsc --noEmit` 본 PR 영향 0 + backend `go build ./...` silent (변경 0). **Tier**: **공용** (docs only, 사내 한정 정보 0). 잔여 = 사내 Keycloak admin console 1회 작업 (group 4 + composite role assign, 사용자 결정) + 사내 staging/prod 적용 후 verify-keycloak-groups.sh 1회 실행 (사용자 결정). branch: `feat/260617-x6-keycloak-groups-docs`. PR (예정, PR #640+). |
| 2026-06-17 | **사용자 보고 3건 (이슈 1+2+3) + codex P1/P2 review 정공법 fix 종합 sprint** (sprint 5개: `fix/260617-dev-request-intake-ip-default` + `fix/260617-kpi-test-card-graceful-fallback` + `feat/260617-kpi-test-detail-page` + `fix/260617-codex-review-feedback` + `fix/260617-final-merge` 마무리). **이슈 1** (테스트 환경 dev-request intake 401 `auth_intake_ip_denied`): `IssueIntakeTokenModal.tsx` default `["0.0.0.0/0", "::/0"]` (dev 환경 한정 useEffect mount 시 자동 적용, prod 환경 `[""]` deny-by-default). **이슈 2** (KPI/test 카드 5xx): backend `platformStoreOrUnavailable` 의 response body 에 `code: "platform_store_unavailable"` 추가 + frontend 6 component error state 공유 `KpiTestErrorState` 교체 + `toUserErrorMessage` 의 503 + code 매핑. **이슈 3** (drill-down detail page): 6 신규 page (`platforms/projects/repositories × kpi/test-results`) + 공유 wrapper `KpiTestDetailPage` + 6 component header `자세히 보기` link (usePathname loop 회피). **Codex P1**: `IssueIntakeTokenModal` prod 환경 default `[""]` + dev 환경 useEffect mount 시 `0.0.0.0/0` 자동 (bearer token leak 시 모든 IP 사용 가능 risk 해결). **Codex P2**: (a) `useRepositoryBuildRuns.loadMore` stale branch `setLoadingMore(false)` (button 영구 disabled 방지). (b) 3 TestsSection drill-down link `href` + `isOnDetailPage` 의 `usePathname().endsWith` path `/test-results` 정합 (404 방지). **정리**: 4 PR (#634/#636/#637/#638) 의 변경이 PR #635 의 squash commit 의 본문에 일부만 들어감 → close + single `fix/260617-final-merge` PR (commit `7189d277`) 로 main 머지. **검증**: 16/16 test file 62/62 PASS (이전 turn 44 → 62, +18 case) + `tsc --noEmit` 본 PR 영향 0 + `go build ./...` silent. §3.5 N-9 row 의 본 turn 정합 mention 부재 → 별도 갱신 불필요 (본 sprint 은 N-9 외부 user-reported issue). 신규 ID 발급 0 (frontend hook/service/shared contract 변경, REQ/ARCH/API/IMPL/UT/TC ID slot 무관). Tier: 사외/공용 (frontend only + backend test + docs, 사내 한정 정보 0). | `fix/260617-final-merge` |

| 2026-06-17 | **X-8 (Keycloak SPI realm events push 전환) 정공법 = compose + docs only** (sprint `feat/260617-x8-keycloak-event-listener-spi-sop`, PR #641+). X-8 (P2-6 + P3-5) 의 backend 코드 변경 0 — `DevHubEventListenerProvider.java` async sendAsync POST + `DevHubEventListenerProviderFactory.java` env-var wire + `keycloak_events_webhook.go` webhook handler + `keycloak_event_puller.go` polling cron + `event_cursors.go` dedup 모두 main 정합. **fix 정공법** (compose mount + SOP + verify script + 문서): (1) `docker-compose.colima.yml` + `docker-compose.deploy.yml` keycloak service 에 SPI JAR volume mount `./infra/idp/keycloak-event-listener-spi/target/devhub-keycloak-event-listener.jar:/opt/keycloak/providers/devhub-keycloak-event-listener.jar:ro` + env `DEVHUB_BACKEND_SPI_WEBHOOK_URL` + `DEVHUB_KEYCLOAK_SPI_WEBHOOK_SECRET` 추가. (2) [`docs/setup/keycloak_event_listener_spi.md`](../setup/keycloak_event_listener_spi.md) 정공법 SOP 신규 (architecture + build + compose mount + realm config + SPI 동작 flow + verify + 운영 SOP + 잔여 carve). (3) [`scripts/verify-keycloak-spi.sh`](../../scripts/verify-keycloak-spi.sh) 자동 검증 4 항목 (SPI JAR classpath / Realm eventsListeners / Env var / Webhook push smoke latency < 1s) 작성. (4) release_v0-1_roadmap.md §3.5 NEXT block 의 X-8 row status `⏳ planned` → `🟡 in_progress (compose + docs only, 2026-06-17)` + Keycloak SPI architecture + mount + verify script mention. 검증: `tsc --noEmit` 본 PR 영향 0 + backend `go build ./...` silent (변경 0) + compose YAML lint 정공법. **Tier**: **공용** (Java source + compose + docs 만, 사내 한정 정보 0). 잔여 = 사내 빌드 (Java 21 + Maven 3.13+ 환경, 사용자 결정) + 사내 staging/prod 적용 후 `verify-keycloak-spi.sh` 1회 실행 (4 항목 PASS 확인). branch: `feat/260617-x8-keycloak-event-listener-spi-sop`. PR (예정, PR #641+). |


| 2026-06-17 | **X-8 staging hand-off + CI e2e smoke 정공법** (sprint `feat/260617-x8-staging-handoff-e2e-smoke`, PR #642+). X-8 (P2-6 + P3-5) 의 backend 변경 0 + PR #641 의 정공법 (compose + docs) + 본 turn 의 staging hand-off + CI e2e smoke 통합. **fix 정공법** (staging hand-off + CI e2e smoke + SOP): (1) [`docs/setup/keycloak_event_listener_spi_staging.md`](../setup/keycloak_event_listener_spi_staging.md) **NEW** (9KB) — staging/prod 사내 SCM hand-off 절차 7 step (사전 준비 → build → compose mount → Keycloak 재시작 → verify 자동 실행 → 결과 보고 → 일상 운영). (2) [`scripts/build-keycloak-spi.sh`](../../scripts/build-keycloak-spi.sh) **NEW** — JAR build (`mvn clean package` 또는 docker buildx) + push (harbor.internal 등 사내 registry). 3 단계 (prerequisite check / maven build / push) + 사내 maven mirror 지원 + exit 0/1. (3) `docker-compose.test.yml` keycloak service — `image: quay.io/keycloak/keycloak:26.0` → `build: context: ./infra/idp, dockerfile: Dockerfile.keycloak` 변경 (CI 환경의 SPI e2e smoke 정공법) + SPI JAR volume mount + env 추가 + `keycloak-realm.ci.json` eventsListeners `["jboss-logging", "devhub-event-listener"]` 추가 (의도적 미적용 → CI e2e smoke 적용). (4) [`frontend/tests/e2e/keycloak-event-listener-spi.spec.ts`](../../frontend/tests/e2e/keycloak-event-listener-spi.spec.ts) **NEW** — Playwright e2e smoke 2 case (TC-KEYCLOAK-SPI-01 push smoke latency < 1s + TC-KEYCLOAK-SPI-02 env 정합). 검증: `tsc --noEmit` 본 PR 영향 0 + backend `go build ./...` silent (변경 0) + compose YAML lint 정공법 (colima + deploy + test 셋 다 OK) + `frontend/tests/e2e/keycloak-event-listener-spi.spec.ts` Playwright pattern 정합. **Tier**: **공용** (Java source + compose + docs + e2e spec + script 만 — 사내 한정 정보 0). 잔여 = 사내 빌드 (Java 21 + Maven 3.13+ 환경) + staging/prod 적용 + `verify-keycloak-spi.sh` 1회 실행 (사용자 결정). branch: `feat/260617-x8-staging-handoff-e2e-smoke`. PR (예정, PR #642+). |
