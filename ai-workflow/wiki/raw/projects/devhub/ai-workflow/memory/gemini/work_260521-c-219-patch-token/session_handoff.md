# Session Handoff — gemini/work_260521-c-219-patch-token (2026-05-21 sprint [P2-3] DREQ PATCH Token 구현 완료)

- 문서 목적: gemini/work_260521-c-219-patch-token 브랜치 기준 세션 상태와 다음 작업 진입점을 인계한다.
- 범위: 이슈 #219 [P2-3] DREQ Intake Token PATCH expires_at & allowed_ips API 및 어드민 UI 편집 모달 구현.
- 대상 독자: 후속 에이전트, 프로젝트 리드, 다음 세션 진입자.
- 상태: M1/M2/M3 done. **이슈 #219 (DREQ Intake Token PATCH expires_at & allowed_ips) 완결**. **옵션 B 정책(만료 토큰의 연장/재활성화 허용)**을 채택하여 백엔드 DB 스토어 및 httpapi handler에 비즈니스 검증용 Go 테스트 보강 완료. 프론트엔드 Playwright E2E 테스트(`dev-requests.spec.ts`)에서 allowed_ips 및 expires_at을 dynamic PATCH하는 시나리오 고도화 및 Flaky 없이 검증 완료.
- 최종 수정일: 2026-05-21 (sprint gemini/work_260521-c-219-patch-token 완료)
- 관련 문서: [v1.0 릴리즈 로드맵](../../docs/planning/release_v1_roadmap.md), [워커 분업](../../docs/governance/worker_division.md), [상태 스냅샷](./state.json), [ADR-0020](../../docs/adr/0020-account-user-management-boundary.md).
- 브랜치: `gemini/work_260521-c-219-patch-token`

## 2026-05-21 sprint gemini/work_260521-c-219-patch-token (본 sprint) — DREQ Token PATCH 완결

이슈 #219 `[P2-3] DREQ Intake Token PATCH expires_at & allowed_ips API 및 어드민 UI 편집 모달 구현`에 관한 최종 정책 반영 및 백엔드/프론트엔드 테스트 정합성을 완결했습니다.

### 1. 주요 구현 및 정책 결정 내용
- **만료 토큰 연장 정책 (옵션 B 채택)**: 토큰이 만료일시를 지났더라도 완전 폐기(`revoked_at`이 기입됨)된 상태가 아니라면, 만료일을 미래의 특정 시점으로 PATCH하여 토큰을 재활성화(연장)할 수 있도록 허용합니다. (UX 유연성 제공)
- **백엔드 검증 보강**:
  - `dev_request_intake_tokens_integration_test.go`에 만료된 토큰의 expires_at을 재활성화(미래 연장)하여 lookup 시 검증을 통과하는 스토어 통합 테스트 `ExtendExpiredToken_OptionB_Success`를 추가했습니다.
  - `dev_requests_test.go`에 만료된 토큰에 대해 PATCH API를 통해 미래 시각으로 `expires_at`을 수정했을 때 `200 OK`로 성공하고 연장된 만료 시간이 정상 반환되는 단위 테스트 `TestUpdateDevRequestIntakeToken_ExtendExpired_Happy`를 추가하여 비즈니스 가드를 확립했습니다.
- **프론트엔드 E2E 고도화**:
  - `dev-requests.spec.ts` 내 `TC-DREQ-ADMIN-TOKEN-PATCH-01` 시나리오에서 allowed_ips 뿐만 아니라 expires_at을 dynamic PATCH하는 스텝을 추가하여 브라우저 수준에서 연장 동작이 Flaky 없이 성공하고 갱신된 만료일시가 정확하게 표출되는지 Assertion을 강화했습니다.

---

## 2026-05-20 sprint -r (이전 PR) — housekeeping #9
...
| #244 | `6810384` | sprint -n-214-service-account-min-role **(issue #217 P2-1 done)** | ADR-0020 sub-carve E 옵션 A 정공법 — 5 commit: organization.go password 분기 제거 + main.go seedLocalAdmin + KeycloakAdminClient write methods 4건 + IdentityAdmin interface 정리 + docs/infrastructure/keycloak-idp/service_account_min_role.md 신규 |
...
| `-f` | #174 | `947bd2f` | **ADR-0019 §5.3 (8) groups → RBAC role 자동 매핑 design** (`docs/domain/rbac-permissions/keycloak_groups_mapping.md`): 옵션 4종 + 권장 B group composite + **backend 변경 없음** + keycloak_operations §4.3/§8.1 갱신 |
| `-g` | #175 | `aa0c029` | **ADR-0019 §5.3 (7) off-boarding 즉시성 design + ADR-0008 §6 통합** (`docs/infrastructure/keycloak-idp/offboarding_immediacy.md`): 옵션 6종 + Phase 1 HR ETL push + Phase 2 LDAP federation + keycloak_operations §8.2 보강 |
| `-h` | #176 | `455556b` | **ADR-0019 §5.3 (6) Keycloak failover design — §5.3 design 완결 milestone** (`docs/infrastructure/keycloak-idp/failover.md`): 옵션 6종 + Phase 1 graceful degradation + Phase 2 HA active-active + 옵션 E backup IdP 명시 제외 (ADR-0019 충돌) |
...
PR #199/#200 의 design 자산 (`docs/domain/auth-session/account_redesign.md`) 따라 8건 carve 실 구현. 권장 순서:
...
본 세션에서 [`docs/domain/platform-lifecycle/project_concept.md`](../../docs/domain/platform-lifecycle/project_concept.md) 컨셉 1차 머지 완료. 후속은 컨셉 §9 의 4 sprint hook 을 따른다.
