...
title: concept
type: source
tags: [domain, concept.md, project-devhub]
sources: [raw/projects/devhub/docs/domain/onboarding/concept.md]
git_commit: 046e0c81
...
- 관련 문서:
  ...
  - [`docs/domain/auth-session/account_redesign.md`](./account_user_management_redesign.md) — Phase 1/2 의 매트릭스 + 옵션 A 채택 배경
...
3. ~~**ADR 발행** (필요 시) — ADR-0020 §3.2 확장 (self-service unit selection 허용 명문화) vs 신규 ADR-XX.~~ — **2026-05-21 done**: 신규 ADR-0021 발행 (`docs/adr/0021-onboarding-self-service-unit-selection.md`, sprint `claude/onboarding-adr-2026-05-21`). ADR-0020 partial supersession (§3.2 신규 user unit 초기 배치 row + §4.1 sub-carve B + §4.2 + §6.2 의 lazy auto-create 결정). ADR-0020 의 핵심 결정 (옵션 A 책임 경계) 은 변경 없이 유지.
4. ~~**구현 carve 분할** — backend (handler/store/migration/gating/review_transition) + frontend (page/picker/banner/account-edit) + docs (cross-link 갱신) 별 sprint 단위.~~ — **2026-05-21 done**: [`docs/domain/onboarding/impl_plan.md`](./onboarding_impl_plan.md) 신규 — 4 carve (RM-ONBOARD-01 backend / 02 frontend / 03 admin UI / 04 tests) + dependency graph + milestone 매핑 (M-v1.1) + DoD + risk + 후속 carve. sprint `claude/onboarding-impl-carve-plan-2026-05-21`.
5. **traceability 매트릭스 cell 채우기** — IMPL / UT / TC 까지 발급 (현재 매트릭스 row 의 미진입 마킹된 셀, IMPL carve 진행과 동기화). Carve A 머지 시 IMPL-onboarding-* cell + Carve D 머지 시 UT-onboarding-* + TC-ONBOARD-* cell 채움.
...
- **2026-05-21** (`main` HEAD `4d882d5`, sprint `claude/onboarding-arch-2026-05-21`): **ARCH phase 진입**. `docs/planning/system_usecases.md` §2.13 신규 (UC-ONBOARD-01..11) + `docs/architecture.md` §9 신규 (ARCH-ONBOARD-01..06 — 컴포넌트 / 3-tier 상태머신 / Gating 정책 / RBAC route permission / 데이터 모델 / Audit 카탈로그) + `docs/backend_api_contract.md` §16 신규 (API-83 POST /me/onboarding + API-84 GET /organizations/search + API-85 PATCH /me + API-86 POST /admin/users/:id/review + API-32 / API-33 확장 명시). `docs/traceability/report.md` §2.1.5 + §2.2 + §3 매트릭스 + §6 changelog 동기화. 본 문서 §9 next-phase #2 (설계) done. **Self-review Stage 3 보강** — §16.3 POST /me/onboarding 응답 status code `200 OK` → `201 Created` 정합 (P1 #1, POST /dev-requests / POST /users 패턴 일관) + REQ §5.7 forward reference 5곳 stale 정정 (P2 #2, concrete ARCH/API reference 로 갱신) + ARCH §9.4 표에 `onboardingGate` column 추가 (P2 #3, 7 endpoint 의 allowlist 분기 명시) + §16.7 `reviewed_at` / `reviewed_by` source-of-truth 명시 (P2 #4, audit_logs join default + 컬럼 신규 대안 명시). 다음 단계 = #3 ADR 발행 결정.
- **2026-05-21** (sprint `claude/issue-272-onboarding-backend`, issue #272 P2-8): **RM-ONBOARD-01 IMPL-backend done (Carve A)**. migration 000033 (`users.onboarding_completed_at` + `review_status` + bi-implication CHECK) + domain model 확장 (`AppUser` + `UpdateUserInput` + `OnboardingSubmitInput`) + store (4 SELECT 갱신 + `SubmitOnboarding` + `ConfirmUserReview` + `SearchOrgUnits` 신규 + `UpdateUser` ReviewStatus 처리) + onboardingGate middleware 신규 + 5 handler 신규 (`submitOnboarding` API-83 + `searchOrganizations` API-84 + `patchMe` API-85 + `confirmUserReview` API-86 + `getMe` 응답 확장 API-32) + permissions.go (4 endpoint) + main.go env wire (`DEVHUB_ONBOARDING_GATE_ENABLED` default OFF) + UT 11건 (gate / submit happy/preseeded/409/404/422 + org search / confirm review / patch me). Feature flag default OFF — Carve A 단독 머지 후 main 안정성 보장. lazy_auto_create 는 flag conditional (false 시 기존 동작 유지). 다음 단계 = Carve B (issue #273 RM-ONBOARD-02) + Carve C (issue #274 RM-ONBOARD-03) 병행 진입.
- **2026-05-21** (`main` HEAD `175bf9a`, sprint `claude/onboarding-impl-carve-plan-2026-05-21`): **IMPL carve plan + RM 발급 done**. `docs/domain/onboarding/impl_plan.md` 신규 (4 carve scope + dependency + milestone + DoD + risk) + `docs/development_roadmap.md` §3 M7 신규 + `docs/planning/release_v1_roadmap.md` §2.3 Onboarding 도메인 + §3.3 P2-8..11 + §4.2 M-v1.1. **ADR-0021 reservation 정정 (9 위치)** — HA Phase 2 (ADR-0019 §5.3 (6) + keycloak_failover.md) / off-boarding Phase 2 (ADR-0008 §6) / groups → role Phase 2 (ADR-0019 §5.3 (8)) 의 "ADR-0021 후보" reservation 정합 갱신 (ADR-0021 이 Onboarding 으로 발급됨 명시 + Phase 2 진입 시 다음 번호 사용). `docs/traceability/report.md` §2.3 RM-ONBOARD-01..04 + §3 + §6 동기화. 본 문서 §9 next-phase #4 done + #5 진입 plan 명시. 다음 단계 = IMPL carve 실 구현 (Carve A → B+C → D).
- **2026-05-21** (`main` HEAD `105b835`, sprint `claude/onboarding-adr-2026-05-21`): **ADR 발행 done**. `docs/adr/0021-onboarding-self-service-unit-selection.md` 신규 — 컨셉 + 요구사항 + 설계 누적 결정 명문화. ADR-0020 partial supersession (§3.2 신규 user unit 초기 배치 row + §4.1 sub-carve B + §4.2 + §6.2 의 lazy auto-create 결정). ADR-0020 메타 헤더 + §3.2/§4.1/§4.2/§6.1/§6.2 5 위치에 inline supersession banner 추가. ADR-0020 의 핵심 결정 (옵션 A 책임 경계 / `rbac_subject_roles` 제거 / service account 권한 축소) 은 변경 없이 유지. `docs/traceability/report.md` §3 + §4 + §6 동기화. 본 문서 §9 next-phase #3 done. 다음 단계 = #4 IMPL carve 분할.
