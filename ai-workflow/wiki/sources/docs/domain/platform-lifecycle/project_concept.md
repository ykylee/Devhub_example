...
type: source
tags: [domain, project_concept.md, project-devhub]
sources: [raw/projects/devhub/docs/domain/platform-lifecycle/project_concept.md]
git_commit: baf1cf24
git_branch: main
...
- 문서 목적: DevHub 의 신규 1차 도메인 **Platform/Repository/Project 운영 모델** 의 컨셉 — 무엇을 묶는 entity 인지, 어떤 usecase 가 핵심인지, MVP 경계 — 를 정의해 후속 요구사항/usecase/설계 단계의 공통 기준으로 삼는다.
...
- 따라서 본 컨셉은:
  - Platform/Repository/Project 가 **어떤 역할을 갖는 계층 entity 인가** 를 정의하고,
  - **두 핵심 usecase** (일반 사용자 조회 / 시스템 관리자 등록·관리) 를 분리하고,
  - **Platform > Repository > Project > GitHub 실행 단위**의 운영 계층을 공식화하고,
...
- **최상단 Platform** 을 명시적으로 정의한다.
- Platform 하위에 **N개의 Repository(실행 단위)** 를 둔다.
...
요약하면, **총괄 계층은 Platform 중심, 실행 계층은 Repository 중심, 운영 계층은 Project 중심**이다.
...
- **Chief PL (Platform 1명)**: 목표/KPI/예산/리스크 총괄, 우선순위 충돌 조정.
- **Delivery PL (repo별 1명)**: 스프린트 목표/백로그/납기 책임.
- 스프린트 실행 책임은 repo 레벨에 두고, Platform 레벨은 롤업·조정에 집중한다.
...
- **Platform Confluence**: 방향성 문서, 의사결정, 분기 계획, 경영/관리 보고.
...
- 상위(Platform): **큰 로드맵/마일스톤** (분기·반기 게이트).
...
| Platform 신규 등록 | name / description / key / owner / visibility / 기간 / KPI | ✅ |
...
2. Platform 레벨은 **주간 Program Sync + 월간 KPI 리뷰** cadence 로 동기화.
...
platform_repositories
  platform_id          UUID FK applications.id
  ...
  PRIMARY KEY (platform_id, repo_provider, repo_full_name)
...
project_integrations
  -- 참고: 본 컨셉 단계에서는 Platform/Project 어느 레벨에서도 integration 을
  ...
  platform_id     UUID FK applications.id NULL    -- scope=application 인 경우 필수
...
| Design sprint (backend) | `/api/v1/platforms/*`, `/api/v1/platforms/:app_id/projects/*`, `/api/v1/projects/:project_id/repositories/*` 계약 + 마이그레이션 초안 | Usecase sprint 머지 직후 |
| Design sprint (frontend) | `/admin/settings/platforms` IA/화면 흐름 | backend design 과 병행 |
...
| Repository 연결 | `platform_repositories`는 `repo_provider + repo_full_name` 기반 연결 관리 |
...
### 13.3 Platform-Repository 연결 라이프사이클
...
- 동일 `platform_id + repo_provider + repo_full_name` 중복 연결은 금지한다.
...
- 예외 2: `custom` (Platform 단위 관리자 정의, 합계 1.0 ±0.001 허용오차 제약).
...
| 2026-05-13 | 컨셉 심화 — Platform > Repository > Project 운영 계층, Jira/Confluence 하이브리드 정책, 상/하위 로드맵·마일스톤 롤업 규칙, PL·스프린트 운영 모델, 템플릿/예시 문서 링크 추가. | 사용자 워크숍 합의 반영. |
| 2026-05-13 | 설계 고도화 — SCM 어댑터 결정표 + Platform 상태전이/연결 라이프사이클/롤업 계산 규칙 추가. | 현재 세션 반영. |
| 2026-05-14 | 리뷰 보강 — §7 데이터 모델 PK 정합 (platform_id+repo_provider+repo_full_name) + 신규 컬럼 동기 + `project_integrations` 명 통일. §10 `key 형식` row close, 비-Jira SCM Jira 매핑 row 신규. §13.4 weight normalize 룰 + 허용오차 ±0.001 명문화. ADR-0011 placeholder 발급. | PR #104 본인 리뷰 보강. |
