...
sources: [raw/projects/devhub/docs/domain/platform-lifecycle/api.md]
git_commit: 6c434887
git_branch: main
...
# platform-lifecycle 도메인 API

- 문서 목적: Platform/Project 관리 + Platform 개발 대시보드(APPDASH) + SCM Provider Catalog API 계약을 정의한다.
...
본 도메인 API 는 프로젝트 관리 도메인의 API 계약이다. historical 모델은 `Platform > Repository > Project` 였고, 현재는 `Platform > Project > Repository(N:M)` 로 전환 중이다. `DEVHUB_PROJECT_MODEL=legacy|hybrid|v2` 토글을 사용하며 기본은 `hybrid`.
...
| `API-43` | `GET /api/v1/platforms` | §4 | activated |
| `API-44` | `POST /api/v1/platforms` | §4 | activated |
| `API-45` | `GET /api/v1/platforms/{platform_id}` | §4 | activated |
| `API-46` | `PATCH /api/v1/platforms/{platform_id}` | §4 | activated |
| `API-47` | `DELETE /api/v1/platforms/{platform_id}` (archive) | §4 | activated |
| `API-48` | `GET /api/v1/platforms/{platform_id}/repositories` | §5 | activated |
| `API-49` | `POST /api/v1/platforms/{platform_id}/repositories` | §5 | activated |
| `API-50` | `DELETE /api/v1/platforms/{platform_id}/repositories/{repo_key}` | §5 | activated (path: gin catch-all, `provider:org/repo` 콜론 컨벤션) |
...
- `Platform.key`는 시스템 전역 unique 식별자다.
- `Platform.key`는 **immutable** — 발급 후 변경 불가. PATCH 본문에 `key` 가 포함되면 `422 application_key_immutable` 로 거절한다.
- `Platform.key` 입력 정책: 영문/숫자 10자 (`^[A-Za-z0-9]{10}$`).
...
### 4.1 `GET /api/v1/platforms` (API-43 (planned))
...
### 4.2 `POST /api/v1/platforms` (API-44 (planned))
...
### 4.3 `GET /api/v1/platforms/{platform_id}` (API-45 (planned))
...
### 4.4 `PATCH /api/v1/platforms/{platform_id}` (API-46 (planned))

- 설명: Platform 메타/상태 수정.
...
### 4.5 `DELETE /api/v1/platforms/{platform_id}` (API-47 (planned))

- 설명: archive (soft-delete) **+ archived 상태에서 `?hard=true` 시 영구 삭제** (sprint `claude/work_260528-archived-hard-delete` 활성화).
- 동작:
  - `?hard=true` + `status=archived` → hard-delete (`DELETE FROM applications`, FK cascade: platform_repositories 자동 삭제, projects.platform_id 는 ON DELETE SET NULL 로 보존). audit `application.deleted`.
...
## 5. Platform-Repository 연결

### 5.1 `GET /api/v1/platforms/{platform_id}/repositories` (API-48 (planned))

- 설명: Platform에 연결된 Repository 조회. 직접 link + 프로젝트 경유 간접 link 의 UNION (#395 활성화, #395/#396 후속 carve).
- 응답 data 의 각 link 객체에 `link_source` 필드 (`direct` | `via_project`) — UI/디버깅/감사용:
  - `direct`: `platform_repositories` 의 직접 link (SCM webhook sync metadata 보유)
...
### 5.2 `POST /api/v1/platforms/{platform_id}/repositories` (API-49 (planned))
...
- **scope**: 본 `sync_error_code` 는 **link (platform_id, repo_provider, repo_full_name) 단위의 최신 에러 1건만 캐시**한다. 개별 webhook event/payload 단위 상세 에러는 `webhook_events` (현행) 또는 후속 `adapter_event_logs` 테이블에 보관 (예: 동일 link 에서 1시간 동안 발생한 N건의 `webhook_signature_invalid` 는 link.sync_error_code 에 최신 1건 + 카운트는 별도 테이블).
...
### 5.3 `DELETE /api/v1/platforms/{platform_id}/repositories/{repo_key}` (API-50 (planned))

- 설명: Platform-Repository 연결 해제.

## 6. Project + Repository 연결

운영 모델은 `Platform > Project > Repository(N:M)` 을 기본으로 한다. legacy 경로(`/repositories/{repository_id}/projects`)는 호환을 위해 유지되며 `DEVHUB_PROJECT_MODEL=v2` 에서는 `410 gone` 으로 비활성화된다.
...
### 6.3 `GET /api/v1/platforms/{platform_id}/projects` (API-56A)

- 설명: Platform 하위 Project 목록 조회 (v2 경로).

### 6.4 `POST /api/v1/platforms/{platform_id}/projects` (API-56A)

- 설명: Platform 하위 Project 생성 (v2 경로).
- 요청 body:
...
- 요청 body 필드 (모두 optional, nil = 변경 안 함):
  ...
  - `platform_id` — application 이전/해제 (#395/#396 후속 carve):
    - `""` (빈 string) → 해제 (NULL, ON DELETE SET NULL 의도 일치)
    - non-empty UUID → 해당 application 으로 이전 (존재하지 않으면 422 `platform_id_invalid`)
  - `hold_reason` / `resume_reason` / `archived_reason` — status 전이 필수 payload
- audit: `project.updated` event 의 details 에 `platform_id_from` / `platform_id_to` 기록 (변경된 경우).

### 6.10 `DELETE /api/v1/projects/{project_id}` (API-56 (planned))

- 설명: Project archive (soft-delete) — Platform archive 와 동일 규칙.

## 7. Platform 롤업

### 7.1 `GET /api/v1/platforms/{platform_id}/rollup` (API-57 (planned))

- 설명: Repository 단위 운영 지표를 Platform 레벨로 집계해 조회.
- Query:
...
- 설명: Platform/Project 연계 통합 설정 조회.
...
- **설명**: Platform 상세 대시보드용 실시간 빌드 상태, 다차원 품질 메트릭, 하위 프로젝트 진척율 및 지연 리스크 배지, 매핑된 DREQ 목록, SCM 및 빌드 시계열 트렌드 데이터를 일괄 병렬 집계하여 반환합니다.
