...
| **X-8 (Keycloak SPI realm events push 전환) 정공법 = compose + docs only** | X-8 의 backend 코드 변경 0 — SPI source + webhook handler + polling cron + dedup 모두 main 정합. compose mount + SOP + verify script + 문서 정공법 | — | ✅ implemented (sprint `feat/260617-x8-keycloak-event-listener-spi-sop`, 2026-06-17) — (1) `infra/idp/keycloak-event-listener-spi/` 의 Java SPI ([`DevHubEventListenerProvider.java`](../../infra/idp/keycloak-event-listener-spi/src/main/java/com/devhub/keycloak/spi/DevHubEventListenerProvider.java) async sendAsync POST + noisy event skip + 3s timeout + [`DevHubEventListenerProviderFactory.java`](../../infra/idp/keycloak-event-listener-spi/src/main/java/com/devhub/keycloak/spi/DevHubEventListenerProviderFactory.java) env-var wire) main 정합. (2) `docker-compose.colima.yml` + `docker-compose.deploy.yml` keycloak service 에 SPI JAR volume mount `./infra/idp/keycloak-event-listener-spi/target/devhub-keycloak-event-listener.jar:/opt/keycloak/providers/devhub-keycloak-event-listener.jar:ro` + env `DEVHUB_BACKEND_SPI_WEBHOOK_URL` + `DEVHUB_KEYCLOAK_SPI_WEBHOOK_SECRET` 추가 (P2-6 deploy 정공법). (3) [`docs/setup/keycloak_event_listener_spi.md`](../setup/keycloak_event_listener_spi.md) 정공법 SOP 신규 (architecture + build + compose mount + realm config + SPI 동작 flow + verify + 운영 SOP + 잔여 carve). (4) [`scripts/verify-keycloak-spi.sh`](../../scripts/verify-keycloak-spi.sh) 자동 검증 4 항목 (SPI JAR classpath / Realm eventsListeners / Env var / Webhook push smoke latency < 1s) 작성. 검증: `tsc --noEmit` 본 PR 영향 0 + backend `go build ./...` silent (변경 0) + compose YAML lint 정공법. **Tier**: **공용** (Java source + compose + docs 만, 사내 한정 정보 0). 잔여 = 사내 빌드 (Java 21 + Maven 3.13+ 환경, 사용자 결정) + 사내 staging/prod 적용 후 `verify-keycloak-spi.sh` 1회 실행 (4 항목 PASS 확인). residual polling cron = at-least-once deliver 보장 (push fail fallback). |
