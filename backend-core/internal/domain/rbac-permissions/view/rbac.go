...
func (h *RBACHandler) GetRBACPolicyLegacyGone(c *gin.Context) {
...
func (h *RBACHandler) ListRBACPolicies(c *gin.Context) {
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "rbac.list_policies")
		return
	}
...
func (h *RBACHandler) CreateRBACPolicy(c *gin.Context) {
	if h.cfg.RBACStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"status": "unavailable", "error": "rbac store is not configured"})
		return
	}
	...
	case errors.Is(err, store.ErrAuditInvariantViolation):
		...
		return
	case err != nil:
		httphelp.WriteServerError(c, err, "rbac.create_policy")
		return
	}
...
func (h *RBACHandler) UpdateRBACPolicies(c *gin.Context) {
	if h.cfg.RBACStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"status": "unavailable", "error": "rbac store is not configured"})
		return
	}
	...
	for _, entry := range req.Roles {
		...
		if err != nil {
			httphelp.WriteServerError(c, err, "rbac.update_policies.lookup")
			return
		}
	...
	for _, plan := range planned {
		...
		if plan.writePM {
			...
			case errors.Is(err, store.ErrAuditInvariantViolation):
				...
				return
			case err != nil:
				httphelp.WriteServerError(c, err, "rbac.update_policies.permissions")
				return
			}
		...
		if plan.writeMD {
			...
			case errors.Is(err, store.ErrNotFound):
				...
				return
			case err != nil:
				httphelp.WriteServerError(c, err, "rbac.update_policies.metadata")
				return
			}
			auditEntries = append(auditEntries, map[string]any{
				"role_id":     entry.ID,
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "rbac.update_policies.list_after")
		return
	}
	data := make([]rbacRoleWire, 0, len(roles))
	for _, role := range roles {
...
func (h *RBACHandler) DeleteRBACPolicy(c *gin.Context) {
	if h.cfg.RBACStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"status": "unavailable", "error": "rbac store is not configured"})
		return
	}
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "rbac.delete_policy.lookup")
		return
	}
	...
	if err := h.cfg.RBACStore.DeleteRBACRole(ctx, roleID); err != nil {
		...
		case errors.Is(err, store.ErrNotFound):
			c.JSON(http.StatusNotFound, gin.H{"status": "not_found", "error": fmt.Sprintf("role %q not found", roleID)})
		default:
			httphelp.WriteServerError(c, err, "rbac.delete_policy")
		}
		return
	}
