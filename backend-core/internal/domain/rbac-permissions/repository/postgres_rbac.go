...
func (r *RBACRepository) ListRBACRoles(ctx context.Context) ([]domain.RBACRole, error) {
	const query = `
...
func (r *RBACRepository) GetRBACRole(ctx context.Context, roleID string) (domain.RBACRole, error) {
	const query = `
SELECT role_id, name, description, is_system, permissions::text, created_at, updated_at
FROM rbac_policies
WHERE role_id = $1`

	row := r.store.Pool().QueryRow(ctx, query, roleID)
	role, err := scanRBACRole(row)
	if errors.Is(err, pgx.ErrNoRows) {
		return domain.RBACRole{}, fmt.Errorf("rbac role %s: %w", roleID, store.ErrNotFound)
	}
	if err != nil {
...
func (r *RBACRepository) CreateRBACRole(ctx context.Context, role domain.RBACRole) (domain.RBACRole, error) {
	if domain.IsSystemRole(role.ID) {
		return domain.RBACRole{}, fmt.Errorf("create role %s: %w", role.ID, store.ErrConflict)
	}
...
RETURNING role_id, name, description, is_system, permissions::text, created_at, updated_at`
	...
	row := r.store.Pool().QueryRow(ctx, query, role.ID, role.Name, role.Description, payload)
	created, err := scanRBACRole(row)
	if err != nil {
		if store.IsUniqueViolation(err) {
			return domain.RBACRole{}, fmt.Errorf("create role %s: %w", role.ID, store.ErrConflict)
		}
		if store.IsCheckViolation(err, rbacAuditInvariantConstraint) {
...
func (r *RBACRepository) UpdateRBACRolePermissions(ctx context.Context, roleID string, perms domain.PermissionMatrix) (domain.RBACRole, error) {
...
RETURNING role_id, name, description, is_system, permissions::text, created_at, updated_at`
	...
	row := r.store.Pool().QueryRow(ctx, query, roleID, payload)
	updated, err := scanRBACRole(row)
	if errors.Is(err, pgx.ErrNoRows) {
		return domain.RBACRole{}, fmt.Errorf("update role %s: %w", roleID, store.ErrNotFound)
	}
	if err != nil {
		if store.IsCheckViolation(err, rbacAuditInvariantConstraint) {
...
RETURNING role_id, name, description, is_system, permissions::text, created_at, updated_at`
	...
	row := r.store.Pool().QueryRow(ctx, query, roleID, name, description)
	updated, err := scanRBACRole(row)
	if errors.Is(err, pgx.ErrNoRows) {
		return domain.RBACRole{}, fmt.Errorf("update role %s metadata: %w", roleID, store.ErrNotFound)
	}
	if err != nil {
...
func (r *RBACRepository) DeleteRBACRole(ctx context.Context, roleID string) error {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return fmt.Errorf("delete role %s: %w", roleID, store.ErrNotFound)
	}
	if err != nil {
