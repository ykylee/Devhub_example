...
func (r *OrganizationRepository) ListUsers(ctx context.Context, opts domain.UserListOptions) ([]domain.AppUser, int, error) {
	limit := opts.Limit
...
WHERE ($1 = '' OR role = $1)
  ...
  AND ($3 = '' OR primary_unit_id = $3)`
	...
	var total int
	if err := r.store.Pool().QueryRow(ctx, countQuery, opts.Role, opts.Status, opts.PrimaryUnitID).Scan(&total); err != nil {
...
LIMIT $1 OFFSET $2`
	...
	rows, err := r.store.Pool().Query(ctx, query, limit, offset, opts.Role, opts.Status, opts.PrimaryUnitID)
	if err != nil {
	...
	if len(userIDs) > 0 {
		appointments, err := r.appointmentsForUsers(ctx, userIDs)
		if err != nil {
			return nil, 0, err
		}
		for i := range users {
...
func (r *OrganizationRepository) GetUser(ctx context.Context, userID string) (domain.AppUser, error) {
	const query = `
SELECT
	id,
	user_id,
...
LIMIT 1`
	...
	var role string
	var status string
	err := r.store.Pool().QueryRow(ctx, query, userID).Scan(
		&user.ID,
		&user.UserID,
		&user.Email,
		&user.DisplayName,
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			...
			// the store.store.ErrNotFound sentinel so callers can `errors.Is`
	...
	appointments, err := r.GetUserAppointments(ctx, userID)
	if err != nil {
...
// O(1) lookup.
func (r *OrganizationRepository) GetUserByIdPSubject(ctx context.Context, identityID string) (domain.AppUser, error) {
	const query = `
SELECT
	id,
	user_id,
...
LIMIT 1`
	...
	var role string
	var status string
	err := r.store.Pool().QueryRow(ctx, query, identityID).Scan(
		&user.ID,
		&user.UserID,
		&user.Email,
		&user.DisplayName,
	...
	appointments, err := r.GetUserAppointments(ctx, user.UserID)
	if err != nil {
		return domain.AppUser{}, err
	}
	user.Appointments = appointments
...
func (r *OrganizationRepository) GetUserAppointments(ctx context.Context, userID string) ([]domain.UnitAppointment, error) {
	const query = `
...
func (r *OrganizationRepository) appointmentsForUsers(ctx context.Context, userIDs []string) (map[string][]domain.UnitAppointment, error) {
	const query = `
SELECT unit_id, user_id, appointment_role
FROM unit_appointments
WHERE user_id = ANY($1)
ORDER BY user_id ASC, unit_id ASC`
	...
	rows, err := r.store.Pool().Query(ctx, query, userIDs)
	if err != nil {
...
func (r *OrganizationRepository) GetHierarchy(ctx context.Context) (domain.Hierarchy, error) {
	const unitsQuery = `
WITH RECURSIVE
descendants AS (
...
ORDER BY o.unit_id ASC`
	...
	rows, err := r.store.Pool().Query(ctx, unitsQuery)
	if err != nil {
...
ORDER BY parent_unit_id, unit_id`
	...
	edgeRows, err := r.store.Pool().Query(ctx, edgesQuery)
	if err != nil {
...
// specific unit.
func (r *OrganizationRepository) ListUnitMembers(ctx context.Context, unitID string) ([]domain.AppUser, error) {
	const query = `
SELECT DISTINCT
	u.id,
	u.user_id,
...
ORDER BY u.user_id ASC`
	...
	rows, err := r.store.Pool().Query(ctx, query, unitID)
	if err != nil {
...
func (r *OrganizationRepository) ReplaceUnitMembers(ctx context.Context, unitID string, userIDs []string) error {
	...
	if err := tx.QueryRow(ctx, `SELECT 1 FROM org_units WHERE unit_id = $1`, unitID).Scan(&exists); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return fmt.Errorf("unit %s not found: %w", unitID, store.ErrNotFound)
		}
...
// CreateUser inserts a new application user. Returns store.ErrConflict if user_id or
// email already exist.
func (r *OrganizationRepository) CreateUser(ctx context.Context, input domain.CreateUserInput) (domain.AppUser, error) {
...
RETURNING id, created_at, updated_at`
	...
	if err != nil {
		if store.IsUniqueViolation(err) {
			return domain.AppUser{}, fmt.Errorf("create user %s: %w", input.UserID, store.ErrConflict)
		}
		if store.IsForeignKeyViolation(err) {
			return domain.AppUser{}, fmt.Errorf("create user %s references missing unit: %w", input.UserID, store.ErrNotFound)
		}
...
// applied. Returns store.ErrNotFound if no row matches.
func (r *OrganizationRepository) UpdateUser(ctx context.Context, userID string, input domain.UpdateUserInput) (domain.AppUser, error) {
	...
	if err != nil {
		if store.IsUniqueViolation(err) {
			return domain.AppUser{}, fmt.Errorf("update user %s: %w", userID, store.ErrConflict)
		}
		if store.IsForeignKeyViolation(err) {
			return domain.AppUser{}, fmt.Errorf("update user %s references missing unit: %w", userID, store.ErrNotFound)
		}
...
// store.ErrNotFound when no row matches.
func (r *OrganizationRepository) DeleteUser(ctx context.Context, userID string) error {
	tag, err := r.store.Pool().Exec(ctx, `DELETE FROM users WHERE user_id = $1`, userID)
	if err != nil {
...
//   - existing row 가 이미 onboarding_completed_at IS NOT NULL → store.ErrConflict
...
//   - row 없음 → INSERT.
//
// Returns store.ErrConflict (이미 완료) / store.ErrNotFound (primary_unit_id 가
// organization_units 에 없음, FK violation).
func (r *OrganizationRepository) SubmitOnboarding(ctx context.Context, input domain.OnboardingSubmitInput) (domain.AppUser, error) {
...
INSERT INTO users (
	...
	if err != nil {
		if store.IsUniqueViolation(err) {
			return domain.AppUser{}, fmt.Errorf("submit onboarding %s: %w", input.UserID, store.ErrConflict)
		}
		if store.IsForeignKeyViolation(err) {
			return domain.AppUser{}, fmt.Errorf("submit onboarding %s references missing unit %s: %w", input.UserID, input.PrimaryUnitID, store.ErrNotFound)
		}
	...
	return r.GetUser(ctx, input.UserID)
}
...
//
// Returns store.ErrNotFound (user 미존재) / store.ErrConflict (이미 reviewed 이거나
// onboarding 미제출).
func (r *OrganizationRepository) ConfirmUserReview(ctx context.Context, userID string) (domain.AppUser, error) {
...
WHERE user_id = $1
  ...
  AND review_status = 'pending_review'`,
	...
	if tag.RowsAffected() == 0 {
		...
		// 본 store layer 는 단순 affected=0 → store.ErrNotFound 반환.
...
func (r *OrganizationRepository) CountPendingReview(ctx context.Context) (int, error) {
	var count int
...
// idp_subject after ADR-0019. This is the only writer. Returns store.ErrNotFound
// when no user matches the given user_id — best-effort callers (lazy
// backfill paths) typically ignore that case.
func (r *OrganizationRepository) SetIdPSubject(ctx context.Context, userID, identityID string) error {
	if strings.TrimSpace(userID) == "" {
	...
	if err != nil {
		if store.IsUniqueViolation(err) {
			return fmt.Errorf("idp_subject %s already mapped: %w", identityID, store.ErrConflict)
		}
...
func (r *OrganizationRepository) SearchOrgUnits(ctx context.Context, q string, limit int) ([]domain.OrgUnit, error) {
	if limit <= 0 || limit > 20 {
		limit = 20
	}
...
LIMIT $2`
	...
	rows, err := r.store.Pool().Query(ctx, query, pattern, limit)
	if err != nil {
...
func (r *OrganizationRepository) GetOrgUnit(ctx context.Context, unitID string) (domain.OrgUnit, error) {
	const query = `
SELECT
	id,
	unit_id,
...
LIMIT 1`
	...
	var unit domain.OrgUnit
	var unitType string
	err := r.store.Pool().QueryRow(ctx, query, unitID).Scan(
		&unit.ID,
		&unit.UnitID,
		&unit.ParentUnitID,
		&unitType,
...
func (r *OrganizationRepository) CreateOrgUnit(ctx context.Context, input domain.CreateOrgUnitInput) (domain.OrgUnit, error) {
	const query = `
...
RETURNING id, created_at, updated_at`
	...
	if err != nil {
		if store.IsUniqueViolation(err) {
			return domain.OrgUnit{}, fmt.Errorf("create unit %s: %w", input.UnitID, store.ErrConflict)
		}
		if store.IsForeignKeyViolation(err) {
			return domain.OrgUnit{}, fmt.Errorf("create unit %s references missing parent: %w", input.UnitID, store.ErrNotFound)
		}
	...
	if leader != "" {
		...
		if _, err := tx.Exec(ctx,
			...
			if store.IsForeignKeyViolation(err) {
				return domain.OrgUnit{}, fmt.Errorf("create unit %s leader %s: %w", input.UnitID, leader, store.ErrNotFound)
			}
...
func (r *OrganizationRepository) UpdateOrgUnit(ctx context.Context, unitID string, input domain.UpdateOrgUnitInput) (domain.OrgUnit, error) {
	...
	if input.ParentUnitID != nil {
		if *input.ParentUnitID == unitID {
			return domain.OrgUnit{}, fmt.Errorf("unit %s cannot be its own parent: %w", unitID, store.ErrConflict)
		}
	...
	if len(setClauses) == 0 {
		return r.GetOrgUnit(ctx, unitID)
	}
	...
	if err != nil {
		if store.IsForeignKeyViolation(err) {
			return domain.OrgUnit{}, fmt.Errorf("update unit %s references missing parent: %w", unitID, store.ErrNotFound)
		}
	...
	if input.LeaderUserID != nil {
		...
		if leaderNormalized != "" {
			if _, err := tx.Exec(ctx,
				...
				if store.IsForeignKeyViolation(err) {
					return domain.OrgUnit{}, fmt.Errorf("update unit %s leader %s: %w", unitID, leaderNormalized, store.ErrNotFound)
				}
...
func (r *OrganizationRepository) DeleteOrgUnit(ctx context.Context, unitID string) error {
	tag, err := r.store.Pool().Exec(ctx, `DELETE FROM org_units WHERE unit_id = $1`, unitID)
	if err != nil {
		if store.IsForeignKeyViolation(err) {
			return fmt.Errorf("delete unit %s: %w", unitID, store.ErrConflict)
		}
...
func (r *OrganizationRepository) UpdateHierarchy(ctx context.Context, hie domain.Hierarchy) error {
	tx, err := r.store.Pool().Begin(ctx)
	if err != nil {
		return err
	}
