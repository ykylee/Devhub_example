...
func (h *OrganizationHandler) ListUsers(c *gin.Context) {
	...
	limit, err := httphelp.ParseBoundedInt(c.DefaultQuery("limit", "50"), 1, 100)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"status": "rejected", "error": "limit must be an integer between 1 and 100"})
		return
	}
	offset, err := httphelp.ParseBoundedInt(c.DefaultQuery("offset", "0"), 0, 100000)
	if err != nil {
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "organization.list_users")
		return
	}
...
func (h *OrganizationHandler) GetUser(c *gin.Context) {
	...
	if err != nil {
		...
		httphelp.WriteServerError(c, err, "organization.get_user")
		return
	}
...
func (h *OrganizationHandler) GetHierarchy(c *gin.Context) {
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "organization.get_hierarchy")
		return
	}
...
func (h *OrganizationHandler) ListUnitMembers(c *gin.Context) {
	if h.cfg.OrganizationStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"status": "unavailable",
			"error":  "organization store is not configured",
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "organization.list_unit_members")
		return
	}
...
// 000004 + 000021 team_manager). PR #205 codex review (P1, sprint -d): team_manager
// 가 누락된 상태로 sprint -d 의 `/rbac/subjects/:id/roles` 폐기와 합쳐지면
// team_manager 가 API 로 할당 불가능한 회귀 — 본 sprint hotfix 로 추가.
...
func writeStoreError(c *gin.Context, err error, op string) {
	...
	default:
		httphelp.WriteServerError(c, err, op)
	}
}
...
func (h *OrganizationHandler) CreateUser(c *gin.Context) {
	...
	if !validAppRoles[req.Role] {
		c.JSON(http.StatusBadRequest, gin.H{"status": "rejected", "error": "role must be developer, team_manager, or system_admin"})
		return
	}
...
func (h *OrganizationHandler) GetOrgUnit(c *gin.Context) {
	if h.cfg.OrganizationStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"status": "unavailable",
			"error":  "organization store is not configured",
...
func (h *OrganizationHandler) CreateOrgUnit(c *gin.Context) {
	if h.cfg.OrganizationStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"status": "unavailable",
			"error":  "organization store is not configured",
...
func (h *OrganizationHandler) UpdateOrgUnit(c *gin.Context) {
	if h.cfg.OrganizationStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"status": "unavailable",
			"error":  "organization store is not configured",
...
func (h *OrganizationHandler) DeleteOrgUnit(c *gin.Context) {
	if h.cfg.OrganizationStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"status": "unavailable",
			"error":  "organization store is not configured",
...
func (h *OrganizationHandler) ReplaceUnitMembers(c *gin.Context) {
	if h.cfg.OrganizationStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"status": "unavailable",
			"error":  "organization store is not configured",
	...
	members, err := h.cfg.OrganizationStore.ListUnitMembers(c.Request.Context(), unitID)
	if err != nil {
		httphelp.WriteServerError(c, err, "organization.replace_unit_members.list")
		return
	}
	data := make([]appUserResponse, 0, len(members))
	for _, user := range members {
...
func (h *OrganizationHandler) UpdateHierarchy(c *gin.Context) {
	if h.cfg.OrganizationStore == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"status": "unavailable",
			"error":  "organization store is not configured",
