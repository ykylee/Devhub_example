...
func (h *DevRequestHandler) DevRequestStoreOrUnavailable(c *gin.Context) (DevRequestStore, bool) {
...
func (h *DevRequestHandler) IntakeDevRequest(c *gin.Context) {
	...
	if req.ExternalRef != "" {
		...
		if !errors.Is(err, store.ErrNotFound) {
			httphelp.WriteServerError(c, err, "dev_request.intake.idempotency_lookup")
			return
		}
	}
	...
	if errors.Is(err, store.ErrConflict) {
		...
		created, err = storeI.CreateDevRequest(c.Request.Context(), dr)
		if err != nil {
			httphelp.WriteServerError(c, err, "dev_request.intake.create_rejected_fallback")
			return
		}
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "dev_request.intake.create")
		return
	}
...
func (h *DevRequestHandler) ListDevRequests(c *gin.Context) {
	storeI, ok := h.DevRequestStoreOrUnavailable(c)
	if !ok {
		return
	}
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "dev_request.list")
		return
	}
...
func (h *DevRequestHandler) GetDevRequest(c *gin.Context) {
	storeI, ok := h.DevRequestStoreOrUnavailable(c)
	if !ok {
		return
	}
...
func (h *DevRequestHandler) RegisterDevRequest(c *gin.Context) {
	storeI, ok := h.DevRequestStoreOrUnavailable(c)
	if !ok {
		return
	}
...
func (h *DevRequestHandler) registerDevRequestLegacy(c *gin.Context, storeI DevRequestStore, drID string, targetType domain.DevRequestTargetType, targetID string) {
	updated, err := storeI.MarkDevRequestRegistered(c.Request.Context(), drID, targetType, targetID)
	if err != nil {
		httphelp.WriteServerError(c, err, "dev_request.register.mark")
		return
	}
...
func (h *DevRequestHandler) registerDevRequestWithNewProject(c *gin.Context, storeI DevRequestStore, drID string, p *registerProjectPayload) {
	// repository_id is required (FK to repositories.id). Frontend resolves
	// the numeric ID from (repo_provider, repo_full_name) before calling this endpoint.
	if p.RepositoryID == 0 {
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "dev_request.register.promote_project")
		return
	}
...
func (h *DevRequestHandler) RejectDevRequest(c *gin.Context) {
	storeI, ok := h.DevRequestStoreOrUnavailable(c)
	if !ok {
		return
	}
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "dev_request.reject.transition")
		return
	}
...
func (h *DevRequestHandler) PatchDevRequest(c *gin.Context) {
	storeI, ok := h.DevRequestStoreOrUnavailable(c)
	if !ok {
		return
	}
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "dev_request.reassign.lookup")
		return
	}
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "dev_request.reassign.update")
		return
	}
...
func (h *DevRequestHandler) CloseDevRequest(c *gin.Context) {
	storeI, ok := h.DevRequestStoreOrUnavailable(c)
	if !ok {
		return
	}
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "dev_request.close.lookup")
		return
	}
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "dev_request.close.transition")
		return
	}
