...
type DevRequestListOptions = store.DevRequestListOptions
...
func (r *DevRequestRepository) CreateDevRequest(ctx context.Context, dr domain.DevRequest) (domain.DevRequest, error) {
...
func (r *DevRequestRepository) GetDevRequest(ctx context.Context, id string) (domain.DevRequest, error) {
...
func (r *DevRequestRepository) GetDevRequestByExternalRef(ctx context.Context, sourceSystem, externalRef string) (domain.DevRequest, error) {
	if externalRef == "" {
		return domain.DevRequest{}, store.ErrNotFound
	}
...
func (r *DevRequestRepository) ListDevRequests(ctx context.Context, opts DevRequestListOptions) ([]domain.DevRequest, int, error) {
	limit := opts.Limit
...
WHERE (cardinality($1::text[]) = 0 OR status = ANY($1::text[]))
  ...
  AND ($3 = '' OR source_system = $3)`
	...
	var total int
	if err := r.store.Pool().QueryRow(ctx, countQuery, statusStrings, opts.AssigneeUserID, opts.SourceSystem).Scan(&total); err != nil {
...
LIMIT $4 OFFSET $5`
	...
	rows, err := r.store.Pool().Query(ctx, listQuery, statusStrings, opts.AssigneeUserID, opts.SourceSystem, limit, offset)
	if err != nil {
...
func (r *DevRequestRepository) TransitionDevRequestStatus(ctx context.Context, id string, to domain.DevRequestStatus, rejectedReason string) (domain.DevRequest, error) {
	const updateQuery = `
...
RETURNING` + devRequestsSelectColumns
	...
	row := r.store.Pool().QueryRow(ctx, updateQuery, id, string(to), rejectedReason)
...
func (r *DevRequestRepository) ReassignDevRequest(ctx context.Context, id, newAssigneeUserID string) (domain.DevRequest, error) {
	const updateQuery = `
UPDATE dev_requests SET
    assignee_user_id = $2,
    updated_at = NOW()
...
RETURNING` + devRequestsSelectColumns
	...
	row := r.store.Pool().QueryRow(ctx, updateQuery, id, newAssigneeUserID)
	updated, err := scanDevRequest(row)
	if errors.Is(err, pgx.ErrNoRows) {
		return domain.DevRequest{}, store.ErrNotFound
	}
...
func (r *DevRequestRepository) MarkDevRequestRegistered(ctx context.Context, id string, targetType domain.DevRequestTargetType, targetID string) (domain.DevRequest, error) {
	const updateQuery = `
UPDATE dev_requests SET
    status = 'registered',
...
RETURNING` + devRequestsSelectColumns
	...
	row := r.store.Pool().QueryRow(ctx, updateQuery, id, string(targetType), targetID)
	updated, err := scanDevRequest(row)
	if errors.Is(err, pgx.ErrNoRows) {
		return domain.DevRequest{}, store.ErrNotFound
	}
