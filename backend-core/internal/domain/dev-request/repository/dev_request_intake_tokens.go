...
func (r *DevRequestRepository) LookupDevRequestIntakeToken(ctx context.Context, hashedToken string) (domain.DevRequestIntakeToken, error) {
	const query = `
...
WHERE hashed_token = $1`
	row := r.store.Pool().QueryRow(ctx, query, hashedToken)
...
func (r *DevRequestRepository) MarkDevRequestIntakeTokenUsed(ctx context.Context, tokenID string) error {
	const query = `UPDATE dev_request_intake_tokens SET last_used_at = NOW() WHERE token_id = $1::uuid`
	_, err := r.store.Pool().Exec(ctx, query, tokenID)
	if err != nil {
...
func (r *DevRequestRepository) CreateDevRequestIntakeToken(ctx context.Context, tok domain.DevRequestIntakeToken) (domain.DevRequestIntakeToken, error) {
...
RETURNING token_id::text, client_label, hashed_token, allowed_ips, source_system,
	...
	row := r.store.Pool().QueryRow(ctx, query, tok.ClientLabel, tok.HashedToken, allowedIPs, tok.SourceSystem, tok.CreatedBy, tok.ExpiresAt)
...
func (r *DevRequestRepository) ListDevRequestIntakeTokens(ctx context.Context) ([]domain.DevRequestIntakeToken, error) {
	const query = `
SELECT token_id::text, client_label, hashed_token, allowed_ips, source_system,
       created_at, created_by, last_used_at, revoked_at, expires_at
FROM dev_request_intake_tokens
...
func (r *DevRequestRepository) RevokeDevRequestIntakeToken(ctx context.Context, tokenID string) (domain.DevRequestIntakeToken, error) {
	const query = `
...
RETURNING token_id::text, client_label, hashed_token, allowed_ips, source_system,
          created_at, created_by, last_used_at, revoked_at, expires_at`
	row := r.store.Pool().QueryRow(ctx, query, tokenID)
...
func (r *DevRequestRepository) UpdateDevRequestIntakeTokenIPs(ctx context.Context, tokenID string, allowedIPs []string) (domain.DevRequestIntakeToken, error) {
	return r.UpdateDevRequestIntakeToken(ctx, tokenID, allowedIPs, nil, true, false)
}
...
func (r *DevRequestRepository) UpdateDevRequestIntakeToken(ctx context.Context, tokenID string, allowedIPs []string, expiresAt *time.Time, updateIPs bool, updateExpiresAt bool) (domain.DevRequestIntakeToken, error) {
	var ipsJSON []byte
	if updateIPs {
		var err error
...
LEFT JOIN upd    ON true`
	...
	if scanErr := r.store.Pool().QueryRow(ctx, query, tokenID, updateIPs, ipsJSON, updateExpiresAt, expiresAt).Scan(
		&lockTokenID,
		&lockRevokedAt,
		&updTokenID,
		&updClientLabel,
...
func (r *DevRequestRepository) HardRevokeExpiredIntakeTokens(ctx context.Context, before time.Time) ([]string, error) {
	const query = `
UPDATE dev_request_intake_tokens
...
RETURNING token_id::text`
	rows, err := r.store.Pool().Query(ctx, query, before)
	if err != nil {
...
func (r *DevRequestRepository) CountExpiringSoonIntakeTokens(ctx context.Context, threshold time.Time) (int, error) {
	const query = `
...
WHERE expires_at IS NOT NULL
  ...
  AND revoked_at IS NULL`
	var count int
	if err := r.store.Pool().QueryRow(ctx, query, threshold).Scan(&count); err != nil {
...
func (r *DevRequestRepository) CountStaleIntakeTokens(ctx context.Context, before time.Time) (int, error) {
	const query = `
SELECT COUNT(*) FROM dev_request_intake_tokens
WHERE revoked_at IS NULL
  AND (
	...
	var count int
	if err := r.store.Pool().QueryRow(ctx, query, before).Scan(&count); err != nil {
