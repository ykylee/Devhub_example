...
func pullUserEvents(
	...
	if !latestTime.IsZero() {
		if err := cursors.UpsertEventCursor(ctx, repository.EventCursor{
...
// upsert 후 반환.
func loadCursor(ctx context.Context, cursors repository.EventCursorStore, key string, now func() time.Time) (repository.EventCursor, error) {
	c, err := cursors.GetEventCursor(ctx, key)
	if err == nil {
		return c, nil
	}
	if isNotFound(err) {
		seed := repository.EventCursor{
			CursorKey:   key,
			LastEventAt: now().UTC(),
		}
		if upsertErr := cursors.UpsertEventCursor(ctx, seed); upsertErr != nil {
			return repository.EventCursor{}, fmt.Errorf("seed cursor %s on first-run init: %w", key, upsertErr)
		}
		return seed, nil
	}
	return repository.EventCursor{}, fmt.Errorf("load cursor %s: %w", key, err)
}
