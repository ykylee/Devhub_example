...
// TestEndToEnd_PortToAuditEmitter — sprint -v PR-C integration test.
// integration.KeycloakEventPort → audit pullOnce → AuditEmitter 흐름을
// 1개 process 안에서 검증.
//
// v1.1 sprint -a follow-up PR1: v1.0 의 HTTPAPIEventLister adapter 가 제거됨
// (canonical port 직접 주입). mock 은 integration.KeycloakEventPort 를 직접
// 충족 — wire 시 adapter wrapper 불요.
//
...
func TestEndToEnd_PortToAuditEmitter(t *testing.T) {
	t0 := time.Date(2026, 5, 19, 12, 0, 0, 0, time.UTC)
	t1 := t0.Add(10 * time.Second)
	t2 := t0.Add(20 * time.Second)
	t3 := t0.Add(30 * time.Second)

	lister := &mockKeycloakEventPort{
		userEventsByTick: [][]integration.KeycloakUserEvent{
			// tick 1
		...
		adminEventsByTick: [][]integration.KeycloakAdminEvent{
			// tick 1
			{
				{
					Time:          t2.UnixMilli(),
	...
	// tick 1
	if err := pullOnce(ctx, lister, cursorStore, opts, func() time.Time { return t1.Add(1 * time.Second) }); err != nil {
		t.Fatalf("tick 1: %v", err)
	}
	// tick 2 — dateFrom inclusive 로 boundary 재등장. emit 추가 없어야 함.
	if err := pullOnce(ctx, lister, cursorStore, opts, func() time.Time { return t2.Add(1 * time.Second) }); err != nil {
		t.Fatalf("tick 2: %v", err)
	}
	// tick 3 — 신규 user event 1건 emit.
	if err := pullOnce(ctx, lister, cursorStore, opts, func() time.Time { return t3.Add(1 * time.Second) }); err != nil {
	...
	// payload 가 ip_address / user_id / auth_user_id 등 payload 키 포함 확인.
...
// mockKeycloakEventPort — integration.KeycloakEventPort 의 in-memory mock.
// v1.1 sprint -a follow-up PR1: mock 이 canonical port 를 직접 충족.
// sso-integrations/keycloak.KeycloakAdminClient 와 동일 시그니처.
type mockKeycloakEventPort struct {
	mu                sync.Mutex
	userEventsByTick  [][]integration.KeycloakUserEvent
	adminEventsByTick [][]integration.KeycloakAdminEvent
	userCallCount     int
...
func (m *mockKeycloakEventPort) ListUserEvents(_ context.Context, _ time.Time, _ int) ([]integration.KeycloakUserEvent, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	idx := m.userCallCount
	m.userCallCount++
...
func (m *mockKeycloakEventPort) ListAdminEvents(_ context.Context, _ time.Time, _ int) ([]integration.KeycloakAdminEvent, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
