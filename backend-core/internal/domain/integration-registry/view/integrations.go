...
func (h *IntegrationHandler) ListIntegrations(c *gin.Context) {
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "integrations.list")
		return
	}
...
func (h *IntegrationHandler) UpdateIntegration(c *gin.Context) {
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "integrations.update.lookup")
		return
	}
	updated := current
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "integrations.update")
		return
	}
...
func (h *IntegrationHandler) DeleteIntegration(c *gin.Context) {
	...
	if err := storeI.DeleteIntegration(c.Request.Context(), id); errors.Is(err, store.ErrNotFound) {
		...
		httphelp.WriteServerError(c, err, "integrations.delete")
		return
	}
