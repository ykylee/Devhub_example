...
func (h *RepositoryIntegrationHandler) scmProviderForCapability(c *gin.Context, storeI PlatformStore, capability string) (domain.IntegrationProvider, bool) {
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "integration.scm.provider.lookup")
		return domain.IntegrationProvider{}, false
	}
...
func (h *RepositoryIntegrationHandler) scmProviderClient(c *gin.Context, provider domain.IntegrationProvider) (*gitea.Client, bool) {
...
func (h *RepositoryIntegrationHandler) ListSCMRepositories(c *gin.Context) {
	...
	if err != nil {
		httphelp.WriteServerError(c, err, "integration.scm.repositories.imported")
		return
	}
...
func (h *RepositoryIntegrationHandler) ImportSCMRepositories(c *gin.Context) {
	...
	for fn := range selected {
		...
		if err := storeI.UpsertRepository(c.Request.Context(), domain.Repository{
			...
			httphelp.WriteServerError(c, err, "integration.scm.repositories.import")
			return
		}
...
func (h *RepositoryIntegrationHandler) CreateSCMRepository(c *gin.Context) {
	...
	if err := storeI.UpsertRepository(c.Request.Context(), domain.Repository{
		...
		httphelp.WriteServerError(c, err, "integration.scm.repositories.create.persist")
		return
	}
