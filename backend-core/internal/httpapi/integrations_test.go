...
// 1) POST /integrations — happy (scope=platform).
func TestCreateIntegration_HappyApplicationScope(t *testing.T) {
	...
	rec := doJSON(t, router, http.MethodPost, "/api/v1/integrations",
		`{"scope":"platform","platform_id":"app-1","integration_type":"jira","external_key":"PROJ","url":"https://example.atlassian.net","policy":"summary_only"}`)
	if rec.Code != http.StatusCreated {
		t.Fatalf("status=%d body=%s", rec.Code, rec.Body.String())
	}
	if !bytes.Contains(rec.Body.Bytes(), []byte(`"scope":"platform"`)) {
		t.Errorf("expected scope=platform: %s", rec.Body.String())
	}
}

// 2) POST /integrations — scope=platform 인데 platform_id 없음 → 422.
func TestCreateIntegration_ScopeTargetMismatch(t *testing.T) {
	router := newPlatformsRouter(newMemoryPlatformStore())
	rec := doJSON(t, router, http.MethodPost, "/api/v1/integrations",
		`{"scope":"platform","integration_type":"jira","external_key":"PROJ","url":"https://x","policy":"summary_only"}`)
...
func TestCreateIntegration_InvalidPolicy(t *testing.T) {
	router := newPlatformsRouter(newMemoryPlatformStore())
	rec := doJSON(t, router, http.MethodPost, "/api/v1/integrations",
		`{"scope":"platform","platform_id":"app-1","integration_type":"jira","external_key":"X","url":"https://x","policy":"forbidden"}`)
...
func TestCreateIntegration_Duplicate(t *testing.T) {
	router := newPlatformsRouter(newMemoryPlatformStore())
	body := `{"scope":"platform","platform_id":"app-1","integration_type":"jira","external_key":"PROJ","url":"https://x","policy":"summary_only"}`
...
func TestUpdateIntegration_UniqueConflict(t *testing.T) {
	...
	first := doJSON(t, router, http.MethodPost, "/api/v1/integrations",
		`{"scope":"platform","platform_id":"app-1","integration_type":"jira","external_key":"PROJ-A","url":"https://x","policy":"summary_only"}`)
	if first.Code != http.StatusCreated {
		t.Fatalf("seed 1 failed: %s", first.Body.String())
	}
	second := doJSON(t, router, http.MethodPost, "/api/v1/integrations",
		`{"scope":"platform","platform_id":"app-1","integration_type":"jira","external_key":"PROJ-B","url":"https://x","policy":"summary_only"}`)
