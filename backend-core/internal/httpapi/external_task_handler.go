...
func (h Handler) receiveExternalTaskWebhook(c *gin.Context) {
	...
	if err != nil {
		...
		writeServerError(c, err, "external_task.webhook.lookup")
		return
	}
	...
	if event.Event == "deleted" {
		if err := taskStore.SoftDeleteExternalTaskItem(c.Request.Context(), providerID, event.ExternalID); err != nil && !errors.Is(err, store.ErrNotFound) {
			writeServerError(c, err, "external_task.webhook.delete")
			return
		}
		_, _ = h.recordAudit(c, "external_task.deleted", "external_task", "", map[string]any{
			"provider_id":  providerID,
		...
		if err != nil {
			writeServerError(c, err, "external_task.webhook.upsert")
			return
		}
		...
		_, _ = h.recordAudit(c, auditAction, "external_task", "", map[string]any{
			"provider_id":  providerID,
			"external_id":  event.ExternalID,
			"webhook_seq":  seq,
		})
...
func (h Handler) listExternalTaskItems(c *gin.Context) {
	...
	if err != nil {
		writeServerError(c, err, "external_task.list")
		return
	}
...
func (h Handler) getExternalTaskItem(c *gin.Context) {
	...
	if err != nil {
		...
		writeServerError(c, err, "external_task.get")
		return
	}
...
func (h Handler) externalTaskStoreOrUnavailable(c *gin.Context) (ExternalTaskStore, bool) {
